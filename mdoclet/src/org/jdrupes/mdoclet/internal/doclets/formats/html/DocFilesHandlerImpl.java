...
import org.jdrupes.mdoclet.internal.doclets.toolkit.DocFileElement;
import org.jdrupes.mdoclet.internal.doclets.toolkit.DocFilesHandler;
...
public class DocFilesHandlerImpl implements DocFilesHandler {
     ...
     *
     */
    public DocFilesHandlerImpl(HtmlConfiguration configuration,
            Element element) {
        ...
        case PACKAGE -> {
            ...
            source = DocPath
                .create(pkg.getQualifiedName().toString().replace('.', '/'))
    ...
    public void copyDocFiles() throws DocFileIOException {
        ...
        for (DocFile srcdir : DocFile.list(configuration, location, source)) {
            ...
            case PACKAGE -> configuration.docPaths
                .forPackage((PackageElement) this.element);
            default -> throw new AssertionError(
                "unknown kind:" + this.element.getKind());
            };
        ...
        for (DocFile srcdir : DocFile.list(configuration, location, source)) {
            for (DocFile srcFile : srcdir.list()) {
                if (srcFile.getName().endsWith(".css"))
                    stylesheets
                        .add(DocPaths.DOC_FILES.resolve(srcFile.getName()));
            }
        }
        return stylesheets;
    }
    ...
    private void copyDirectory(DocFile srcdir, final DocPath dstDocPath,
        ...
        for (DocFile srcfile : srcdir.list()) {
            // ensure that the name is a valid component in an eventual full
            // path
            ...
            if (srcfile.isFile()) {
                if (destfile.exists() && !first) {
                    configuration.messages.warning(
                        "doclet.Copy_Overwrite_warning",
                        srcfile.getPath(), dstdir.getPath());
                } else {
                    if (Utils.toLowerCase(srcfile.getPath())
                        .endsWith(".html")) {
                        handleHtmlFile(srcfile, dstDocPath);
                    } else {
                        configuration.messages.notice(
                ...
                if (options.copyDocfileSubdirs()
                    && !configuration
                        .shouldExcludeDocFileDir(srcfile.getName())) {
    ...
    private void handleHtmlFile(DocFile srcfile, DocPath dstPath)
            throws DocFileIOException {
        Utils utils = configuration.utils;
        FileObject fileObject = srcfile.getFileObject();
        DocFileElement dfElement
            = new DocFileElement(utils, element, fileObject);

        DocPath dfilePath = dstPath.resolve(srcfile.getName());
        PackageElement pkg = dfElement.getPackageElement();

        HtmlDocletWriter docletWriter
            = new DocFileWriter(configuration, dfilePath, element, pkg);

        List<? extends DocTree> localTags
            = getLocalHeaderTags(utils.getPreamble(dfElement));
        Content localTagsContent
            = docletWriter.commentTagsToContent(dfElement, localTags, false);
        ...
        Content pageContent
            = docletWriter.commentTagsToContent(dfElement, fullBody, false);
        ...
        docletWriter.printHtmlDocument(List.of(), null, localTagsContent,
            List.of(), htmlContent);
    }

    private List<? extends DocTree>
            getLocalHeaderTags(List<? extends DocTree> dtrees) {
        List<DocTree> localTags = new ArrayList<>();
        DocTreeFactory docTreeFactory
            = configuration.docEnv.getDocTrees().getDocTreeFactory();
        boolean inHead = false;
        boolean inTitle = false;
        loop: for (DocTree dt : dtrees) {
            switch (dt.getKind()) {
            case START_ELEMENT:
    ...
    private String getWindowTitle(HtmlDocletWriter docletWriter,
            Element element) {
    ...
    private static class DocFileWriter extends HtmlDocletWriter {
         ...
         */
        public DocFileWriter(HtmlConfiguration configuration, DocPath path,
                Element e, PackageElement pkg) {
