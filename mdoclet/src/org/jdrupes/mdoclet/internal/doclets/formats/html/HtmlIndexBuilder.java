...
public class HtmlIndexBuilder extends IndexBuilder {
        ...
        for (Character ch : getFirstCharacters()) {
            for (IndexItem item : getItems(ch)) {
                if (item.isElementItem()) {
                    boolean addModuleInfo = duplicateLabelCheck
                        .get(item.getFullyQualifiedLabel(utils)) > 1;
    ...
    private void addContainingInfo(IndexItem item, boolean addModuleInfo) {
        ...
        case INTERFACE:
            item.setContainingPackage(
                utils.getPackageName(utils.containingPackage(element)));
        ...
        case ENUM_CONSTANT:
            ...
            item.setContainingPackage(
                utils.getPackageName(utils.containingPackage(containingType)));
            ...
            if (utils.isExecutableElement(element)) {
                String url = HtmlTree.encodeURL(
                    htmlIds.forMember((ExecutableElement) element).name());
    ...
    private void createSearchIndexFile(DocPath searchIndexJS,
        ...
        DocFile jsFile
            = DocFile.createFileForOutput(configuration, searchIndexJS);
        try (Writer wr = jsFile.openWriter()) {
            ...
            throw new DocFileIOException(jsFile, DocFileIOException.Mode.WRITE,
                ie);
        }
    }
}
