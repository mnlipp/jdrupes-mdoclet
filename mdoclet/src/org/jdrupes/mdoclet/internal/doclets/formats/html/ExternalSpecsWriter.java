...
import org.jdrupes.mdoclet.internal.doclets.toolkit.OverviewElement;
...
public class ExternalSpecsWriter extends HtmlDocletWriter {
    ...
    private static void generate(HtmlConfiguration configuration,
        ...
        ExternalSpecsWriter w
            = new ExternalSpecsWriter(configuration, fileName);
        w.buildExternalSpecsPage();
        configuration.conditionalPages
            .add(HtmlConfiguration.ConditionalPage.EXTERNAL_SPECS);
    }
    ...
    protected void buildExternalSpecsPage() throws DocFileIOException {
        ...
        if (configuration.mainIndex != null) {
            configuration.mainIndex
                .add(IndexItem.of(IndexItem.Category.TAGS, title, path));
        }
    }

    protected void checkUniqueItems() {
        Map<String, Map<String, List<IndexItem>>> itemsByURL = new HashMap<>();
        Map<String, Map<String, List<IndexItem>>> itemsByTitle
            = new HashMap<>();
        for (IndexItem ii : configuration.mainIndex
            .getItems(DocTree.Kind.SPEC)) {
            if (ii.getDocTree() instanceof SpecTree st) {
                String url = st.getURL().toString();
                String title = ii.getLabel(); // normalized form of
                                              // st.getTitle()
                itemsByTitle
        ...
        itemsByURL.forEach((url, title) -> {
            if (title.size() > 1) {
                ...
                title.forEach((t, list) -> list.forEach(
                    ii -> report(ii, "doclet.extSpec.url.title", url, t)));
            }
        });
        ...
        itemsByTitle.forEach((title, urls) -> {
            if (urls.size() > 1) {
                ...
                urls.forEach((u, list) -> list.forEach(
                    ii -> report(ii, "doclet.extSpec.title.url", title, u)));
            }
        });
    }
    ...
    private void report(IndexItem ii, String key, Object... args) {
        ...
        if (e == null) {
            ...
            DocTreePath dtp = new DocTreePath(
                new DocTreePath(tp, utils.getDocCommentTree(e)),
                ii.getDocTree());
    ...
    protected void addExternalSpecs(Content content) {
        ...
        var table = new Table<URI>(HtmlStyle.summaryTable)
            ...
            .setHeader(new TableHeader(contents.specificationLabel,
        ...
        if ((hostNamesList.size() + (noHost ? 1 : 0)) > 1) {
            ...
            if (noHost) {
                table.addTab(
                    Text.of(resources
                        .getText("doclet.External_Specifications.no-host")),
                    u -> u.getHost() == null);
            }
        }
        table.setDefaultTab(Text.of(resources
            .getText("doclet.External_Specifications.All_Specifications")));
        ...
        for (List<IndexItem> searchIndexItems : searchIndexMap.values()) {
            ...
            Content referencesList
                = HtmlTree.UL(HtmlStyle.refList, searchIndexItems,
                    item -> HtmlTree.LI(createLink(item)));
            Content references
                = searchIndexItems.size() < USE_DETAILS_THRESHHOLD
                    ? referencesList
                    : HtmlTree.DETAILS()
                        .add(HtmlTree
                            .SUMMARY(contents.getContent("doclet.references",
    ...
    private Map<String, List<IndexItem>> groupExternalSpecs() {
        return configuration.mainIndex.getItems(DocTree.Kind.SPEC).stream()
            .collect(groupingBy(IndexItem::getLabel,
                () -> new TreeMap<>(getTitleComparator()), toList()));
    }
        ...
        return new Comparator<>() {
            ...
            public int compare(String s1, String s2) {
                ...
                while (i1 < s1.length() && i2 < s2.length()) {
                    ...
                    int cmp = collator.compare(s1.substring(i1, j1),
                        s2.substring(i2, j2));
                    if (cmp != 0) {
                        return cmp;
                    }
                    ...
                    cmp = Integer.compare(
                        Integer.parseInt(s1.substring(j1, k1)),
                        Integer.parseInt(s2.substring(j2, k2)));
                    if (cmp != 0) {
                        return cmp;
                    }
                    i1 = k1;
    ...
    private Content createLink(IndexItem i) {
        ...
        if (element instanceof OverviewElement) {
            ...
            Content link = links.createLink(pathToRoot.resolve(i.getUrl()),
                i.getHolder());
    ...
    private Content createSpecLink(IndexItem i) {
        ...
        try {
            ...
            // should not happen: items with bad URIs should not make it into
            // the index
            return title;
        }
    }
