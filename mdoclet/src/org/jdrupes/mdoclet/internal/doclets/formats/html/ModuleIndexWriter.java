...
public class ModuleIndexWriter extends AbstractOverviewIndexWriter {
    ...
    public static void generate(HtmlConfiguration configuration)
        ...
        ModuleIndexWriter mdlgen
            = new ModuleIndexWriter(configuration, filename);
        mdlgen.buildOverviewIndexFile("doclet.Window_Overview_Summary",
            "module index");
    }
    ...
    protected void addIndex(Content target) {
        ...
        if (!groupModuleMap.keySet().isEmpty()) {
            TableHeader tableHeader = new TableHeader(contents.moduleLabel,
            ...
            for (ModuleElement mdle : modules) {
                if (!mdle.isUnnamed()) {
                    if (!(options.noDeprecated() && utils.isDeprecated(mdle))) {
                        Content moduleLinkContent = getModuleLink(mdle,
