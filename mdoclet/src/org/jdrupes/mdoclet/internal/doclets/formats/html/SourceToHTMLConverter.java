...
public class SourceToHTMLConverter {
    ...
    private SourceToHTMLConverter(HtmlConfiguration configuration,
            DocPath outputdir) {
     ...
     */
    public static void convertRoot(HtmlConfiguration configuration,
            DocPath outputdir)
    ...
    void generate() throws DocFileIOException, SimpleDocletException {
        ...
        for (ModuleElement mdl : configuration.getSpecifiedModuleElements()) {
            // If -nodeprecated option is set and the module is marked as
            // deprecated,
        ...
        for (TypeElement te : configuration.getSpecifiedTypeElements()) {
            // If -nodeprecated option is set and the class is marked as
            // deprecated
    ...
    public void convertClass(TypeElement te, DocPath outputdir)
        ...
        try {
            ...
            writeToFile(body,
                outputdir.resolve(configuration.docPaths.forClass(te)), te);
        } catch (IOException e) {
            String message
                = resources.getText("doclet.exception.read.file", fo.getName());
     ...
     */
    private void writeToFile(Content body, DocPath path, TypeElement te)
...
//                .setCharset(options.charset) // temporary: compatibility!
        ...
        var html = HtmlTree.HTML(configuration.getLocale().getLanguage(), head,
            body);
    ...
    public void addStyleSheetProperties(Content head) {
        ...
        var link
            = HtmlTree.LINK("stylesheet", "text/css", p.getPath(), "Style");
        head.add(link);
        addStylesheets(head);
    }
    ...
    protected void addStylesheets(Content head) {
        options.additionalStylesheets().forEach(css -> {
            ...
            var slink = HtmlTree.LINK("stylesheet", "text/css",
                relativePath.resolve(cssPath).getPath(),
                "Style");
            head.add(slink);
        });
    }
