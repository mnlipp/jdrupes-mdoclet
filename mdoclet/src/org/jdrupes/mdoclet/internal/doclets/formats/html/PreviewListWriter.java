 ...
 * appropriate links.
 */
public class PreviewListWriter
        extends SummaryListWriter<PreviewAPIListBuilder> {
    ...
    public static void generate(HtmlConfiguration configuration)
        ...
        if (configuration.conditionalPages
            ...
            PreviewListWriter depr
                = new PreviewListWriter(configuration, filename);
            depr.generateSummaryListFile(PageMode.PREVIEW, "preview elements",
                configuration.contents.previewAPI,
                "doclet.Window_Preview_List");
        }
    }
    ...
    protected void addContentSelectors(Content target) {
        ...
        if (!jeps.isEmpty()) {
            int index = 0;
            target.add(HtmlTree
                .P(contents.getContent("doclet.Preview_API_Checkbox_Label")));
            Content list = HtmlTree.UL(HtmlStyle.previewFeatureList);
            for (var jep : jeps) {
                index++;
                HtmlId htmlId = HtmlId.of("feature-" + index);
                String jepUrl
                    = resources.getText("doclet.Preview_JEP_URL", jep.number());
                list.add(HtmlTree.LI(HtmlTree.LABEL(htmlId.name(),
                    ...
                    .add(HtmlTree.SPAN(Text.of(jep.number() + ": "))
                        .add(HtmlTree.A(jepUrl, Text
                            .of(jep.title() + " (" + jep.status() + ")"))))));
            }
            target.add(list);
        }
    }
    ...
    protected Content getExtraContent(Element element) {
        PreviewAPIListBuilder.JEP jep
            = configuration.previewAPIListBuilder.getJEP(element);
