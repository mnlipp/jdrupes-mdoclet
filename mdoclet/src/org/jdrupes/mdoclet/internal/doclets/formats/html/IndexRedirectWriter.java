...
public class IndexRedirectWriter extends HtmlDocletWriter {
    ...
    public static void generate(HtmlConfiguration configuration,
            DocPath fileName, DocPath target)
            throws DocFileIOException {
        IndexRedirectWriter indexRedirect
            = new IndexRedirectWriter(configuration, fileName, target);
        indexRedirect.generateIndexFile();
    }

    private DocPath target;

    private IndexRedirectWriter(HtmlConfiguration configuration,
            DocPath filename, DocPath target) {
    ...
    private void generateIndexFile() throws DocFileIOException {
        Head head = new Head(path, configuration.getDocletVersion(),
            configuration.getBuildDate())
                ...
                .setStylesheets(configuration.getMainStylesheet(), List.of()) // avoid
                                                                              // reference
                                                                              // to
                                                                              // default
                                                                              // stylesheet
        ...
        bodyContent
            .add(HtmlTree.P(HtmlTree.A(targetPath, Text.of(targetPath))));

        var body
            = new HtmlTree(TagName.BODY).setStyle(HtmlStyle.indexRedirectPage);
