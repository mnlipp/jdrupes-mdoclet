...
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.SummaryAPIListBuilder.SummaryElementKind;
...
public abstract class SummaryListWriter<B extends SummaryAPIListBuilder>
     ...
     */
    protected void generateSummaryListFile(PageMode pageMode,
            String description,
        ...
        content.add(HtmlTree.HEADING_TITLE(Headings.CONTENT_HEADING,
        ...
        for (SummaryElementKind kind : SummaryElementKind.values()) {
            if (builder.hasDocumentation(kind)) {
                addSummaryAPI(builder.getSet(kind),
                    HtmlIds.forSummaryKind(kind),
                    getHeadingKey(kind), getHeaderKey(kind), content);
            }
        }
        bodyContents.addMainContent(content);
        // The script below enables checkboxes in the page and invokes their
        // click handler
        // to restore any previous state when the page is loaded via
        // back/forward button.
    ...
    protected void addIndexLink(HtmlId id, String headingKey, Content content) {
        // The "contents-" + id value is used in JavaScript code to toggle
        // visibility of the link.
        var li = HtmlTree.LI(links.createLink(id,
            contents.getContent(headingKey)))
            .setId(HtmlId.of("contents-" + id.name()));
        content.add(li);
    }
    ...
    public Content getContentsList() {
        ...
        for (SummaryElementKind kind : SummaryElementKind.values()) {
            if (builder.hasDocumentation(kind)) {
                addIndexLink(HtmlIds.forSummaryKind(kind), getHeadingKey(kind),
                    ul);
            }
        }
        return ul;
    ...
    protected Content getSummaryLink(Element e) {
        ...
        case INTERFACE, CLASS, ENUM, ANNOTATION_TYPE, RECORD -> new NestedClassWriterImpl(
            this);
        ...
        case RECORD_COMPONENT -> throw new AssertionError(
            "Record components are not supported by SummaryListWriter!");
        default -> throw new UnsupportedOperationException(
            "Unsupported element kind: " + e.getKind());
        };
     ...
     */
    protected abstract void addTableTabs(Table<Element> table,
            String headingKey);
}
