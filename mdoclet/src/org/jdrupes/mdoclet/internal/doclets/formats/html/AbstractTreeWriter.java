...
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.ClassTree.Hierarchy;
...
public abstract class AbstractTreeWriter extends HtmlDocletWriter {
     ...
     */
    protected void addLevelInfo(TypeElement parent,
     ...
     */
    protected void addTree(Hierarchy hierarchy, String heading,
            Content content) {
        ...
        if (!roots.isEmpty()) {
            ...
            var sectionHeading
                = HtmlTree.HEADING_TITLE(Headings.CONTENT_HEADING,
                    headingContent);
            var section = HtmlTree.SECTION(HtmlStyle.hierarchy, sectionHeading);
            addLevelInfo(
                !utils.isPlainInterface(firstTypeElement) ? firstTypeElement
                    : null,
    ...
    protected void addExtendsImplements(TypeElement parent,
        ...
        typeElement.getInterfaces()
            .forEach(t -> interfaces.add(utils.asTypeElement(t)));
                ...
                if (parent != intf) {
                    if (utils.isPublic(intf) || utils.isLinkable(intf)) {
                        ...
                        addPreQualifiedClassLink(
                            HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS, intf, content);
                    }
                }
            }
            if (!isFirst) {
    ...
    protected void addPartialInfo(TypeElement typeElement, Content content) {
        addPreQualifiedStrongClassLink(HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS,
            typeElement, content);
    }
}
