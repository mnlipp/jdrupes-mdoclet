...
public class NewAPIListWriter extends SummaryListWriter<NewAPIBuilder> {
    ...
    public static void generate(HtmlConfiguration configuration)
        ...
        if (configuration.conditionalPages
            ...
            NewAPIListWriter writer
                = new NewAPIListWriter(configuration, DocPaths.NEW_LIST);
            ...
            for (String release : releases) {
                table.addTab(
                    ...
                    element -> {
                        ...
                        return since.stream().anyMatch(tree -> release
                            .equals(ch.getBody(tree).toString()));
                    });
            }
        }
    }
    ...
    protected Content getTableCaption(String headingKey) {
        return contents.getContent("doclet.New_Elements",
            super.getTableCaption(headingKey));
    }
    ...
    private List<? extends DocTree> getSinceTree(Element element) {
        return utils.hasDocCommentTree(element)
            ? utils.getBlockTags(element, SINCE)
            : List.of();
    }
    ...
    private static String getHeading(HtmlConfiguration configuration) {
        ...
        return label == null
            ? configuration.docResources.getText("doclet.New_API")
            : label;
    }
}
