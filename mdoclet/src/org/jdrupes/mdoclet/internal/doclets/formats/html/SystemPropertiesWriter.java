...
public class SystemPropertiesWriter extends HtmlDocletWriter {
    ...
    private static void generate(HtmlConfiguration configuration,
        ...
        boolean hasSystemProperties = configuration.mainIndex != null
            && !configuration.mainIndex.getItems(DocTree.Kind.SYSTEM_PROPERTY)
                .isEmpty();
        ...
        SystemPropertiesWriter systemPropertiesGen
            = new SystemPropertiesWriter(configuration, fileName);
        systemPropertiesGen.buildSystemPropertiesPage();
        configuration.conditionalPages
            .add(HtmlConfiguration.ConditionalPage.SYSTEM_PROPERTIES);
    }
    ...
    protected void addSystemProperties(Content target) {
        ...
        var table = new Table<Void>(HtmlStyle.summaryTable)
            ...
            .setHeader(
                new TableHeader(contents.propertyLabel, contents.referencedIn))
        ...
        for (Entry<String, List<IndexItem>> entry : searchIndexMap.entrySet()) {
            ...
            for (int i = 1; i < searchIndexItems.size(); i++) {
                ...
                separatedReferenceLinks
                    .add(createLink(searchIndexItems.get(i)));
            }
            table.addRow(propertyName,
                HtmlTree.DIV(HtmlStyle.block, separatedReferenceLinks));
        }
        target.add(table);
    }

    private Map<String, List<IndexItem>> groupSystemProperties() {
        return configuration.mainIndex.getItems(DocTree.Kind.SYSTEM_PROPERTY)
            .stream()
            .collect(groupingBy(IndexItem::getLabel, TreeMap::new,
