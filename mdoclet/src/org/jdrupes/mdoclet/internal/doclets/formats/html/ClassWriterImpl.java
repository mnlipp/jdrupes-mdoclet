...
public class ClassWriterImpl extends SubWriterHolderWriter
     ...
     */
    public ClassWriterImpl(HtmlConfiguration configuration,
    ...
    public Content getHeader(String header) {
        HtmlTree body
            = getBody(getWindowTitle(utils.getSimpleName(typeElement)));
        var div = HtmlTree.DIV(HtmlStyle.header);
        if (configuration.showModules) {
            ModuleElement mdle = configuration.docEnv.getElementUtils()
                .getModuleOf(typeElement);
            var classModuleLabel = HtmlTree.SPAN(HtmlStyle.moduleLabelInType,
        ...
        if (!pkg.isUnnamed()) {
            var classPackageLabel = HtmlTree.SPAN(HtmlStyle.packageLabelInType,
                contents.packageLabel);
            var pkgNameDiv
                = HtmlTree.DIV(HtmlStyle.subTitle, classPackageLabel);
    ...
    protected Navigation getNavBar(PageMode pageMode, Element element) {
        Content linkContent
            = getModuleLink(utils.elementUtils.getModuleOf(element),
                contents.moduleLabel);
        return super.getNavBar(pageMode, element)
            ...
            .setSubNavLinks(() -> {
                ...
                VisibleMemberTable vmt
                    = configuration.getVisibleMemberTable(typeElement);
                ...
                for (VisibleMemberTable.Kind kind : summarySet) {
                    list.add(links.createLink(HtmlIds.forMemberSummary(kind),
                        contents.getNavLinkLabelContent(kind),
                        vmt.hasVisibleMembers(kind)));
                }
                return list;
            });
    }
    ...
    public void printDocument(Content content) throws DocFileIOException {
        ...
        printHtmlDocument(
            configuration.metakeywords.getMetaKeywords(typeElement),
            description, localStylesheets, content);
    }

    @Override
    public Content getClassInfo(Content classInfo) {
        return getMember(HtmlIds.CLASS_DESCRIPTION, HtmlStyle.classDescription,
            classInfo);
    }
    ...
    private Content getClassInheritanceTreeContent(TypeMirror type) {
        ...
        do {
            ...
            var entry = HtmlTree.DIV(HtmlStyle.inheritance,
                getClassHelperContent(type));
            if (classTree != null)
                entry.add(classTree);
            classTree = entry;
            type = sup;
        } while (sup != null);
        classTree.put(HtmlAttr.TITLE,
            contents.getContent("doclet.Inheritance_Tree").toString());
        return classTree;
    }
    ...
    private Content getClassHelperContent(TypeMirror type) {
        ...
        if (utils.typeUtils.isSameType(type, typeElement.asType())) {
            ...
            if (configuration.shouldExcludeQualifier(
                utils.containingPackage(typeElement).toString())) {
            ...
            Content link = getLink(new HtmlLinkInfo(configuration,
                HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS, type)
                    .label(
                        configuration.getClassName(utils.asTypeElement(type))));
            result.add(link);
        }
        return result;
    }
    ...
    public void addParamInfo(Content target) {
        if (utils.hasBlockTag(typeElement, DocTree.Kind.PARAM)) {
            Content paramInfo
                = (new ParamTaglet()).getAllBlockTagOutput(typeElement,
    ...
    public void addSubClassInfo(Content target) {
        if (utils.isClass(typeElement)) {
            ...
            Set<TypeElement> subclasses
                = classTree.hierarchy(typeElement).subtypes(typeElement);
            if (!subclasses.isEmpty()) {
                var dl = HtmlTree.DL(HtmlStyle.notes);
                dl.add(HtmlTree.DT(contents.subclassesLabel));
                dl.add(HtmlTree
                    .DD(getClassLinks(HtmlLinkInfo.Kind.PLAIN, subclasses)));
                target.add(dl);
            }
        }
    }
    ...
    public void addSubInterfacesInfo(Content target) {
        if (utils.isPlainInterface(typeElement)) {
            Set<TypeElement> subInterfaces
                = classTree.hierarchy(typeElement).allSubtypes(typeElement);
            if (!subInterfaces.isEmpty()) {
                var dl = HtmlTree.DL(HtmlStyle.notes);
                dl.add(HtmlTree.DT(contents.subinterfacesLabel));
                dl.add(HtmlTree.DD(getClassLinks(
                    HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS, subInterfaces)));
                target.add(dl);
            }
        }
    }
    ...
    public void addImplementedInterfacesInfo(Content target) {
        ...
        if (utils.isClass(typeElement) && !interfaces.isEmpty()) {
            ...
            dl.add(HtmlTree.DD(
                getClassLinks(HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS, interfaces)));
            target.add(dl);
        }
    }
    ...
    public void addNestedClassInfo(final Content target) {
        ...
        new SimpleElementVisitor8<Void, Void>() {
            ...
            public Void visitType(TypeElement e, Void p) {
                ...
                dl.add(HtmlTree.DD(
                    getClassLinks(HtmlLinkInfo.Kind.LINK_TYPE_PARAMS_AND_BOUNDS,
                        List.of(e))));
                target.add(dl);
                return null;
            }
    ...
    public void addClassDeprecationInfo(Content classInfo) {
        ...
        if (utils.isDeprecated(typeElement)) {
            var deprLabel = HtmlTree.SPAN(HtmlStyle.deprecatedLabel,
                getDeprecatedPhrase(typeElement));
     ...
     * @return the links
     */
    private Content getClassLinks(HtmlLinkInfo.Kind context,
            Collection<?> list) {
        ...
        for (Object type : list) {
            ...
            if (type instanceof TypeElement te) {
                ...
                Content link = getLink(
                    new HtmlLinkInfo(configuration, context,
                        ((TypeMirror) type)));
                content.add(HtmlTree.CODE(link));
            }
        }
