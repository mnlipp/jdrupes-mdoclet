...
public class Table<T> extends Content {
    ...
    public Table<T> addTab(Content label, Predicate<T> predicate) {
        if (tabs == null) {
            tabs = new ArrayList<>();         // preserves order that tabs are
                                              // added
            occurringTabs = new HashSet<>();  // order not significant
        }
        // Use current size of tabs list as id so we have tab ids that are
        // consistent
    ...
    public void addRow(T item, List<Content> contents) {
        ...
        if (contents.size() != columnStyles.size()) {
            throw new IllegalArgumentException(
                "row content size does not match number of columns");
        }

        Content row = new ContentBuilder();

        int rowIndex = bodyRows.size();
        HtmlStyle rowStyle = rowIndex % 2 == 0 ? HtmlStyle.evenRowColor
            : HtmlStyle.oddRowColor;

        List<String> tabClasses = new ArrayList<>();
        if (tabs != null) {
            // Construct a series of values to add to the HTML 'class' attribute
            // for the cells of
            // this row, such that there is a default value and a value
            // corresponding to each tab
            // whose predicate matches the item. The values correspond to the
            // equivalent ids.
            // The values are used to determine the cells to make visible when a
            // tab is selected.
        ...
        for (Content c : contents) {
            ...
            var cell = HtmlTree.DIV(cellStyle)
                .addUnchecked(c.isEmpty() ? Text.EMPTY : c);
    ...
    @Override
    public boolean write(Writer out, String newline, boolean atNewline)
            throws IOException {
    ...
    private Content toContent() {
        ...
        if ((tabs == null || occurringTabs.size() == 1)
            && !alwaysShowDefaultTab) {
            if (tabs == null) {
                main.add(caption);
            } else {
            ...
            if (renderTabs) {
                tablist.add(createTab(defaultTabId, HtmlStyle.activeTableTab,
                    true, defaultTab));
            } else {
            ...
            if (renderTabs) {
                for (var tab : tabs) {
                    if (occurringTabs.contains(tab)) {
                        tablist.add(createTab(HtmlIds.forTab(id, tab.index()),
                            HtmlStyle.tableTab, false, tab.label()));
                    }
                }
            }
            if (id == null) {
    ...
    private HtmlTree createTab(HtmlId tabId, HtmlStyle style,
            boolean defaultTab, Content tabLabel) {
        var tab = new HtmlTree(TagName.BUTTON)
            ...
            .put(HtmlAttr.ONCLICK,
                "show('" + id.name() + "', '" + (defaultTab ? id : tabId).name()
                    + "', " + columnStyles.size() + ")")
            .setStyle(style);
        tab.add(tabLabel);
        return tab;
