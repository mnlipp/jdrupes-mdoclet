...
import org.jdrupes.mdoclet.internal.doclets.toolkit.ModuleSummaryWriter;
...
public class ModuleWriterImpl extends HtmlDocletWriter
    ...
    private final Map<PackageElement, PackageEntry> packages
        = new TreeMap<>(utils.comparators.makePackageComparator());
     ...
     */
    private final Map<ModuleElement,
            SortedSet<PackageElement>> indirectOpenPackages
                = new TreeMap<>(comparators.makeModuleComparator());
    ...
    public Content getModuleHeader(String heading) {
        HtmlTree body
            = getBody(getWindowTitle(mdle.getQualifiedName().toString()));
        var div = HtmlTree.DIV(HtmlStyle.header);
        Content moduleHead = new ContentBuilder();
        moduleHead.add(mdle.isOpen()
            && (configuration.docEnv.getModuleMode() == ModuleMode.ALL)
    ...
    protected Navigation getNavBar(PageMode pageMode, Element element) {
        return super.getNavBar(pageMode, element)
            .setSubNavLinks(() -> List.of(
                links.createLink(HtmlIds.MODULE_DESCRIPTION,
    ...
    public void computeModulesData() {
        ...
        Map<ModuleElement, String> dependentModules
            = utils.getDependentModules(mdle);
        // Add all dependent modules to indirect modules set. We will remove the
        // modules,
        // listed using the requires directive, from this set to come up with
        // the table of indirect
        // required modules.
        ...
        ElementFilter.requiresIn(mdle.getDirectives()).forEach(directive -> {
            ...
            if (shouldDocument(m)) {
                if (moduleMode == ModuleMode.ALL || directive.isTransitive()) {
                    ...
                    // For api mode, just keep the public requires in
                    // dependentModules for display of
        ...
        for (PackageElement pkg : utils.getModulePackageMap().getOrDefault(mdle,
            Set.of())) {
            if (shouldDocument(pkg)
                && (mdle.isOpen() || moduleMode == ModuleMode.ALL)) {
        ...
        // Get all exported packages for the module, using the exports directive
        // for the module.
        for (ModuleElement.ExportsDirective directive : ElementFilter
            .exportsIn(mdle.getDirectives())) {
            PackageElement p = directive.getPackage();
            if (shouldDocument(p)) {
                List<? extends ModuleElement> targetMdles
                    = directive.getTargetModules();
                // Include package if in details mode, or exported to all (i.e.
                // targetModules == null)
                if (moduleMode == ModuleMode.ALL || targetMdles == null) {
                    PackageEntry packageEntry = packages.computeIfAbsent(p,
                        pkg -> new PackageEntry());
                    SortedSet<ModuleElement> mdleList = new TreeSet<>(
                        utils.comparators.makeModuleComparator());
                    if (targetMdles != null) {
        ...
        // Get all opened packages for the module, using the opens directive for
        // the module.
        // If it is an open module, there will be no separate opens directives.
        for (ModuleElement.OpensDirective directive : ElementFilter
            .opensIn(mdle.getDirectives())) {
            PackageElement p = directive.getPackage();
            if (shouldDocument(p)) {
                List<? extends ModuleElement> targetMdles
                    = directive.getTargetModules();
                // Include package if in details mode, or opened to all (i.e.
                // targetModules == null)
                if (moduleMode == ModuleMode.ALL || targetMdles == null) {
                    PackageEntry packageEntry = packages.computeIfAbsent(p,
                        pkg -> new PackageEntry());
        ...
        // Get all the exported and opened packages, for the transitive closure
        // of the module, to be displayed in
        // the indirect packages tables.
        dependentModules.forEach((module, mod) -> {
            SortedSet<PackageElement> exportedPackages
                = new TreeSet<>(utils.comparators.makePackageComparator());
            ElementFilter.exportsIn(module.getDirectives())
                .forEach(directive -> {
                    PackageElement pkg = directive.getPackage();
                    if (shouldDocument(pkg)) {
                        // Qualified exports are not displayed in API mode
                        if (moduleMode == ModuleMode.ALL
                            || directive.getTargetModules() == null) {
                            exportedPackages.add(pkg);
                        }
                    }
                });
            // If none of the indirect modules have exported packages to be
            // displayed, we should not be
            // displaying the table and so it should not be added to the map.
            if (!exportedPackages.isEmpty()) {
                indirectPackages.put(module, exportedPackages);
            }
            SortedSet<PackageElement> openPackages
                = new TreeSet<>(utils.comparators.makePackageComparator());
            if (module.isOpen()) {
                openPackages.addAll(
                    utils.getModulePackageMap().getOrDefault(module, Set.of()));
            } else {
                ElementFilter.opensIn(module.getDirectives())
                    .forEach(directive -> {
                        PackageElement pkg = directive.getPackage();
                        if (shouldDocument(pkg)) {
            ...
            // If none of the indirect modules have opened packages to be
            // displayed, we should not be
            // displaying the table and so it should not be added to the map.
        ...
        // Get all the services and implementations listed as provides
        // directive.
        ElementFilter.providesIn(mdle.getDirectives()).forEach(directive -> {
            TypeElement u = directive.getService();
            if (shouldDocument(u)) {
                List<? extends TypeElement> implList
                    = directive.getImplementations();
                SortedSet<TypeElement> implSet = new TreeSet<>(
                    utils.comparators.makeAllClassesComparator());
                implSet.addAll(implList);
                provides.put(u, implSet);
            }
        });
        // Generate the map of all services listed using @provides, and the
        // description.
        utils.getProvidesTrees(mdle).forEach(tree -> {
            TypeElement t = ch.getServiceType(tree);
            if (t != null) {
                providesTrees.put(t, commentTagsToContent(mdle,
                    ch.getDescription(tree), false, true));
            }
        });
        // Generate the map of all services listed using @uses, and the
        // description.
        utils.getUsesTrees(mdle).forEach(tree -> {
            TypeElement t = ch.getServiceType(tree);
            if (t != null) {
                usesTrees.put(t, commentTagsToContent(mdle,
                    ch.getDescription(tree), false, true));
            }
        });
    }
    ...
    private boolean displayServices(Set<TypeElement> typeElements,
        ...
        return typeElements != null &&
            typeElements.stream()
                .anyMatch(v -> displayServiceDirective(v, tagsMap));
    }
    ...
    public void addSummaryHeader(Content startMarker, Content heading,
        ...
        target.add(HtmlTree.HEADING(Headings.ModuleDeclaration.SUMMARY_HEADING,
            heading));
    }
    ...
    public void addModulesSummary(Content summariesList) {
        if (display(requires) || display(indirectModules)) {
            ...
            addSummaryHeader(MarkerComments.START_OF_MODULES_SUMMARY,
     ...
     */
    private void addModulesList(Map<ModuleElement, Content> mdleMap,
            Table<?> table) {
        for (ModuleElement m : mdleMap.keySet()) {
            Content modifiers = mdleMap.get(m);
            Content moduleLink
                = getModuleLink(m, Text.of(m.getQualifiedName()));
    ...
    public void addPackagesSummary(Content summariesList) {
        if (display(packages)
            ...
            addSummaryHeader(MarkerComments.START_OF_PACKAGES_SUMMARY,
            ...
            if (display(indirectPackages)) {
                String aepText
                    = resources.getText("doclet.Indirect_Exports_Summary");
                var aepTable
                    = getTable2(Text.of(aepText), indirectPackagesHeader);
                addIndirectPackages(aepTable, indirectPackages);
                section.add(aepTable);
            }
            if (display(indirectOpenPackages)) {
                String aopText
                    = resources.getText("doclet.Indirect_Opens_Summary");
                var aopTable
                    = getTable2(Text.of(aopText), indirectPackagesHeader);
    ...
    public void addPackageSummary(HtmlTree li) {
        var table = new Table<PackageElement>(HtmlStyle.summaryTable)
            ...
            .addTab(contents.getContent("doclet.Exported_Packages_Summary"),
                this::isExported)
            .addTab(contents.getContent("doclet.Opened_Packages_Summary"),
                this::isOpened)
            .addTab(contents.getContent("doclet.Concealed_Packages_Summary"),
                this::isConcealed);
        ...
        boolean showExportedTo = numExports > 0
            && (numOpens > 0 || numUnqualifiedExports < packages.size());
        boolean showOpenedTo = numOpens > 0
            && (numExports > 0 || numUnqualifiedOpens < packages.size());
    ...
    public void addIndirectPackages(Table<?> table,
        ...
        for (Map.Entry<ModuleElement, SortedSet<PackageElement>> entry : ip
            .entrySet()) {
    ...
    public void addServicesSummary(Content summariesList) {
        ...
        boolean haveProvides
            = displayServices(provides.keySet(), providesTrees);

        if (haveProvides || haveUses) {
            var section = HtmlTree.SECTION(HtmlStyle.servicesSummary)
                .setId(HtmlIds.SERVICES);
            addSummaryHeader(MarkerComments.START_OF_SERVICES_SUMMARY,
                contents.navServices, section);
            TableHeader usesProvidesTableHeader = new TableHeader(
                contents.typeLabel, contents.descriptionLabel);
            if (haveProvides) {
    ...
    public void addUsesList(Table<?> table) {
        ...
        for (TypeElement t : uses) {
            ...
            typeLinkContent = getLink(new HtmlLinkInfo(configuration,
                HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS_AND_BOUNDS, t));
            Content summary = new ContentBuilder();
    ...
    public void addProvidesList(Table<?> table) {
        ...
        Content description;
        for (Map.Entry<TypeElement, SortedSet<TypeElement>> entry : provides
            .entrySet()) {
            ...
            Content srvLinkContent = getLink(new HtmlLinkInfo(configuration,
                HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS_AND_BOUNDS, srv));
            ...
            if (moduleMode == ModuleMode.ALL && !implSet.isEmpty()) {
                ...
                desc.add("(");
                var implSpan = HtmlTree.SPAN(HtmlStyle.implementationLabel,
                ...
                for (TypeElement impl : implSet) {
                    desc.add(sep);
                    desc.add(getLink(new HtmlLinkInfo(configuration,
                        HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS_AND_BOUNDS, impl)));
                    sep = ", ";
                }
                desc.add(")");
            }
    ...
    public void addDeprecationInfo(Content div) {
        ...
        if (utils.isDeprecated(mdle)) {
            ...
            var deprPhrase = HtmlTree.SPAN(HtmlStyle.deprecatedLabel,
                getDeprecatedPhrase(mdle));
    ...
    public void printDocument(Content content) throws DocFileIOException {
        ...
        printHtmlDocument(
            configuration.metakeywords.getMetaKeywordsForModule(mdle),
            getDescription("declaration", mdle), getLocalStylesheets(mdle),
            content);
    }
    ...
    public void addPackageDeprecationInfo(Content li, PackageElement pkg) {
        if (utils.isDeprecated(pkg)) {
            List<? extends DeprecatedTree> deprs
                = utils.getDeprecatedTrees(pkg);
            var deprDiv = HtmlTree.DIV(HtmlStyle.deprecationBlock);
            var deprPhrase = HtmlTree.SPAN(HtmlStyle.deprecatedLabel,
                getDeprecatedPhrase(pkg));
            deprDiv.add(deprPhrase);
            if (!deprs.isEmpty()) {
