...
public class HtmlConfiguration extends BaseConfiguration {
     ...
     */
    public TypeElement currentTypeElement = null;  // Set this TypeElement in
                                                   // the ClassWriter.
    ...
    public enum ConditionalPage {
        CONSTANT_VALUES, DEPRECATED, EXTERNAL_SPECS, PREVIEW, SERIALIZED_FORM,
        SYSTEM_PROPERTIES, NEW
    }
    ...
    public HtmlConfiguration(Doclet doclet, Locale locale, Reporter reporter) {
        ...
        Resources msgResources = new Resources(Locale.getDefault(),
            ...
            "org.jdrupes.mdoclet.internal.doclets.formats.html.resources.standard");

        // Use the provided locale for generated docs
        // Ideally, the doc resources would be in different resource files than
        // the
        // message resources, so that we do not have different copies of the
        // same resources.
    ...
    protected void setTopFile() {
        ...
        if (options.createOverview()) {
            ...
            if (showModules) {
                topFile = DocPath.empty
                    .resolve(docPaths.moduleSummary(modules.first()));
            } else if (!packages.isEmpty()) {
                topFile = docPaths.forPackage(packages.first())
    ...
    public JavaFileObject getOverviewPath() {
        ...
        if (overviewpath != null
            && getFileManager() instanceof StandardJavaFileManager fm) {
    ...
    protected boolean finishOptionSettings0() throws DocletException {
        if (options.docEncoding() == null) {
            if (options.charset() == null) {
                String charset
                    = (options.encoding() == null) ? HTML_DEFAULT_CHARSET
                        : options.encoding();
            ...
            if (options.charset() == null) {
                ...
                messages.error("doclet.Option_conflict", "-charset",
                    "-docencoding");
                return false;
            }
        }
