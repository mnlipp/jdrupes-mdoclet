...
import static org.jdrupes.mdoclet.internal.doclets.formats.html.HtmlLinkInfo.Kind.LINK_TYPE_PARAMS;
import static org.jdrupes.mdoclet.internal.doclets.formats.html.HtmlLinkInfo.Kind.LINK_TYPE_PARAMS_AND_BOUNDS;
import static org.jdrupes.mdoclet.internal.doclets.formats.html.HtmlLinkInfo.Kind.PLAIN;
import static org.jdrupes.mdoclet.internal.doclets.formats.html.HtmlLinkInfo.Kind.SHOW_PREVIEW;
import static org.jdrupes.mdoclet.internal.doclets.formats.html.HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS_AND_BOUNDS;
...
public abstract class AbstractExecutableMemberWriter
    ...
    protected Content getTypeParameters(ExecutableElement member) {
        HtmlLinkInfo linkInfo = new HtmlLinkInfo(configuration,
            LINK_TYPE_PARAMS_AND_BOUNDS, member)
    ...
    protected Content getSummaryLink(Element member) {
        ...
        String signature
            = utils.flatSignature((ExecutableElement) member, typeElement);
        if (signature.length() > 2) {
            content.add(new HtmlTree(TagName.WBR));
        }
        content.add(signature);

        return writer.getDocLink(SHOW_PREVIEW,
            utils.getEnclosingTypeElement(member),
            member, content, null, false);
    }

    @Override
    protected void addSummaryLink(HtmlLinkInfo.Kind context, TypeElement te,
            Element member,
            Content target) {
        ExecutableElement ee = (ExecutableElement) member;
        Content memberLink = writer.getDocLink(context, te, ee, name(ee),
    ...
    @Override
    protected void addInheritedSummaryLink(TypeElement te, Element member,
            Content target) {
     ...
     */
    protected void addParam(VariableElement param, TypeMirror paramType,
            boolean isVarArg,
            Content target) {
        HtmlLinkInfo linkInfo
            = new HtmlLinkInfo(configuration, LINK_TYPE_PARAMS, paramType)
                .varargs(isVarArg)
                .showTypeParameterAnnotations(true);
     ...
     */
    protected void addReceiver(ExecutableElement member, TypeMirror rcvrType,
            Content target) {
        var info = new HtmlLinkInfo(configuration, SHOW_TYPE_PARAMS_AND_BOUNDS,
            rcvrType)
                .linkToSelf(false);
    ...
    protected boolean isAnnotatedReceiver(TypeMirror receiverType) {
        return new SimpleTypeVisitor14<Boolean, Void>() {
            ...
            public Boolean visitDeclared(DeclaredType t, Void unused) {
                if (super.visitDeclared(t, unused)
     ...
     */
    protected Content getParameters(ExecutableElement member,
        ...
        if (includeAnnotations && rcvrType != null
            && isAnnotatedReceiver(rcvrType)) {
            addReceiver(member, rcvrType, result);
            sep = "," + Text.NL + " ";
        }
        int paramstart;
        ExecutableType instMeth
            = utils.asInstantiatedMethodType(typeElement, member);
            ...
            if (param.getKind() != ElementKind.INSTANCE_INIT) {
                if (includeAnnotations) {
                    Content annotationInfo
                        = writer.getAnnotationInfo(param, false);
                ...
                addParam(param, paramType,
                    (paramstart == parameters.size() - 1) && member.isVarArgs(),
                    result);
                break;
            }
        }
        ...
        for (int i = paramstart + 1; i < parameters.size(); i++) {
            ...
            if (includeAnnotations) {
                Content annotationInfo
                    = writer.getAnnotationInfo(parameters.get(i), false);
                if (!annotationInfo.isEmpty()) {
                    result.add(annotationInfo)
                        .add(Text.NL)
                        .add(" ");
    ...
    protected Content getExceptions(ExecutableElement member) {
        List<? extends TypeMirror> exceptions = utils
            .asInstantiatedMethodType(typeElement, member).getThrownTypes();
        Content result = new ContentBuilder();
        for (TypeMirror t : exceptions) {
            ...
            Content link
                = writer.getLink(new HtmlLinkInfo(configuration, PLAIN, t));
            result.add(link);
        }
        return result;
    }
