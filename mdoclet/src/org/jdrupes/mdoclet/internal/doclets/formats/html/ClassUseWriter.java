...
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.ClassUseMapper;
...
public class ClassUseWriter extends SubWriterHolderWriter {
    ...
    final Map<PackageElement,
            List<Element>> pkgToConstructorParameterAnnotations;
    ...
    public ClassUseWriter(HtmlConfiguration configuration,
        ...
        if (mapper.classToPackageAnnotations.containsKey(typeElement)) {
            ...
            pkgToPackageAnnotations
                .addAll(mapper.classToPackageAnnotations.get(typeElement));
        }
        configuration.currentTypeElement = typeElement;
        this.pkgSet = new TreeSet<>(comparators.makePackageComparator());
        this.pkgToClassTypeParameter = pkgDivide(mapper.classToClassTypeParam);
        this.pkgToClassAnnotations = pkgDivide(mapper.classToClassAnnotations);
        this.pkgToMethodTypeParameter
            = pkgDivide(mapper.classToMethodTypeParam);
        this.pkgToMethodArgTypeParameter
            = pkgDivide(mapper.classToMethodArgTypeParam);
        ...
        this.pkgToMethodReturnTypeParameter
            = pkgDivide(mapper.classToMethodReturnTypeParam);
        this.pkgToMethodAnnotations
            = pkgDivide(mapper.classToMethodAnnotations);
        this.pkgToMethodParameterAnnotations
            = pkgDivide(mapper.classToMethodParamAnnotation);
        this.pkgToSubclass = pkgDivide(mapper.classToSubclass);
        this.pkgToSubinterface = pkgDivide(mapper.classToSubinterface);
        this.pkgToImplementingClass
            = pkgDivide(mapper.classToImplementingClass);
        ...
        this.pkgToConstructorAnnotations
            = pkgDivide(mapper.classToConstructorAnnotations);
        this.pkgToConstructorParameterAnnotations
            = pkgDivide(mapper.classToConstructorParamAnnotation);
        this.pkgToConstructorArgs = pkgDivide(mapper.classToConstructorArgs);
        this.pkgToConstructorArgTypeParameter
            = pkgDivide(mapper.classToConstructorArgTypeParam);
        this.pkgToConstructorThrows
            = pkgDivide(mapper.classToConstructorThrows);
        // tmp test
        if (pkgSet.size() > 0 &&
            ...
            configuration.reporter.print(Diagnostic.Kind.WARNING,
                "Internal error: package sets don't match: "
                    + pkgSet + " with: "
                    + mapper.classToPackage.get(this.typeElement));
        }
    ...
    public static void generate(HtmlConfiguration configuration,
        ...
        for (TypeElement aClass : configuration.getIncludedTypeElements()) {
            // If -nodeprecated option is set and the containing package is
            // marked
            // as deprecated, do not generate the class-use page. We will still
            // generate
            // the class-use page if the class is marked as deprecated but the
            // containing
            // package is not since it could still be linked from that
            // package-use page.
            if (!(nodeprecated &&
    ...
    private Map<PackageElement, List<Element>> pkgDivide(
            Map<TypeElement, ? extends List<? extends Element>> classMap) {
    ...
    protected void addPackageAnnotationList(Content content) {
        ...
        for (PackageElement pkg : pkgToPackageAnnotations) {
            ...
            table.addRow(getPackageLink(pkg, getLocalizedPackageName(pkg)),
                summary);
        }
        content.add(table);
    }
    ...
    protected void addClassList(Content content) {
        ...
        for (PackageElement pkg : pkgSet) {
            ...
            var heading
                = HtmlTree.HEADING(Headings.TypeUse.SUMMARY_HEADING, link);
            section.add(heading);
    ...
    protected void addPackageUse(PackageElement pkg, Table<?> table) {
        Content pkgLink = links.createLink(htmlIds.forPackage(pkg),
    ...
    protected void addClassUse(PackageElement pkg, Content content) {
        ...
        methodSubWriter.addUseInfo(pkgToMethodParameterAnnotations.get(pkg),
            contents.getContent("doclet.ClassUse_MethodParameterAnnotations",
                classLink,
                pkgLink),
            content);
        methodSubWriter.addUseInfo(pkgToMethodTypeParameter.get(pkg),
            contents.getContent("doclet.ClassUse_MethodTypeParameter",
                classLink,
                pkgLink),
            content);
        ...
        methodSubWriter.addUseInfo(pkgToMethodReturnTypeParameter.get(pkg),
            contents.getContent("doclet.ClassUse_MethodReturnTypeParameter",
                classLink,
                pkgLink),
            content);
        ...
        methodSubWriter.addUseInfo(pkgToMethodArgTypeParameter.get(pkg),
            contents.getContent("doclet.ClassUse_MethodArgsTypeParameters",
                classLink,
                pkgLink),
            content);
        ...
        constrSubWriter.addUseInfo(pkgToConstructorAnnotations.get(pkg),
            contents.getContent("doclet.ClassUse_ConstructorAnnotations",
                classLink,
                pkgLink),
            content);
        constrSubWriter.addUseInfo(
            pkgToConstructorParameterAnnotations.get(pkg),
            contents.getContent(
                "doclet.ClassUse_ConstructorParameterAnnotations", classLink,
                pkgLink),
            content);
        ...
        constrSubWriter.addUseInfo(pkgToConstructorArgTypeParameter.get(pkg),
            contents.getContent("doclet.ClassUse_ConstructorArgsTypeParameters",
                classLink,
                pkgLink),
            content);
    ...
    protected HtmlTree getClassUseHeader() {
        ...
        headingContent
            .add(contents.getContent("doclet.ClassUse_Title", cltype));
        ...
        bodyContents.setHeader(getHeader(PageMode.USE, typeElement))
            .addMainContent(div);
        return body;
    }
    ...
    protected Navigation getNavBar(PageMode pageMode, Element element) {
        Content mdleLinkContent
            = getModuleLink(utils.elementUtils.getModuleOf(typeElement),
