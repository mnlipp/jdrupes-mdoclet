...
import org.jdrupes.mdoclet.internal.doclets.toolkit.PackageSummaryWriter;
...
public class PackageWriterImpl extends HtmlDocletWriter
     ...
     */
    private final HtmlTree section
        = HtmlTree.SECTION(HtmlStyle.packageDescription, new ContentBuilder());

    private final BodyContents bodyContents = new BodyContents();

    // Maximum number of subpackages and sibling packages to list in related
    // packages table
    ...
    public Content getPackageHeader() {
        ...
        if (configuration.showModules) {
            ModuleElement mdle = configuration.docEnv.getElementUtils()
                .getModuleOf(packageElement);
            var classModuleLabel = HtmlTree.SPAN(HtmlStyle.moduleLabelInPackage,
                contents.moduleLabel);
            var moduleNameDiv
                = HtmlTree.DIV(HtmlStyle.subTitle, classModuleLabel);
    ...
    private SortedSet<TypeElement> filterClasses(SortedSet<TypeElement> types) {
        List<TypeElement> typeList = types
            .stream()
            .filter(
                te -> utils.isCoreClass(te) && configuration.isGeneratedDoc(te))
    ...
    private List<PackageElement> findRelatedPackages() {
        ...
        // only add sibling packages if there is a non-empty superpackage, we
        // are beneath threshold,
        // and number of siblings is beneath threshold as well
        if (hasSuperPackage && pkgPrefix != null
            && packages.size() <= MAX_SIBLING_PACKAGES) {
            Pattern siblingPattern
                = Pattern.compile(pkgPrefix.replace(".", "\\.") + "\\.\\w+");

            List<PackageElement> siblings = filterPackages(
                p -> siblingPattern.matcher(p.getQualifiedName().toString())
                    .matches());
    ...
    protected Navigation getNavBar(PageMode pageMode, Element element) {
        Content linkContent
            = getModuleLink(utils.elementUtils.getModuleOf(packageElement),
                contents.moduleLabel);
        return super.getNavBar(pageMode, element)
            .setNavLinkModule(linkContent)
            .setSubNavLinks(() -> List.of(
                links.createLink(HtmlIds.PACKAGE_DESCRIPTION,
                    contents.navDescription,
                    !utils.getFullBody(packageElement).isEmpty()
                        && !options.noComment()),
                links.createLink(HtmlIds.RELATED_PACKAGE_SUMMARY,
                    contents.relatedPackages,
                    relatedPackages != null && !relatedPackages.isEmpty()),
                links.createLink(HtmlIds.CLASS_SUMMARY,
    ...
    public void addDeprecationInfo(Content div) {
        List<? extends DeprecatedTree> deprs
            = utils.getDeprecatedTrees(packageElement);
        if (utils.isDeprecated(packageElement)) {
            ...
            var deprPhrase = HtmlTree.SPAN(HtmlStyle.deprecatedLabel,
                getDeprecatedPhrase(packageElement));
            deprDiv.add(deprPhrase);
            if (!deprs.isEmpty()) {
                List<? extends DocTree> commentTags
                    = ch.getDescription(deprs.get(0));
                if (!commentTags.isEmpty()) {
                    addInlineDeprecatedComment(packageElement, deprs.get(0),
                        deprDiv);
                }
            }
            div.add(deprDiv);
    ...
    public void addRelatedPackagesSummary(Content summaryContent) {
        boolean showModules = configuration.showModules
            && hasRelatedPackagesInOtherModules(relatedPackages);
        TableHeader tableHeader = showModules
            ? new TableHeader(contents.moduleLabel, contents.packageLabel,
                contents.descriptionLabel)
            : new TableHeader(contents.packageLabel, contents.descriptionLabel);
        addPackageSummary(relatedPackages, contents.relatedPackages,
            tableHeader,
    ...
    public void addAllClassesAndInterfacesSummary(Content target) {
        var table = new Table<TypeElement>(HtmlStyle.summaryTable)
            .setHeader(
                new TableHeader(contents.classLabel, contents.descriptionLabel))
        ...
        for (TypeElement typeElement : allClasses) {
            if (typeElement != null && utils.isCoreClass(typeElement)) {
                ...
                if (utils.isDeprecated(typeElement)) {
                    ...
                    List<? extends DeprecatedTree> tags
                        = utils.getDeprecatedTrees(typeElement);
                    if (!tags.isEmpty()) {
                        addSummaryDeprecatedComment(typeElement, tags.get(0),
                            description);
                    }
                } else {
                    addSummaryComment(typeElement, description);
                }
                table.addRow(typeElement,
                    Arrays.asList(classLink, description));
            }
        }
        if (!table.isEmpty()) {
    ...
    public void addPackageSummary(List<PackageElement> packages, Content label,
        ...
        if (!packages.isEmpty()) {
            ...
            if (showModules) {
                table.setColumnStyles(HtmlStyle.colPlain, HtmlStyle.colFirst,
                    HtmlStyle.colLast);
            } else {
            ...
            for (PackageElement pkg : packages) {
                Content packageLink
                    = getPackageLink(pkg, Text.of(pkg.getQualifiedName()));
                Content moduleLink = Text.EMPTY;
                if (showModules) {
                    ModuleElement module
                        = (ModuleElement) pkg.getEnclosingElement();
                    if (module != null && !module.isUnnamed()) {
                        moduleLink = getModuleLink(module,
                            Text.of(module.getQualifiedName()));
                    }
                }
                ContentBuilder description = new ContentBuilder();
                ...
                if (utils.isDeprecated(pkg)) {
                    ...
                    if (!tags.isEmpty()) {
                        addSummaryDeprecatedComment(pkg, tags.get(0),
                            description);
                    }
                } else {
    ...
    public void addPackageSignature(Content packageContent) {
        ...
        packageContent
            .add(Signatures.getPackageSignature(packageElement, this));
    }
    ...
    private boolean hasRelatedPackagesInOtherModules(
            List<PackageElement> relatedPackages) {
        final ModuleElement module
            = (ModuleElement) packageElement.getEnclosingElement();
        return relatedPackages.stream()
            .anyMatch(pkg -> module != pkg.getEnclosingElement());
    }

    private List<PackageElement>
            filterPackages(Predicate<? super PackageElement> filter) {
