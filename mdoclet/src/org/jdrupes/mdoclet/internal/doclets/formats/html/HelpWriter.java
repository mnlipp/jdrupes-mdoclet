...
public class HelpWriter extends HtmlDocletWriter {
    ...
    public HelpWriter(HtmlConfiguration configuration,
        ...
        overviewLink = links.createLink(DocPaths.INDEX,
            resources.getText("doclet.Overview"));
        allPackagesLink = links.createLink(DocPaths.ALLPACKAGES_INDEX,
            resources.getText("doclet.All_Packages"));
        allClassesLink = links.createLink(DocPaths.ALLCLASSES_INDEX,
    ...
    protected void addHelpFileContents(Content content) {
        ...
        content
            .add(HtmlTree.HEADING(Headings.PAGE_TITLE_HEADING, HtmlStyle.title,
    ...
    protected Navigation getNavBar(PageMode pageMode, Element element) {
        return super.getNavBar(pageMode, element)
            .setSubNavLinks(() -> List.of(
                links.createLink(HtmlIds.HELP_NAVIGATION,
    ...
    private Content getNavigationSection(HtmlTree mainTOC) {
        ...
        var navSection = HtmlTree.DIV(HtmlStyle.subTitle)
            .add(HtmlTree.HEADING(Headings.CONTENT_HEADING, navHeading)
                .setId(HtmlIds.HELP_NAVIGATION))
            .add(contents.getContent("doclet.help.navigation.intro",
                overviewLink));
        if (options.createIndex()) {
            ...
            navSection.add(" ")
                .add(contents.getContent("doclet.help.navigation.index",
                    indexLink, links));
        }
        ...
        if (options.createIndex()) {
            section = newHelpSection(getContent("doclet.help.search.head"),
                PageMode.SEARCH, subTOC);
            var searchIntro
                = HtmlTree.P(getContent("doclet.help.search.intro"));
            ...
            var searchSpecLink = HtmlTree.A(
                resources.getText("doclet.help.search.spec.url",
                    configuration.getDocletVersion().feature()),
                getContent("doclet.help.search.spec.title"));
            var searchRefer = HtmlTree
                .P(getContent("doclet.help.search.refer", searchSpecLink));
        ...
        mainTOC.add(HtmlTree.LI(new ContentBuilder(
            links.createLink(DocLink.fragment(HtmlIds.HELP_NAVIGATION.name()),
                navHeading),
            Text.of(": "), subTOC)));
    ...
    private Content getPageKindSection(HtmlTree mainTOC) {
        Content pageKindsHeading
            = contents.getContent("doclet.help.page_kinds.head");
        var pageKindsSection = HtmlTree.DIV(HtmlStyle.subTitle)
            .add(HtmlTree.HEADING(Headings.CONTENT_HEADING, pageKindsHeading)
                .setId(HtmlIds.HELP_PAGES))
        ...
        if (configuration.showModules) {
            section
                = newHelpSection(contents.moduleLabel, PageMode.MODULE, subTOC);
        ...
        // Package
        section
            = newHelpSection(contents.packageLabel, PageMode.PACKAGE, subTOC)
        ...
        Content notes = new ContentBuilder(
            HtmlTree.SPAN(HtmlStyle.helpNote,
                getContent("doclet.help.class_interface.note")),
            Text.of(" "),
        ...
        section = newHelpSection(getContent("doclet.help.class_interface.head"),
            PageMode.CLASS, subTOC)
                .add(
                    HtmlTree.P(getContent("doclet.help.class_interface.intro")))
                .add(newHelpSectionList(
                    getContent(
                ...
                .add(HtmlTree
                    .P(getContent("doclet.help.class_interface.member_order")));
        pageKindsSection.add(section);

        section = newHelpSection(getContent("doclet.help.other_files.head"),
            PageMode.DOC_FILE, subTOC)
        ...
        if (configuration.conditionalPages
            ...
            section
                = newHelpSection(contents.previewAPI, PageMode.PREVIEW, subTOC);
        ...
        if (configuration.conditionalPages
            .contains(HtmlConfiguration.ConditionalPage.CONSTANT_VALUES)) {
            section = newHelpSection(contents.constantsSummaryTitle,
                PageMode.CONSTANT_VALUES, subTOC);
        ...
        // Serialized Form
        if (configuration.conditionalPages
            .contains(HtmlConfiguration.ConditionalPage.SERIALIZED_FORM)) {
            section = newHelpSection(contents.serializedForm,
                PageMode.SERIALIZED_FORM, subTOC)
                    .add(
                        HtmlTree.P(getContent("doclet.help.serial_form.body")));
            pageKindsSection.add(section);
        }

        // System Properties
        if (configuration.conditionalPages
            .contains(HtmlConfiguration.ConditionalPage.SYSTEM_PROPERTIES)) {
            section = newHelpSection(contents.systemPropertiesLabel,
                PageMode.SYSTEM_PROPERTIES, subTOC);
            Content sysPropsBody
                = getContent("doclet.help.systemProperties.body",
                    links.createLink(DocPaths.SYSTEM_PROPERTIES,
                        resources.getText("doclet.systemProperties")));
        ...
        if (configuration.conditionalPages
            .contains(HtmlConfiguration.ConditionalPage.EXTERNAL_SPECS)) {
            section = newHelpSection(contents.externalSpecifications,
                PageMode.EXTERNAL_SPECS, subTOC);
            Content extSpecsBody
                = getContent("doclet.help.externalSpecifications.body",
                    links.createLink(DocPaths.EXTERNAL_SPECS,
                        resources.getText("doclet.External_Specifications")));
        ...
        if (options.createIndex()) {
            if (!configuration.packages.isEmpty()) {
                section = newHelpSection(
                    getContent("doclet.help.all_packages.head"),
                    PageMode.ALL_PACKAGES, subTOC)
                        .add(HtmlTree.P(getContent(
                            "doclet.help.all_packages.body", allPackagesLink)));
                pageKindsSection.add(section);
            }

            section = newHelpSection(getContent("doclet.help.all_classes.head"),
                PageMode.ALL_CLASSES, subTOC)
                    .add(HtmlTree.P(getContent("doclet.help.all_classes.body",
                        allClassesLink)));
            pageKindsSection.add(section);
            ...
            section = newHelpSection(getContent("doclet.help.index.head"),
                PageMode.INDEX, subTOC)
                    .add(HtmlTree.P(getContent("doclet.help.index.body",
                        indexLink, links)));
            pageKindsSection.add(section);
        }

        mainTOC.add(HtmlTree.LI(new ContentBuilder(
            links.createLink(DocLink.fragment(HtmlIds.HELP_PAGES.name()),
                pageKindsHeading),
            Text.of(": "), subTOC)));
    ...
    private HtmlTree newHelpSection(Content headingContent, HtmlTree toc,
            HtmlId id) {
        Content link
            = links.createLink(DocLink.fragment(id.name()), headingContent);
    ...
    private HtmlTree newHelpSection(Content headingContent,
            Navigation.PageMode pm, HtmlTree toc) {
