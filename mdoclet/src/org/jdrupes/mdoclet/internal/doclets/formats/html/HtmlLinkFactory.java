...
public class HtmlLinkFactory {
    ...
    public Content getLink(HtmlLinkInfo linkInfo) {
        if (linkInfo.getType() != null) {
            SimpleTypeVisitor14<Content, HtmlLinkInfo> linkVisitor
                = new SimpleTypeVisitor14<>() {
                    ...
                    @Override
                    protected Content defaultAction(TypeMirror type,
                    ...
                    @Override
                    public Content visitArray(ArrayType type,
                            HtmlLinkInfo linkInfo) {
                        // keep track of the dimension depth and replace the
                        // last dimension
                        // specifier with varargs, when the stack is fully
                        // unwound.
                        currentDepth++;
                    ...
                    @Override
                    public Content visitWildcard(WildcardType type,
                            HtmlLinkInfo linkInfo) {
                        link.add(getTypeAnnotationLinks(linkInfo));
                        link.add("?");
                        ...
                        if (extendsBound != null) {
                            link.add(" extends ");
                            link.add(getLink(
                                getBoundsLinkInfo(linkInfo, extendsBound)));
                        }
                        TypeMirror superBound = type.getSuperBound();
                        if (superBound != null) {
                            link.add(" super ");
                            link.add(getLink(
                                getBoundsLinkInfo(linkInfo, superBound)));
                        }
                        return link;
                    }
                    ...
                    public Content visitTypeVariable(TypeVariable type,
                        ...
                        if (linkInfo.linkTypeParameters()
                            && utils.isTypeElement(owner)) {
                        ...
                        if (linkInfo.showTypeBounds()) {
                            ...
                            TypeParameterElement tpe
                                = ((TypeParameterElement) typevariable
                                    .asElement());
                            boolean more = false;
                            List<? extends TypeMirror> bounds
                                = utils.getBounds(tpe);
                            for (TypeMirror bound : bounds) {
                                // we get everything as extends java.lang.Object
                                // we suppress
                                // all of them except those that have multiple
                                // extends
                                if (bounds.size() == 1 &&
                                    utils.typeUtils.isSameType(bound,
                                        utils.getObjectType())
                                    &&
                                ...
                                link.add(getLink(
                                    getBoundsLinkInfo(linkInfo, bound)));
                                more = true;
                            }
                        }
                        return link;
                    ...
                    @Override
                    public Content visitDeclared(DeclaredType type,
                            HtmlLinkInfo linkInfo) {
                        TypeMirror enc = type.getEnclosingType();
                        if (enc instanceof DeclaredType dt
                            && utils.isGenericType(dt)) {
                            // If an enclosing type has type parameters render
                            // them as separate links as
                            // otherwise this information is lost. On the other
                            // hand, plain enclosing types
                            // are not linked separately as they are easy to
                            // reach from the nested type.
    ...
    protected Content getClassLink(HtmlLinkInfo linkInfo) {
        ...
        if (!hasFragment) {
            boolean isTypeLink = linkInfo.getType() != null &&
                utils
                    .isTypeVariable(utils.getComponentType(linkInfo.getType()));
            title = getClassToolTip(typeElement, isTypeLink);
        }
        Content label = linkInfo.getClassLinkLabel(configuration);
        if (linkInfo
            .getContext() == HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS_IN_LABEL) {
            // For this kind of link, type parameters are included in the link
            // label
        ...
        if (!hasFragment && showPreview) {
            ...
            TypeElement enclosing
                = utils.getEnclosingTypeElement(linkInfo.getTargetMember());
            Set<ElementFlag> enclosingFlags = utils.elementFlags(enclosing);
            if (flags.contains(ElementFlag.PREVIEW)
                && enclosingFlags.contains(ElementFlag.PREVIEW)) {
        ...
        if (utils.isIncluded(typeElement)) {
            if (configuration.isGeneratedDoc(typeElement)
                ...
                if (linkInfo.linkToSelf()
                    || typeElement != m_writer.getCurrentPageElement()) {
                    ...
                    if (flags.contains(ElementFlag.PREVIEW)) {
                        link.add(HtmlTree.SUP(m_writer.links.createLink(
                            filename.fragment(m_writer.htmlIds
                                .forPreviewSection(previewTarget).name()),
    ...
    protected Content getTypeParameterLinks(HtmlLinkInfo linkInfo) {
        ...
        if (linkInfo.getExecutableElement() != null) {
            linkInfo.getExecutableElement().getTypeParameters()
                .forEach(t -> vars.add(t.asType()));
        } else if (linkInfo.getType() != null
            && utils.isDeclaredType(linkInfo.getType())) {
            ...
            linkInfo.getTypeElement().getTypeParameters()
                .forEach(t -> vars.add(t.asType()));
        } else {
            // Nothing to document.
            return links;
    ...
    public Content getTypeAnnotationLinks(HtmlLinkInfo linkInfo) {
        ...
        } else if (utils.isTypeVariable(linkInfo.getType())
            && linkInfo.showTypeParameterAnnotations()) {
     ...
     */
    private HtmlLinkInfo getBoundsLinkInfo(HtmlLinkInfo linkInfo,
            TypeMirror bound) {
     ...
     */
    private String getClassToolTip(TypeElement typeElement,
            boolean isTypeLink) {
        ...
        if (isTypeLink) {
            ...
            return resources.getText("doclet.Href_Interface_Title",
                m_writer.getLocalizedPackageName(
                    utils.containingPackage(typeElement)));
    ...
    private DocPath getPath(HtmlLinkInfo linkInfo) {
        return m_writer.pathToRoot
            .resolve(docPaths.forClass(linkInfo.getTypeElement()));
    }
}
