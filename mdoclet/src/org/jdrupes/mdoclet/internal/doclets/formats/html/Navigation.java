...
import static org.jdrupes.mdoclet.internal.doclets.toolkit.util.VisibleMemberTable.Kind.*;
...
public class Navigation {
     ...
     */
    public Navigation(Element element, HtmlConfiguration configuration,
        ...
        this.rowListTitle
            = configuration.getDocResources().getText("doclet.Navigation");
        this.searchLabel = contents.getContent("doclet.search");
        this.searchPlaceholder = configuration.getDocResources()
            .getText("doclet.search_placeholder");
    }
    ...
    private void addMainNavLinks(Content target) {
        ...
        case OVERVIEW:
            addActivePageLink(target, contents.overviewLabel,
                options.createOverview());
        ...
        case MODULE:
            ...
            addActivePageLink(target, contents.moduleLabel,
                configuration.showModules);
            addPackageLink(target);
            addPageLabel(target, contents.classLabel, true);
            addPageLabel(target, contents.useLabel, options.classUse());
            addTreeLink(target);
        ...
        case CLASS:
            ...
            if (options.classUse()) {
                addItemToList(target,
                    links.createLink(
                        DocPaths.CLASS_USE.resolve(path.basename()),
                        contents.useLabel));
            }
            if (options.createTree()) {
                addItemToList(target, links.createLink(DocPaths.PACKAGE_TREE,
        ...
        case USE:
            ...
            if (element instanceof PackageElement) {
                ...
                addItemToList(target,
                    configuration.utils
                        .isEnclosingPackageIncluded((TypeElement) element)
                            ? links.createLink(
                                DocPath.parent.resolve(DocPaths.PACKAGE_TREE),
                                contents.treeLabel)
                            : links.createLink(
        ...
        case TREE:
            addOverviewLink(target);
            if (element == null) {
                addPageLabel(target, contents.moduleLabel,
                    configuration.showModules);
        ...
        case NEW:
            ...
            if (documentedPage == PageMode.DEPRECATED) {
                addActivePageLink(target, contents.deprecatedLabel,
                    configuration.conditionalPages.contains(
                        HtmlConfiguration.ConditionalPage.DEPRECATED));
            } else {
                addDeprecatedLink(target);
            }
            if (documentedPage == PageMode.INDEX) {
                addActivePageLink(target, contents.indexLabel,
                    options.createIndex());
            } else {
                addIndexLink(target);
            }
            if (documentedPage == PageMode.HELP) {
                addActivePageLink(target, contents.helpLabel,
                    !options.noHelp());
            } else {
                addHelpLink(target);
            }
    ...
    private void addDetailLinks(Content target, boolean nested) {
        if (documentedPage == PageMode.CLASS) {
            ...
            VisibleMemberTable vmt
                = configuration.getVisibleMemberTable((TypeElement) element);
            Set<VisibleMemberTable.Kind> detailSet
                = VisibleMemberTable.Kind.forDetailsOf(element.getKind());
            for (VisibleMemberTable.Kind kind : detailSet) {
                addTypeDetailLink(kind, !vmt.getVisibleMembers(kind).isEmpty(),
                    listContents);
            }
            if (!listContents.isEmpty()) {
                if (nested) {
     ...
     */
    protected void addTypeDetailLink(VisibleMemberTable.Kind kind, boolean link,
            List<Content> listContents) {
        addContentToList(listContents, switch (kind) {
        case CONSTRUCTORS -> links.createLink(HtmlIds.CONSTRUCTOR_DETAIL,
            contents.navConstructor, link);
        case ENUM_CONSTANTS -> links.createLink(HtmlIds.ENUM_CONSTANT_DETAIL,
            contents.navEnum, link);
        case FIELDS -> links.createLink(HtmlIds.FIELD_DETAIL, contents.navField,
            link);
        case METHODS -> links.createLink(HtmlIds.METHOD_DETAIL,
            contents.navMethod, link);
        case PROPERTIES -> links.createLink(HtmlIds.PROPERTY_DETAIL,
            contents.navProperty, link);
        case ANNOTATION_TYPE_MEMBER -> links.createLink(
            HtmlIds.ANNOTATION_TYPE_ELEMENT_DETAIL,
    ...
    private void addListToNav(List<? extends Content> listContents,
            Content target) {
        int count = 0;
    ...
    private void addActivePageLink(Content target, Content label,
            boolean display) {
        if (display) {
    ...
    private void addOverviewLink(Content target) {
        if (options.createOverview()) {
            target.add(
                HtmlTree.LI(links.createLink(pathToRoot.resolve(DocPaths.INDEX),
        ...
        if (configuration.showModules) {
            if (configuration.modules.size() == 1) {
                ...
                target.add(HtmlTree.LI((included)
                    ? links.createLink(
                        pathToRoot.resolve(
                            configuration.docPaths.moduleSummary(mdle)),
    ...
    private void addPackageLink(Content target) {
        if (configuration.packages.size() == 1) {
            ...
            boolean included = packageElement != null
                && configuration.utils.isIncluded(packageElement);
            if (!included) {
            ...
            if (included || packageElement == null) {
                ...
                if (crossPkgLink != null) {
                    target.add(HtmlTree.LI(
                        links.createLink(crossPkgLink, contents.packageLabel)));
                } else {
    ...
    private void addPackageOfElementLink(Content target) {
        target.add(HtmlTree.LI(
            links.createLink(DocPath.parent.resolve(DocPaths.PACKAGE_SUMMARY),
                contents.packageLabel)));
    }

    private void addPackageSummaryLink(Content target) {
        target.add(HtmlTree.LI(
            links.createLink(DocPaths.PACKAGE_SUMMARY, contents.packageLabel)));
    }

    private void addTreeLink(Content target) {
        if (options.createTree()) {
            List<PackageElement> packages
                = new ArrayList<>(configuration.getSpecifiedPackageElements());
            DocPath docPath = packages.size() == 1
                && configuration.getSpecifiedTypeElements().isEmpty()
                    ? pathToRoot.resolve(
                        configuration.docPaths.forPackage(packages.get(0))
                            .resolve(DocPaths.PACKAGE_TREE))
                    : pathToRoot.resolve(DocPaths.OVERVIEW_TREE);
            target.add(
                HtmlTree.LI(links.createLink(docPath, contents.treeLabel, "")));
        }
    }
    ...
    private void addDeprecatedLink(Content target) {
        if (configuration.conditionalPages
            ...
            target.add(HtmlTree.LI(
                links.createLink(pathToRoot.resolve(DocPaths.DEPRECATED_LIST),
    ...
    private void addPreviewLink(Content target) {
        if (configuration.conditionalPages
            .contains(HtmlConfiguration.ConditionalPage.PREVIEW)) {
            target.add(HtmlTree
                .LI(links.createLink(pathToRoot.resolve(DocPaths.PREVIEW_LIST),
    ...
    private void addNewLink(Content target) {
        if (configuration.conditionalPages
            .contains(HtmlConfiguration.ConditionalPage.NEW)) {
            target.add(HtmlTree
                .LI(links.createLink(pathToRoot.resolve(DocPaths.NEW_LIST),
                    contents.newLabel, "")));
        }
    }
    ...
    private void addHelpLink(Content target) {
        if (!options.noHelp()) {
            ...
            if (helpfile.isEmpty()) {
                ...
                DocFile file
                    = DocFile.createFileForInput(configuration, helpfile);
                helpfilenm = DocPath.create(file.getName());
            }
            target.add(HtmlTree.LI(links.createLink(
                new DocLink(pathToRoot.resolve(helpfilenm),
                    htmlIds.forPage(documentedPage).name()),
                contents.helpLabel, "")));
        }
    }
    ...
    public Content getContent() {
        ...
        Content skipNavLinks
            = contents.getContent("doclet.Skip_navigation_links");
        String toggleNavLinks = configuration.getDocResources()
            .getText("doclet.Toggle_navigation_links");
        navigationBar.add(MarkerComments.START_OF_TOP_NAVBAR);
        // The mobile menu button uses three empty spans to produce its animated
        // icon
        HtmlTree iconSpan = HtmlTree.SPAN(HtmlStyle.navBarToggleIcon)
        ...
        boolean addSearch
            = options.createIndex() && documentedPage != PageMode.SEARCH;
