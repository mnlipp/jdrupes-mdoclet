...
import org.jdrupes.mdoclet.internal.doclets.toolkit.MemberWriter;
import org.jdrupes.mdoclet.internal.doclets.toolkit.Resources;
import org.jdrupes.mdoclet.internal.doclets.toolkit.taglets.DeprecatedTaglet;
...
public abstract class AbstractMemberWriter
     ...
     */
    public abstract void addInheritedSummaryLabel(TypeElement typeElement,
            Content content);
    ...
    protected void addModifiersAndType(Element member, TypeMirror type,
        ...
        if (type == null) {
            ...
            List<? extends TypeParameterElement> list
                = utils.isExecutableElement(member)
    ...
    private void addModifiers(Element member, Content target) {
        ...
        if (!utils.isAnnotationInterface(member.getEnclosingElement())
            && utils.isMethod(member)) {
            if (!utils.isPlainInterface(member.getEnclosingElement())
                && utils.isAbstract(member)) {
                target.add("abstract ");
            }
     ...
     */
    protected void addUseInfo(List<? extends Element> members, Content heading,
            Content content) {
        ...
        for (Element element : members) {
            ...
            addSummaryLink(
                utils.isClass(element) || utils.isPlainInterface(element)
