...
public class IndexWriter extends HtmlDocletWriter {
    ...
    public static void generate(HtmlConfiguration configuration)
        ...
        if (configuration.getOptions().splitIndex()) {
            ...
            while (iter.hasNext()) {
                ...
                DocPath file = DocPaths.INDEX_FILES
                    .resolve(DocPaths.indexN(iter.nextIndex()));
                IndexWriter writer = new IndexWriter(configuration, file);
                writer.generateIndexFile(firstCharacters, List.of(ch));
            }
        } else {
            IndexWriter writer
                = new IndexWriter(configuration, DocPaths.INDEX_ALL);
    ...
    protected void generateIndexFile(List<Character> allFirstCharacters,
        ...
        String title = splitIndex
            ? resources.getText("doclet.Window_Split_Index",
                displayFirstCharacters.get(0))
        ...
        String description
            = splitIndex ? "index: " + displayFirstCharacters.get(0) : "index";
     ...
     */
    protected void addContents(char ch, SortedSet<IndexItem> items,
            Content content) {
    ...
    protected void addHeading(char ch, Content content) {
        ...
        var heading = HtmlTree
            .HEADING(Headings.CONTENT_HEADING, HtmlStyle.title, headContent)
    ...
    protected void addElementDescription(IndexItem item, Content target) {
        ...
        case MODULE:
            dt = HtmlTree
                .DT(getModuleLink((ModuleElement) element, Text.of(label)));
            dt.add(" - ").add(contents.module_).add(" " + label);
            break;

        case PACKAGE:
            dt = HtmlTree
                .DT(getPackageLink((PackageElement) element, Text.of(label)));
            if (configuration.showModules) {
                item.setContainingModule(utils
                    .getFullyQualifiedName(utils.containingModule(element)));
            }
        ...
        case INTERFACE:
            dt = HtmlTree.DT(getLink(new HtmlLinkInfo(configuration,
                HtmlLinkInfo.Kind.SHOW_TYPE_PARAMS_IN_LABEL,
                (TypeElement) element).style(HtmlStyle.typeNameLink)));
            dt.add(" - ");
        ...
        case ENUM_CONSTANT:
            ...
            dt = HtmlTree
                .DT(getDocLink(HtmlLinkInfo.Kind.PLAIN, containingType, element,
    ...
    protected void addTagDescription(IndexItem item, Content target) {
        String itemPath
            = pathToRoot.isEmpty() ? "" : pathToRoot.getPath() + "/";
    ...
    protected void addComment(Element element, Content content) {
        var span = HtmlTree.SPAN(HtmlStyle.deprecatedLabel,
            getDeprecatedPhrase(element));
     ...
     */
    protected void addMemberDesc(Element member, TypeElement enclosing,
            Content content) {
        String kindName = utils.getTypeElementKindName(enclosing, true);
        String resource = switch (member.getKind()) {
        case ENUM_CONSTANT -> "doclet.Enum_constant_in";
        case FIELD -> utils.isStatic(member) ? "doclet.Static_variable_in"
            : "doclet.Variable_in";
        case CONSTRUCTOR -> "doclet.Constructor_for";
        case METHOD -> utils.isAnnotationInterface(enclosing)
            ? "doclet.Element_in"
            : utils.isStatic(member) ? "doclet.Static_method_in"
                : "doclet.Method_in";
        case RECORD_COMPONENT -> "doclet.Record_component_in";
     ...
     */
    protected void addLinksForIndexes(List<Character> allFirstCharacters,
            Content content) {
        ...
        List<Content> pageLinks = Stream.of(IndexItem.Category.values())
            ...
            .sorted(
                (i1, i2) -> utils.compareStrings(i1.getLabel(), i2.getLabel()))
