...
public class Head extends Content {
     ...
     */
    public Head(DocPath path, Runtime.Version docletVersion,
            ZonedDateTime generatedDate) {
    ...
    private Content toContent() {
        ...
        head.add(
            HtmlTree.META("viewport", "width=device-width, initial-scale=1"));
        ...
        if (showTimestamp) {
            ...
            head.add(
                HtmlTree.META("dc.created", generatedDate.format(dateFormat)));
        }
    ...
    private Comment getGeneratedBy(boolean timestamp, ZonedDateTime buildDate) {
        String text = "Generated by javadoc"; // marker string, deliberately not
                                              // localized
    ...
    private void addScripts(HtmlTree head) {
        if (addDefaultScript) {
            head.add(HtmlTree
                .SCRIPT(pathToRoot.resolve(DocPaths.JAVASCRIPT).getPath()));
        }
        if (index) {
    ...
    private void addScriptElement(HtmlTree head, DocPath filePath) {
        DocPath scriptFile
            = pathToRoot.resolve(DocPaths.SCRIPT_DIR).resolve(filePath);
