...
public class SearchWriter extends HtmlDocletWriter {
    ...
    protected void addSearchFileContents(Content contentTree) {
        ...
        // Suppress link to help page if no help page is generated or a custom
        // help page is used.
        HtmlOptions options = configuration.getOptions();
        if (!options.noHelp() && options.helpFile().isEmpty()) {
            Content helpLink = HtmlTree.A("help-doc.html#search",
                contents.getContent("doclet.search.help_page_link"));
            helpSection = HtmlTree.P(
                contents.getContent("doclet.search.help_page_info", helpLink));
        }
        ...
        contentTree
            ...
            .add(HtmlTree
                .DIV(HtmlTree.INPUT("text", HtmlId.of("page-search-input"))
                    .put(HtmlAttr.PLACEHOLDER,
                        resources.getText("doclet.search_placeholder")))
                .add(HtmlTree.INPUT("reset", HtmlId.of("page-search-reset"))
                    .put(HtmlAttr.VALUE,
                        resources.getText("doclet.search_reset"))
                    .put(HtmlAttr.STYLE, "margin: 6px;"))
                .add(HtmlTree.DETAILS(HtmlStyle.pageSearchDetails)
                    .add(HtmlTree
                        .SUMMARY(contents.getContent("doclet.search.show_more"))
                        .setId(HtmlId.of("page-search-expand")))))
            .add(HtmlTree.DIV(HtmlStyle.pageSearchInfo, helpSection)
                .add(HtmlTree
                    .P(contents.getContent("doclet.search.keyboard_info")))
                .add(HtmlTree
                    .P(contents.getContent("doclet.search.browser_info")))
                ...
                .add(new HtmlTree(TagName.BUTTON)
                    .add(new HtmlTree(TagName.IMG)
                        .put(HtmlAttr.SRC,
                            pathToRoot.resolve(DocPaths.CLIPBOARD_SVG)
                                .getPath())
                ...
                .add(HtmlTree
                    .P(HtmlTree.INPUT("checkbox", HtmlId.of("search-redirect")))
            ...
            .add(HtmlTree.DIV(new HtmlTree(TagName.DIV)
                ...
                .add(HtmlTree.SCRIPT(
                    pathToRoot.resolve(DocPaths.SEARCH_PAGE_JS).getPath())));
    }
}
