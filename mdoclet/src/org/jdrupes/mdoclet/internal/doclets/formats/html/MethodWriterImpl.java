...
import org.jdrupes.mdoclet.internal.doclets.toolkit.MethodWriter;
...
public class MethodWriterImpl extends AbstractExecutableMemberWriter
    ...
    @Override
    public void addComments(TypeMirror holderType, ExecutableElement method,
        ...
        if (!utils.getFullBody(method).isEmpty()) {
            if (holder.equals(typeElement) ||
                ...
                if (!utils.hasHiddenTag(holder)
                    && !utils.hasHiddenTag(method)) {
                    Content link = writer.getDocLink(HtmlLinkInfo.Kind.PLAIN,
                        holder, method,
                        utils.isIncluded(holder)
                            ? utils.getSimpleName(holder)
                            : utils.getFullyQualifiedName(holder));
                    var codeLink = HtmlTree.CODE(link);
                    var descriptionFromTypeLabel
                        = HtmlTree.SPAN(HtmlStyle.descriptionFromTypeLabel,
                    ...
                    methodContent.add(HtmlTree.DIV(HtmlStyle.block,
    ...
    protected Table<Element> createSummaryTable() {
        return new Table<Element>(HtmlStyle.summaryTable)
            ...
            .addTab(contents.getContent("doclet.Static_Methods"),
                utils::isStatic)
            .addTab(contents.getContent("doclet.Instance_Methods"),
                e -> !utils.isStatic(e))
            .addTab(contents.getContent("doclet.Abstract_Methods"),
                utils::isAbstract)
            .addTab(contents.getContent("doclet.Concrete_Methods"),
                e -> !utils.isAbstract(e)
                    && !utils.isPlainInterface(e.getEnclosingElement()))
            .addTab(contents.getContent("doclet.Default_Methods"),
                utils::isDefault)
    ...
    @Override
    public void addInheritedSummaryLabel(TypeElement typeElement,
            Content content) {
        ...
        var labelHeading = HtmlTree.HEADING(
            Headings.TypeDeclaration.INHERITED_SUMMARY_HEADING,
            label);
    ...
    protected void addSummaryType(Element member, Content content) {
        ...
        addModifiersAndType(meth, utils.getReturnType(typeElement, meth),
            content);
    }
    ...
    protected static void addOverridden(HtmlDocletWriter writer,
        ...
        Content overriddenTypeLink = writer.getLink(
            new HtmlLinkInfo(writer.configuration, context, overriddenType));
        var codeOverriddenTypeLink = HtmlTree.CODE(overriddenTypeLink);
        Content methlink = writer.getLink(
            new HtmlLinkInfo(writer.configuration, HtmlLinkInfo.Kind.PLAIN,
                holder)
    ...
    protected static void addImplementsInfo(HtmlDocletWriter writer,
        ...
        VisibleMemberTable vmt
            = writer.configuration.getVisibleMemberTable(enclosing);
        ...
        for (ExecutableElement implementedMeth : implementedMethods) {
            TypeMirror intfac
                = vmt.getImplementedMethodHolder(method, implementedMeth);
            intfac = utils.getDeclaredType(enclosing, intfac);
            Content intfaclink = writer.getLink(new HtmlLinkInfo(
                writer.configuration,
                HtmlLinkInfo.Kind.LINK_TYPE_PARAMS_AND_BOUNDS, intfac));
    ...
    protected Content getReturnType(ExecutableElement method) {
        ...
        if (type != null) {
            return writer.getLink(new HtmlLinkInfo(configuration,
