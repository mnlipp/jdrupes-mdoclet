...
public class DeprecatedListWriter
        extends SummaryListWriter<DeprecatedAPIListBuilder> {

    private static final String TERMINALLY_DEPRECATED_KEY
        = "doclet.Terminally_Deprecated_Elements";
    ...
    public static void generate(HtmlConfiguration configuration)
        ...
        if (configuration.conditionalPages
            ...
            depr.generateSummaryListFile(PageMode.DEPRECATED,
                "deprecated elements",
                configuration.contents.deprecatedAPI,
    ...
    protected void addContentSelectors(Content target) {
        ...
        if (releases.size() > 1) {
            Content tabs
                = HtmlTree.DIV(HtmlStyle.checkboxes, contents.getContent(
                    "doclet.Deprecated_API_Checkbox_Label"));
            for (int i = 0; i < releases.size(); i++) {
                // Table column ids are 1-based
                tabs.add(Text.of(" "))
                    .add(getReleaseCheckbox(releases.get(i), i + 1));
            }
            target.add(tabs);
        }
    }
    ...
    private Content getReleaseCheckbox(String name, int index) {
        // Empty string represents other/uncategorized releases. Since we can't
        // make any assumptions
        // about release names this is arguably the safest way to avoid naming
        // collisions.
        boolean isOtherReleases = name.isEmpty();
        Content releaseLabel = isOtherReleases
            ? contents
                .getContent("doclet.Deprecated_API_Checkbox_Other_Releases")
            : Text.of(name);
    ...
    protected void addExtraIndexLink(Content target) {
        if (!builder.getForRemoval().isEmpty()) {
            addIndexLink(HtmlIds.FOR_REMOVAL, "doclet.Terminally_Deprecated",
                target);
        }
    }
        ...
        if (releases.size() > 1) {
            ...
            for (String release : releases) {
                Content tab = TERMINALLY_DEPRECATED_KEY.equals(headingKey)
                    ? contents.getContent(
                        "doclet.Terminally_Deprecated_In_Release", release)
                    : contents.getContent("doclet.Deprecated_In_Release",
                        release);
                table.addTab(tab,
                    element -> release
                        .equals(utils.getDeprecatedSince(element)));
            }
        }
    }
    ...
    protected TableHeader getTableHeader(String headerKey) {
        ...
        return new TableHeader(
            ...
            contents
                .getContent("doclet.Deprecated_Elements_Release_Column_Header"),
            contents.descriptionLabel)
                .sortable(true, true, false); // Allow sorting by element name
                                              // and release
    }
    ...
    protected HtmlStyle[] getColumnStyles() {
        ...
        return new HtmlStyle[] { HtmlStyle.colSummaryItemName,
    ...
    protected Content getTableCaption(String headingKey) {
        ...
        return TERMINALLY_DEPRECATED_KEY.equals(headingKey)
            ? caption
            : contents.getContent("doclet.Deprecated_Elements", caption);
    }
}
