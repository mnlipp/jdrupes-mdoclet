...
public class SpecTaglet extends BaseTaglet implements InheritableTaglet {
    ...
    public Content getAllBlockTagOutput(Element holder, TagletWriter writer) {
        ...
        if (utils.isMethod(holder)) {
            ...
            Optional<
                    Documentation> result = docFinder
                        .search((ExecutableElement) holder,
                            m -> Result.fromOptional(extract(utils, m)))
                        .toOptional();
            if (result.isPresent()) {
                e = result.get().method();
    ...
    private record Documentation(List<? extends SpecTree> specTrees,
    ...
    private static Optional<Documentation> extract(Utils utils,
        ...
        return tags.isEmpty() ? Optional.empty()
            : Optional.of(new Documentation(tags, method));
    }
}
