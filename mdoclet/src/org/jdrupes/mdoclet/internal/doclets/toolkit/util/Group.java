...
public class Group {
     ...
     */
    public boolean checkModuleGroups(String groupname,
            String moduleNameFormList) {
        ...
        for (String mdlPattern : mdlPatterns) {
            if (mdlPattern.length() == 0) {
                initMessages();
                messages.warning("doclet.Error_in_grouplist", groupname,
                    moduleNameFormList);
                return false;
            }
     ...
     */
    public Map<String, SortedSet<ModuleElement>>
            groupModules(Set<ModuleElement> modules) {
        ...
        String defaultGroupName
            = (elementNameGroupMap.isEmpty() && regExpGroupMap.isEmpty())
                ...
                : configuration.getDocResources()
                    .getText("doclet.Other_Modules");
        ...
        for (ModuleElement mdl : modules) {
            String moduleName
                = mdl.isUnnamed() ? null : mdl.getQualifiedName().toString();
            String groupName
                = mdl.isUnnamed() ? null : elementNameGroupMap.get(moduleName);
    ...
    public Map<String, SortedSet<PackageElement>>
            groupPackages(Set<PackageElement> packages) {
        ...
        for (PackageElement pkg : packages) {
            ...
            String groupName
                = pkg.isUnnamed() ? null : elementNameGroupMap.get(pkgName);
     ...
     */
    SortedSet<PackageElement> getPkgList(
            Map<String, SortedSet<PackageElement>> map,
            String groupname) {
        return map.computeIfAbsent(groupname, g -> new TreeSet<>(
            configuration.utils.comparators.makePackageComparator()));
    }
     ...
     * @param groupname Group name to search.
     */
    SortedSet<ModuleElement> getModuleList(
            Map<String, SortedSet<ModuleElement>> map,
            String groupname) {
        return map.computeIfAbsent(groupname, g -> new TreeSet<>(
            configuration.utils.comparators.makeModuleComparator()));
    }
