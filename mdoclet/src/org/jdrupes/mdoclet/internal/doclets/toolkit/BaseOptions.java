...
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.DocletConstants;
...
public abstract class BaseOptions {
     ...
     */
    private final LinkedHashSet<List<String>> customTagStrs
        = new LinkedHashSet<>();
    ...
    private final List<Utils.Pair<String, String>> linkOfflineList
        = new ArrayList<>();
     ...
     */
    private ModularityMismatchPolicy linkModularityMismatch
        = ModularityMismatchPolicy.WARN;
    ...
    public Set<? extends Option> getSupportedOptions() {
        ...
        List<Option> options = List.of(
            ...
            new XOption(resources, "--date", 1) {
                ...
                public boolean process(String opt, List<String> args) {
                    if (noTimestamp) {
                        messages.error("doclet.Option_conflict", "--date",
                            "-notimestamp");
                        return false;
                    }
                    ...
                    try {
                        ...
                        if (date.isBefore(DATE_MIN) || date.isAfter(DATE_MAX)) {
                            messages.error("doclet.Option_date_out_of_range",
                                arg);
                            return false;
                        }
                        return true;
            ...
            new Option(resources, "-excludedocfilessubdir", 1) {
                ...
                public boolean process(String opt, List<String> args) {
                    excludedDocFileDirs
                        .addAll(List.of(args.get(0).split("[,:]")));
                    return true;
                }
            },
            ...
            new Option(resources, "-linkoffline", 2) {
                ...
                public boolean process(String opt, List<String> args) {
                    linkOfflineList
                        .add(new Utils.Pair<>(args.get(0), args.get(1)));
                    return true;
                }
            },
            ...
            new Option(resources, "--link-modularity-mismatch", 1) {
                ...
                public boolean process(String opt, List<String> args) {
                    String s = args.get(0);
                    switch (s) {
                    case "warn", "info" -> linkModularityMismatch
                        = ModularityMismatchPolicy
                            .valueOf(s.toUpperCase(Locale.ROOT));
                    default -> {
                        reporter.print(ERROR, resources.getText(
                            "doclet.Option_invalid", s,
                ...
                public boolean process(String opt, List<String> args) {
                    ...
                    default -> {
                        reporter.print(ERROR,
                            resources.getText("doclet.Option_invalid", o,
                                "--override-methods"));
                        return false;
                    }
                    }
            ...
            new Option(resources, "--since", 1) {
                @Override
                public boolean process(String opt, List<String> args) {
                    since = Arrays.stream(args.get(0).split(","))
                ...
                public boolean process(String opt, List<String> args) {
                    ...
                    try {
                        ...
                        config.reporter.print(ERROR,
                            config.getDocResources().getText(
                                "doclet.Invalid_URL",
                                e.getMessage()));
                        return false;
                    }
    ...
    private boolean checkOutputFileEncoding(String docencoding) {
        ...
        try {
            ...
            config.reporter.print(ERROR,
                config.getDocResources()
                    .getText("doclet.Encoding_not_supported", docencoding));
            return false;
        } finally {
            try {
                if (osw != null) {
    ...
    protected abstract static class Option
            implements Doclet.Option, Comparable<Option> {
        ...
        protected Option(Resources resources, String keyBase, String name,
                int argCount) {
            this.names = name.trim().split("\\s+");
            if (keyBase == null) {
                keyBase = "doclet.usage."
                    + Utils.toLowerCase(names[0]).replaceAll("^-+", "");
            }
            String desc
                = getOptionsMessage(resources, keyBase + ".description");
            if (desc.isEmpty()) {
                ...
                this.parameters
                    = getOptionsMessage(resources, keyBase + ".parameters");
            }
        ...
        public boolean matches(String option) {
            for (String name : names) {
                ...
                if (option.startsWith("--") && option.contains("=")) {
                    return name
                        .equals(option.substring(option.indexOf("=") + 1));
                } else if (matchCase) {
    ...
    protected abstract static class XOption extends Option {
        ...
        public XOption(Resources resources, String prefix, String name,
                int argCount) {
