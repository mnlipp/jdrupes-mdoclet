...
public class AnnotationTypeMemberBuilder extends AbstractMemberBuilder {
    ...
    protected AnnotationTypeMemberBuilder(Context context,
        ...
        // In contrast to the annotation interface member summaries the details
        // generated
        // by this builder share a single list for both required and optional
        // members.
    ...
    protected void buildAnnotationTypeMember(Content target)
        ...
        if (hasMembersToDocument()) {
            ...
            Content annotationDetailsHeader
                = writer.getAnnotationDetailsHeader();
            Content memberList = writer.getMemberList();

            for (Element member : members) {
                currentMember = member;
                Content annotationContent
                    = writer.getAnnotationHeaderContent(currentMember);

                buildAnnotationTypeMemberChildren(annotationContent);

                memberList.add(writer.getMemberListItem(annotationContent));
            }
            Content annotationDetails = writer
                .getAnnotationDetails(annotationDetailsHeader, memberList);
            target.add(annotationDetails);
        }
    }

    protected void
            buildAnnotationTypeMemberChildren(Content annotationContent) {
