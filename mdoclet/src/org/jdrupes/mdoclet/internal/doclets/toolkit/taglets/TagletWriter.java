...
import org.jdrupes.mdoclet.internal.doclets.toolkit.taglets.Taglet.UnsupportedTagletOperationException;
...
public abstract class TagletWriter {
     ...
     * @return the output
     */
    protected abstract Content literalTagOutput(Element element,
            LiteralTree tag);
     ...
     * @return the output
     */
    protected abstract Content paramTagOutput(Element element,
            ParamTree paramTag, String paramName);
     ...
     * @return the output
     */
    protected abstract Content returnTagOutput(Element element,
            ReturnTree returnTag, boolean inline);
     ...
     * @return the output
     */
    protected abstract Content seeTagOutput(Element element,
     ...
     * @return the output
     */
    protected abstract Content snippetTagOutput(Element element,
            SnippetTree snippetTag, StyledText text,
     ...
     * @return the output
     */
    protected abstract Content specTagOutput(Element element,
     ...
     * @return the output
     */
    protected abstract Content invalidTagOutput(String summary,
    ...
    public Content getBlockTagOutput(TagletManager tagletManager,
        ...
        for (Taglet taglet : taglets) {
            if (utils.isTypeElement(element) && taglet instanceof ParamTaglet) {
                // The type parameters and state components are documented in a
                // special
            ...
            if (element.getKind() == ElementKind.MODULE
                && taglet instanceof BaseTaglet t) {
                switch (t.getTagKind()) {
            ...
            try {
                Content tagletOutput
                    = taglet.getAllBlockTagOutput(element, this);
                ...
                // malformed taglet:
                // claims to support block tags (see Taglet.isBlockTag) but does
                // not provide the
    ...
    public Content getInlineTagOutput(Element holder,
        ...
        try {
            Content tagletOutput
                = t.getInlineTagOutput(holder, inlineTag, this);
            tagletManager.seenTag(t.getName());
            return tagletOutput;
        } catch (UnsupportedTagletOperationException e) {
            // malformed taglet:
            // claims to support inline tags (see Taglet.isInlineTag) but does
            // not provide the
     ...
     */
    public abstract Content commentTagsToOutput(DocTree holderTree,
     ...
     * @return the generated content
     */
    public abstract Content commentTagsToOutput(Element element,
            List<? extends DocTree> trees);
