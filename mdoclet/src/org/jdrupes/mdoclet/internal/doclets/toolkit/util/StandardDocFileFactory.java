...
class StandardDocFileFactory extends DocFileFactory {
    ...
    public void setDestDir(String destDirName) throws SimpleDocletException {
        ...
        if (!destDirName.isEmpty()
            || !fileManager
                .hasLocation(DocumentationTool.Location.DOCUMENTATION_OUTPUT)) {
            try {
                ...
                // generic IOException from file manager, setting location, e.g.
                // file not a directory
                String message = configuration.getDocResources()
                    .getText("doclet.error.initializing.dest.dir", e);
                throw new SimpleDocletException(message, e);
            }
        }

        destDir = fileManager
            .getLocationAsPaths(DocumentationTool.Location.DOCUMENTATION_OUTPUT)
            .iterator().next();
    }
    ...
    public DocFile createFileForOutput(DocPath path) {
        return new StandardDocFile(
            DocumentationTool.Location.DOCUMENTATION_OUTPUT, path);
    }
    ...
    class StandardDocFile extends DocFile {
        ...
        StandardDocFile(Location location, DocPath path) {
            super(location, path);
            Assert.check(
                location == DocumentationTool.Location.DOCUMENTATION_OUTPUT);
        ...
        public InputStream openInputStream() throws DocFileIOException {
            try {
                ...
                throw new DocFileIOException(this, DocFileIOException.Mode.READ,
                    e);
            }
        }
        ...
        public OutputStream openOutputStream() throws DocFileIOException {
            ...
            try {
                OutputStream out
                    = getFileObjectForOutput(path).openOutputStream();
        ...
        public Writer openWriter()
            ...
            try {
                ...
                return new BufferedWriter(
                    new OutputStreamWriter(out, docencoding));
            } catch (IOException e) {
                throw new DocFileIOException(this,
                    DocFileIOException.Mode.WRITE, e);
            }
        ...
        private FileObject getFileObjectForOutput(DocPath path)
                throws IOException {
