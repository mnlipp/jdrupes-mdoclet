...
public class ParamTaglet extends BaseTaglet implements InheritableTaglet {
     ...
     */
    public ParamTaglet() {
        super(DocTree.Kind.PARAM, false,
            EnumSet.of(Location.TYPE, Location.CONSTRUCTOR, Location.METHOD));
    }
    ...
    public Output inherit(Element owner, DocTree tag, boolean isFirstSentence,
        ...
        Map<String, Integer> stringIntegerMap
            = mapNameToPosition(configuration.utils, parameterElements);
        ...
        // try to inherit description of the respective parameter in an
        // overridden method
        try {
            var docFinder = configuration.utils.docFinder();
            var r = docFinder.trySearch(method,
                m -> Result.fromOptional(extract(configuration.utils, m,
                    position, param.isTypeParameter())))
                .toOptional();
            return r
                .map(result -> new Output(result.paramTree, result.method,
                    result.paramTree.getDescription(), true))
     ...
     */
    private static Map<String, Integer> mapNameToPosition(Utils utils,
    ...
    public Content getAllBlockTagOutput(Element holder, TagletWriter writer) {
        ...
        if (utils.isExecutableElement(holder)) {
            ...
            Content output
                = convertParams(typeElement, ParamKind.TYPE_PARAMETER,
                    utils.getTypeParamTrees(typeElement),
                    typeElement.getTypeParameters(), writer);
            output.add(convertParams(typeElement, ParamKind.RECORD_COMPONENT,
                utils.getParamTrees(typeElement),
                typeElement.getRecordComponents(), writer));
            return output;
        }
    }
    ...
    private Content convertParams(Element e,
        ...
        if (!tags.isEmpty()) {
            Map<String, Integer> positionOfName
                = mapNameToPosition(writer.configuration().utils, parameters);
            for (ParamTree tag : tags) {
                String name = ch.getParameterName(tag);
                String paramName
                    = kind == ParamKind.TYPE_PARAMETER ? "<" + name + ">"
                        : name;
                if (!positionOfName.containsKey(name)) {
                    ...
                    if (!writer.configuration()
                        ...
                        messages.warning(ch.getDocTreePath(tag), key,
                            paramName);
                    }
                }
        ...
        if (tags.size() > tagOfPosition.size()) {
            // Generate documentation for remaining tags that do not match a
            // declared parameter.
    ...
    private Content getInheritedTagletOutput(ParamKind kind,
        ...
        var r = utils.docFinder().search((ExecutableElement) holder,
            m -> Result.fromOptional(
                extract(utils, m, position, kind == ParamKind.TYPE_PARAMETER)))
            .toOptional();
        if (r.isPresent()) {
    ...
    private record Documentation(ParamTree paramTree,
            ExecutableElement method) {
    }

    private static Optional<Documentation> extract(Utils utils,
            ExecutableElement method, Integer position, boolean typeParam) {
        ...
        return tags.stream().filter(
            t -> position.equals(positionOfName.get(ch.getParameterName(t))))
