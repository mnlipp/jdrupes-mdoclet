...
public class ReturnTaglet extends BaseTaglet implements InheritableTaglet {
    ...
    public Output inherit(Element owner, DocTree tag, boolean isFirstSentence,
        ...
        try {
            ...
            var r
                = docFinder
                    .trySearch((ExecutableElement) owner,
                        m -> Result
                            .fromOptional(extract(configuration.utils, m)))
                    .toOptional();
            return r
                .map(result -> new Output(result.returnTree, result.method,
                    result.returnTree.getDescription(), true))
    ...
    public Content getAllBlockTagOutput(Element holder, TagletWriter writer) {
        ...
        // make sure we are not using @return on a method with the void return
        // type
        TypeMirror returnType
            = utils.getReturnType(writer.getCurrentPageElement(), method);
        if (returnType != null && utils.isVoid(returnType)) {
            if (!tags.isEmpty()
                && !writer.configuration().isDocLintReferenceGroupEnabled()) {
        ...
        return docFinder
            .search(method, m -> Result.fromOptional(extract(utils, m)))
            .toOptional()
            .map(r -> writer.returnTagOutput(r.method, r.returnTree, false))
            .orElse(null);
    }

    private record Documentation(ReturnTree returnTree,
    ...
    private static Optional<Documentation> extract(Utils utils,
            ExecutableElement method) {
        // TODO
        // Using getBlockTags(..., Kind.RETURN) for clarity. Since @return has
        // become a bimodal tag,
        // Utils.getReturnTrees is now a misnomer: it returns only block
        // returns, not all returns.
        // We could revisit this later.
        Stream<? extends ReturnTree> blockTags
            = utils.getBlockTags(method, DocTree.Kind.RETURN, ReturnTree.class)
                .stream();
        Stream<? extends ReturnTree> mainDescriptionTags
            = utils.getFirstSentenceTrees(method).stream()
                .mapMulti((t, c) -> {
                    if (t.getKind() == DocTree.Kind.RETURN)
                        c.accept((ReturnTree) t);
                });
        // this method should not check validity of @return tags, hence findAny
        // and not findFirst or what have you
