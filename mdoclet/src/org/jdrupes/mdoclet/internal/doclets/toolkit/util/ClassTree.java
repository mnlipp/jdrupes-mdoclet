...
public class ClassTree {
    ...
    public static class Hierarchy {
        ...
        public SortedSet<TypeElement> subtypes(TypeElement typeElement) {
            return subtypes.getOrDefault(typeElement,
    ...
    private static class SubtypeMap
            extends HashMap<TypeElement, SortedSet<TypeElement>> {
        private final Comparator<Element> comparator;
        ...
        SortedSet<TypeElement> getSubtypes(TypeElement typeElement) {
            return computeIfAbsent(typeElement,
                te -> new TreeSet<>(comparator));
        }
    ...
    */
    ...
    public ClassTree(BaseConfiguration configuration) {
        ...
        Comparator<Element> comparator
            = utils.comparators.makeClassUseComparator();
     ...
     */
    public ClassTree(SortedSet<TypeElement> classesSet,
    ...
    private void buildTree(Iterable<TypeElement> typeElements) {
        for (TypeElement te : typeElements) {
            ...
            if (utils.isEnum(te)) {
                ...
                processType(te,
                    hierarchies.get(HierarchyKind.ANNOTATION_INTERFACES));
            }
        }
    }
    ...
    private void processType(TypeElement typeElement, Hierarchy hierarchy) {
        TypeElement superclass
            = utils.getFirstVisibleSuperClassAsTypeElement(typeElement);
        if (superclass != null) {
    ...
    private void processInterface(TypeElement typeElement) {
        Hierarchy interfacesHierarchy
            = hierarchies.get(HierarchyKind.INTERFACES);
        List<? extends TypeMirror> interfaces = typeElement.getInterfaces();
        if (!interfaces.isEmpty()) {
            for (TypeMirror t : interfaces) {
                if (!interfacesHierarchy.subtypes
                    .addSubtype(utils.asTypeElement(t), typeElement)) {
                    return;
                } else {
    ...
    public SortedSet<TypeElement> implementingClasses(TypeElement typeElement) {
        SortedSet<TypeElement> result
            = implementingClasses.getSubtypes(typeElement);
        SortedSet<TypeElement> interfaces
            = hierarchy(typeElement).allSubtypes(typeElement);
