...
import org.jdrupes.mdoclet.internal.doclets.toolkit.taglets.TagletManager;
...
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.DocFileFactory;
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.Extern;
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.Group;
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.MetaKeywords;
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.SimpleDocletException;
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.TypeElementCatalog;
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.Utils;
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.VisibleMemberCache;
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.VisibleMemberTable;
import org.jdrupes.mdoclet.internal.doclets.toolkit.util.Utils.Pair;
import org.jdrupes.mdoclet.internal.doclint.DocLint;
import org.jdrupes.mdoclet.internal.doclint.Env;
...
public abstract class BaseConfiguration {
    ...
    protected void initConfiguration(DocletEnvironment docEnv,
        ...
        if (initialized) {
            throw new IllegalStateException(
                "configuration previously initialized");
        }
        initialized = true;
        this.docEnv = docEnv;
        ...
        specifiedModuleElements
            = Collections.unmodifiableSet(specifiedSplitter.mset);
        specifiedPackageElements
            = Collections.unmodifiableSet(specifiedSplitter.pset);
        specifiedTypeElements
            = Collections.unmodifiableSet(specifiedSplitter.tset);

        Splitter includedSplitter = new Splitter(docEnv, true);
        includedModuleElements
            = Collections.unmodifiableSet(includedSplitter.mset);
        includedPackageElements
            = Collections.unmodifiableSet(includedSplitter.pset);
    ...
    private void initModules() {
        ...
        for (PackageElement p : packages) {
            ...
            if (mdle != null && !mdle.isUnnamed()) {
                Set<PackageElement> s = modulePackages
                    .computeIfAbsent(mdle, m -> new TreeSet<>(
                        comparators.makePackageComparator()));
                s.add(p);
            }
        }
        ...
        modules.forEach(
            mdle -> modulePackages.computeIfAbsent(mdle, m -> Set.of()));
    ...
    protected boolean finishOptionSettings0() throws DocletException {
        ...
        for (Pair<String, String> linkOfflinePair : options.linkOfflineList()) {
            extern.link(linkOfflinePair.first, linkOfflinePair.second,
                reporter);
        }
        if (!options.noPlatformLinks()) {
            extern.checkPlatformLinks(options.linkPlatformProperties(),
                reporter);
        }
        ...
        if (snippetPath != null) {
            ...
            if (fm instanceof StandardJavaFileManager) {
                try {
                    List<Path> sp
                        = Arrays.stream(snippetPath.split(File.pathSeparator))
                            .map(Path::of)
                            .toList();
                    StandardJavaFileManager sfm = (StandardJavaFileManager) fm;
                    sfm.setLocationFromPaths(
                        DocumentationTool.Location.SNIPPET_PATH, sp);
                } catch (IOException | InvalidPathException e) {
                    throw new SimpleDocletException(
                        messages.getResources().getText(
                            "doclet.error_setting_snippet_path", snippetPath,
                            e),
                        e);
                }
            } else {
        ...
        overviewElement
            = new OverviewElement(unnamedPackage, getOverviewPath());
        return true;
    }
    ...
    public void initDocLint(List<String> opts, Set<String> customTagNames) {
        ...
        // basic analysis of -Xmsgs and -Xmsgs: options to see if doclint is
        // enabled
        ...
        for (String opt : opts) {
            if (opt.equals(DocLint.XMSGS_OPTION)) {
                ...
                String[] args
                    = opt.substring(DocLint.XMSGS_CUSTOM_PREFIX.length())
        ...
        doclint = new DocLint();
        doclint.init(docEnv.getDocTrees(), docEnv.getElementUtils(),
            docEnv.getTypeUtils(),
            doclintOpts.toArray(new String[0]));
    }

    public boolean isDocLintReferenceGroupEnabled() {
        return isDocLintGroupEnabled(
            org.jdrupes.mdoclet.internal.doclint.Messages.Group.REFERENCE);
    }

    public boolean isDocLintSyntaxGroupEnabled() {
        return isDocLintGroupEnabled(
            org.jdrupes.mdoclet.internal.doclint.Messages.Group.SYNTAX);
    }

    private boolean isDocLintGroupEnabled(
            org.jdrupes.mdoclet.internal.doclint.Messages.Group group) {
        // Use AccessKind.PUBLIC as a stand-in, since it is not common to
        // set DocLint options per access kind (as is common with javac.)
        // A more sophisticated solution might be to derive the access kind from
        // the
        // element owning the comment, and its enclosing elements.
        return doclint != null
            && doclint.isGroupEnabled(group, Env.AccessKind.PUBLIC);
    }
    // </editor-fold>
}
