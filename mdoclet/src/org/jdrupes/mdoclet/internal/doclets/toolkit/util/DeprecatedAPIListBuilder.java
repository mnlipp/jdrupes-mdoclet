...
public class DeprecatedAPIListBuilder extends SummaryAPIListBuilder {
    ...
    public DeprecatedAPIListBuilder(BaseConfiguration configuration,
        ...
        buildSummaryAPIInfo();
        // The releases list is set to the intersection of releases defined via
        // `--since` option
        // and actually occurring values of `Deprecated.since` in documented
        // deprecated elements.
        // If there are `Deprecated.since` values not contained in the `--since`
        // option list
        // an empty string is added to the releases list which causes the writer
        // to generate
        ...
        if (!releases.isEmpty()) {
            ...
            if (!releases.containsAll(foundReleases)) {
                // Empty string is added for other releases, including the
                // default value ""
                releases.add("");
            }
        }
