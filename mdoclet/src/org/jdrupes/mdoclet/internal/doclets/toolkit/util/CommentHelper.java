...
public class CommentHelper {
     ...
     */
    public CommentHelper(BaseConfiguration configuration, Element element,
            TreePath path, DocCommentTree dcTree) {
    ...
    public String getTagName(DocTree dtree) {
        ...
        case AUTHOR, DEPRECATED, PARAM, PROVIDES, RETURN, SEE, SERIAL_DATA, SERIAL_FIELD, THROWS, UNKNOWN_BLOCK_TAG, USES, VERSION -> ((BlockTagTree) dtree)
            .getTagName();
    ...
    Element getElement(ReferenceTree rtree) {
        ...
        return e == null || e.getKind() == ElementKind.CLASS
            && e.asType().getKind() != TypeKind.DECLARED ? null : e;
    }
    ...
    public Element getReferencedMember(Element e) {
        ...
        return (utils.isExecutableElement(e) || utils.isVariableElement(e)) ? e
            : null;
    }
    ...
    public List<? extends DocTree>
            getFirstSentenceTrees(List<? extends DocTree> body) {
    ...
    private static class ReferenceDocTreeVisitor<R>
            extends SimpleDocTreeVisitor<R, Void> {
        @Override
    ...
    public List<? extends DocTree> getTags(DocTree dtree) {
        return new SimpleDocTreeVisitor<List<? extends DocTree>, Void>() {
            ...
            @Override
            public List<? extends DocTree> visitAuthor(AuthorTree node,
                    Void p) {
                return node.getName();
            }

            @Override
            public List<? extends DocTree> visitComment(CommentTree node,
                    Void p) {
                return asList(node.getBody());
            }

            @Override
            public List<? extends DocTree> visitDeprecated(DeprecatedTree node,
                    Void p) {
                return node.getBody();
            }

            @Override
            public List<? extends DocTree> visitDocComment(DocCommentTree node,
                    Void p) {
                return node.getBody();
            }

            @Override
            public List<? extends DocTree> visitEscape(EscapeTree node,
                    Void p) {
                return asList(node.getBody());
            }

            @Override
            public List<? extends DocTree> visitLiteral(LiteralTree node,
                    Void p) {
                return asList(node.getBody().getBody());
            }

            @Override
            public List<? extends DocTree> visitProvides(ProvidesTree node,
                    Void p) {
            ...
            @Override
            public List<? extends DocTree> visitVersion(VersionTree node,
                    Void p) {
                return node.getBody();
            }
            ...
            @Override
            public List<? extends DocTree> visitReturn(ReturnTree node,
                    Void p) {
                return node.getDescription();
            }
            ...
            @Override
            public List<? extends DocTree> visitSerial(SerialTree node,
                    Void p) {
                return node.getDescription();
            }

            @Override
            public List<? extends DocTree> visitSerialData(SerialDataTree node,
                    Void p) {
                return node.getDescription();
            }
            ...
            @Override
            public List<? extends DocTree>
                    visitSerialField(SerialFieldTree node, Void p) {
                return node.getDescription();
            }
            ...
            @Override
            public List<? extends DocTree> visitThrows(ThrowsTree node,
                    Void p) {
                return node.getDescription();
            }
            ...
            @Override
            public List<? extends DocTree>
                    visitUnknownBlockTag(UnknownBlockTagTree node, Void p) {
            ...
            @Override
            protected List<? extends DocTree> defaultAction(DocTree node,
                    Void p) {
                return List.of();
            }
        }.visit(dtree, null);
    ...
    public DocTreePath getDocTreePath(DocTree dtree) {
        ...
        if (dtPath == null && element instanceof ExecutableElement ee) {
            // The overriding element has a doc tree, but it doesn't contain
        ...
        Optional<ExecutableElement> inheritedDoc = docFinder.search(ee,
            (m -> {
                Optional<ExecutableElement> optional
                    = utils.getFullBody(m).isEmpty() ? Optional.empty()
                        : Optional.of(m);
