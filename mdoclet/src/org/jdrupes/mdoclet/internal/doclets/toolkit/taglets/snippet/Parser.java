...
package org.jdrupes.mdoclet.internal.doclets.toolkit.taglets.snippet;
...
import org.jdrupes.mdoclet.internal.doclets.toolkit.taglets.SnippetTaglet;
...
public final class Parser {
    ...
    public Result parse(SnippetTaglet.Diags diags,
            Optional<SnippetTaglet.Language> language, String source)
            throws ParseException {
        SnippetTaglet.Language lang
            = language.orElse(SnippetTaglet.Language.JAVA);
        var p = switch (lang) {
     ...
     */
    private Result parse(SnippetTaglet.Diags diags, Pattern commentPattern,
        ...
        // while lines could be computed lazily, it would yield more complex
        // code
        record OffsetAndLine(int offset, String line) {
        }
        var offsetAndLines = new LinkedList<OffsetAndLine>();
        forEachLine(source,
            (off, line) -> offsetAndLines.add(new OffsetAndLine(off, line)));
        ...
        while (iterator.hasNext()) {
            ...
            if (!markedUpLine.matches()) { // (1)
                ...
                String maybeMarkup
                    = rawLine.substring(markedUpLine.start("markup"));
                List<Tag> parsedTags;
                try {
                    parsedTags = markupParser.parse(maybeMarkup);
                } catch (ParseException e) {
                    // translate error position from markup to file line
                    throw new ParseException(e::getMessage, next.offset()
                        + markedUpLine.start("markup") + e.getPosition());
                }
                ...
                for (var tagIterator = thisLineTags.iterator();
                        tagIterator.hasNext();) {
                ...
                if (parsedTags.isEmpty()) { // (2)
                    diags.warn(
                        resources.getText("doclet.snippet.markup.spurious"),
                        next.offset() + markedUpLine.start("markup"));
                    line = rawLine + (addLineTerminator ? "\n" : "");
                } else { // (3)
                    hasMarkup = true;
                    String payload
                        = rawLine.substring(0, markedUpLine.end("payload"));
            ...
            for (Tag t : thisLineTags) {
                t.start = lineStart;
                t.end = lineStart + line.length(); // this includes line
                                                   // terminator, if any
                processTag(t);
            }
            ...
            append(text,
                line.isBlank() && hasMarkup ? Set.of(new Style.Markup())
                    : Set.of(),
                line);
        ...
        if (!previousLineTags.isEmpty()) {
            ...
            String message = resources
                .getText("doclet.snippet.markup.tag.non.existent.lines");
        ...
        for (var t : tags) {
            ...
            final var substring
                = attributes.get("substring", Attribute.Valued.class);
            final var regex = attributes.get("regex", Attribute.Valued.class);

            if (!t.name().equals("start") && substring.isPresent()
                && regex.isPresent()) {
            ...
            case "link" -> {
                ...
                String typeValue
                    = type.isPresent() ? type.get().value() : "link";
                if (!typeValue.equals("link")
                    && !typeValue.equals("linkplain")) {
                ...
                AddStyle a = new AddStyle(new Style.Link(target.value()),
                    // the default regex is different so as not to include
                    // newline
            ...
            case "replace" -> {
                var replacement
                    = attributes.get("replacement", Attribute.Valued.class)
                        .orElseThrow(() -> newParseException(t.lineSourceOffset
                            + t.markupLineOffset + t.nameLineOffset,
            ...
            case "highlight" -> {
                ...
                String typeValue
                    = type.isPresent() ? type.get().value() : "bold";
            ...
            case "start" -> {
                ...
                actions.add(new Bookmark(region.value(),
        ...
        if (!regions.isEmpty()) {
            Optional<Tag> tag = regions.removeLast(); // any of these tags would
                                                      // do
            Tag t = tag.get();
            String message
                = resources.getText("doclet.snippet.markup.region.unpaired");
            throw new ParseException(() -> message, t.lineSourceOffset
                + t.markupLineOffset + t.nameLineOffset);
        }
    ...
    private ParseException newParseException(int pos, String key,
            Object... args) {
    ...
    private Pattern createRegexPattern(Optional<Attribute.Valued> substring,
        ...
        if (substring.isPresent()) {
            ...
            try {
                ...
                String message
                    = resources.getText("doclet.snippet.markup.regex.invalid");
    ...
    private void processTag(Tag t) throws ParseException {
        ...
        Attributes attributes = new Attributes(t.attributes()); // TODO: avoid
                                                                // creating
                                                                // attributes
                                                                // twice
        ...
        if (!t.name().equals("end")) {
            ...
            if (region.isPresent()) {
                if (region.get() instanceof Attribute.Valued v) {
                    ...
                    if (!regions.addNamed(name, t)) {
                        throw newParseException(
                            t.lineSourceOffset + t.markupLineOffset
                                + v.valueStartPosition(),
                            "doclet.snippet.markup.region.duplicated", name);
                    }
                } else {
                    // TODO: change to exhaustive switch after "Pattern Matching
                    // for switch" is implemented
            ...
            if (region.isEmpty()
                || region.get() instanceof Attribute.Valueless) {
                ...
                if (tag.isEmpty()) {
                    throw newParseException(
                        t.lineSourceOffset + t.markupLineOffset
                            + region.get().nameStartPosition(),
                        "doclet.snippet.markup.region.unpaired", name);
                }
                completeTag(tag.get(), t);
            }
        }
    ...
    public static final class Regions {
         ...
         */
        private final ArrayList<Map.Entry<Optional<String>, Tag>> tags
            = new ArrayList<>();
