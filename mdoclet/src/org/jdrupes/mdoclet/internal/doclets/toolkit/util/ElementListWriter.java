...
public class ElementListWriter {
    ...
    public ElementListWriter(BaseConfiguration configuration) {
        file = DocFile.createFileForOutput(configuration,
     ...
     */
    public static void generate(BaseConfiguration configuration)
            throws DocFileIOException {
        ElementListWriter elemgen = new ElementListWriter(configuration);
        elemgen.generateElementListFile(configuration.docEnv);
    }

    protected void generateElementListFile(DocletEnvironment docEnv)
            throws DocFileIOException {
        try (Writer fileWriter = file.openWriter();
                BufferedWriter out
                    = (fileWriter instanceof BufferedWriter b) ? b
                        : new BufferedWriter(fileWriter)) {
            if (configuration.showModules) {
                for (ModuleElement mdle : configuration.modulePackages
                    .keySet()) {
                    if (!(options.noDeprecated() && utils.isDeprecated(mdle))) {
                        out.write(
                            DocletConstants.MODULE_PREFIX + mdle.toString());
                        out.newLine();
                        for (PackageElement pkg : configuration.modulePackages
                            .get(mdle)) {
                ...
                for (PackageElement pkg : configuration.packages) {
                    // if the -nodeprecated option is set and the package is
                    // marked as
            ...
            throw new DocFileIOException(file, DocFileIOException.Mode.WRITE,
                e);
        }
    }
}
