...
public class SummaryAPIListBuilder {
    ...
    protected void buildSummaryAPIInfo() {
        ...
        for (TypeElement te : configuration.getIncludedTypeElements()) {
            ...
            if (belongsToSummary.test(te)) {
                ...
                case CLASS -> {
                    if (utils.isThrowable(te)) {
                        eset = summaryMap
                            .get(SummaryElementKind.EXCEPTION_CLASS);
                    } else {
            ...
            if (utils.isEnum(te)) {
                composeSummaryList(
                    summaryMap.get(SummaryElementKind.ENUM_CONSTANT),
                    utils.getEnumConstants(te));
            }
            if (utils.isRecord(te)) {
                for (RecordComponentElement component : te
                    .getRecordComponents()) {
                    if (belongsToSummary.test(component)) {
                        throw new AssertionError(
                            "record components not supported in summary builders: "
                                +
            ...
            if (utils.isAnnotationInterface(te)) {
                composeSummaryList(
                    summaryMap.get(SummaryElementKind.ANNOTATION_TYPE_MEMBER),
     ...
     */
    private void composeSummaryList(SortedSet<Element> sset,
