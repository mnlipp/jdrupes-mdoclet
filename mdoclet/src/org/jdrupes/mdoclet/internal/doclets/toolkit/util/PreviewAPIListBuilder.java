...
public class PreviewAPIListBuilder extends SummaryAPIListBuilder {
     ...
     */
    public record JEP(int number, String title, String status)
            implements Comparable<JEP> {
        @Override
    ...
    protected void handleElement(Element e) {
        ...
        JEP jep = jeps.computeIfAbsent(feature, (featureName) -> {
            Map<? extends ExecutableElement, ? extends AnnotationValue> anno
                = configuration.workArounds.getJepInfo(featureName);
            int number = 0;
            String title = "";
            String status = "Preview"; // Default value is not returned by the
                                       // method we use above.
