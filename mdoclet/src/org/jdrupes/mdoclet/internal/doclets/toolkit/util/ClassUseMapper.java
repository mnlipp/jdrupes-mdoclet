...
public class ClassUseMapper {
    ...
    public final Map<TypeElement,
            List<PackageElement>> classToPackageAnnotations = new HashMap<>();

    /**
     * Mapping of TypeElements to a set of TypeElements used by that class.
     */
    public final Map<TypeElement, Set<TypeElement>> classToClass
        = new HashMap<>();
     ...
     * that class.
     */
    public final Map<TypeElement, List<TypeElement>> classToSubclass
        = new HashMap<>();
     ...
     */
    public final Map<TypeElement, List<VariableElement>> classToField
        = new HashMap<>();

    /**
     * Mapping of TypeElements to list of ExecutableElements returning that class.
     */
    public final Map<TypeElement, List<ExecutableElement>> classToMethodReturn
        = new HashMap<>();

    /**
     * Mapping of TypeElements to list of ExecutableElements having that class as an arg.
     */
    public final Map<TypeElement, List<ExecutableElement>> classToMethodArgs
        = new HashMap<>();

    /**
     * Mapping of TypeElements to list of ExecutableElements which throws that class.
     */
    public final Map<TypeElement, List<ExecutableElement>> classToMethodThrows
        = new HashMap<>();
     ...
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToConstructorArgs = new HashMap<>();

    /**
     * Mapping of TypeElements to list of constructors which throws that class.
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToConstructorThrows = new HashMap<>();

    /**
     * The mapping of TypeElements representing annotations to constructors that use them.
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToConstructorAnnotations
                = new HashMap<>();
     ...
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToConstructorParamAnnotation
                = new HashMap<>();
     ...
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToConstructorArgTypeParam
                = new HashMap<>();

    /**
     * The mapping of TypeElement to TypeElement that use them as type parameters.
     */
    public final Map<TypeElement, List<TypeElement>> classToClassTypeParam
        = new HashMap<>();

    /**
     * The mapping of TypeElement representing annotation to TypeElements that use them.
     */
    public final Map<TypeElement, List<TypeElement>> classToClassAnnotations
        = new HashMap<>();
     ...
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToMethodTypeParam = new HashMap<>();

    /**
     * The mapping of TypeElement to method arguments that use them as type parameters.
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToMethodArgTypeParam
                = new HashMap<>();
     ...
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToMethodAnnotations = new HashMap<>();
     ...
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToMethodReturnTypeParam
                = new HashMap<>();
     ...
     */
    public final Map<TypeElement,
            List<ExecutableElement>> classToMethodParamAnnotation
                = new HashMap<>();

    /**
     * The mapping of TypeElements to fields that use them as type parameters.
     */
    public final Map<TypeElement, List<VariableElement>> classToFieldTypeParam
        = new HashMap<>();

    /**
     * The mapping of TypeElements representing annotation to fields that use them.
     */
    public final Map<TypeElement, List<VariableElement>> annotationToField
        = new HashMap<>();
    ...
    public ClassUseMapper(BaseConfiguration configuration,
            ClassTree classTree) {
        ...
        for (TypeElement aClass : classes) {
            ...
            VisibleMemberTable vmt
                = configuration.getVisibleMemberTable(aClass);

            List<VariableElement> fields
                = ElementFilter.fieldsIn(vmt.getVisibleMembers(FIELDS));
            ...
            List<ExecutableElement> ctors
                = ElementFilter.constructorsIn(vmt.getMembers(CONSTRUCTORS));
            ...
            List<ExecutableElement> methods
                = ElementFilter.methodsIn(vmt.getMembers(METHODS));
            ...
            for (ExecutableElement method : methods) {
                ...
                var stv = new SimpleTypeVisitor9<Void, ExecutableElement>() {
                    ...
                    public Void visitDeclared(DeclaredType t,
                        ...
                        add(classToMethodReturn, (TypeElement) t.asElement(),
                            p);
                        return null;
                    }

                    @Override
                    protected Void defaultAction(TypeMirror e,
                            ExecutableElement p) {
                        return null;
                    }
                };
    ...
    private void mapExecutable(ExecutableElement ee) {
        ...
        Set<TypeMirror> classArgs
            = new TreeSet<>(comparators.makeTypeMirrorClassUseComparator());
        for (VariableElement param : ee.getParameters()) {
            TypeMirror pType = param.asType();
            // primitives don't get mapped and type variables are mapped
            // elsewhere
            if (!pType.getKind().isPrimitive()
                && !utils.isTypeVariable(pType)) {
        ...
        for (TypeMirror anException : ee.getThrownTypes()) {
            var stv = new SimpleTypeVisitor9<Void, ExecutableElement>() {
                ...
                public Void visitDeclared(DeclaredType t, ExecutableElement p) {
                    add(isConstructor ? classToConstructorThrows
                        : classToMethodThrows,
                        (TypeElement) t.asElement(), p);
                    return null;
                }
    ...
    private <T> List<T> refList(Map<TypeElement, List<T>> map,
            TypeElement element) {
    ...
    private <T extends Element> void add(Map<TypeElement, List<T>> map,
            TypeElement te, T ref) {
    ...
    private void addAll(Map<TypeElement, List<TypeElement>> map, TypeElement te,
            Collection<TypeElement> refs) {
        if (refs == null) {
            return;
        }
        // add to specified map
     ...
     */
    private <T extends Element> void mapTypeParameters(
            final Map<TypeElement, List<T>> map,
     ...
     */
    private <T extends Element> void mapAnnotations(
            final Map<TypeElement, List<T>> map,
        ...
        new SimpleElementVisitor14<Void, Void>() {
            ...
            void addAnnotations(Element e) {
                for (AnnotationMirror a : e.getAnnotationMirrors()) {
                    add(map, (TypeElement) a.getAnnotationType().asElement(),
                        holder);
                }
            }
    ...
    private <T extends Element> void addTypeParameterToMap(
            final Map<TypeElement, List<T>> map,
