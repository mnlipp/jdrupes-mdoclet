...
public abstract class DocFile {
    ...
    public static DocFile createFileForDirectory(
            BaseConfiguration configuration, String file) {
        return DocFileFactory.getFactory(configuration)
            .createFileForDirectory(file);
    }

    /** Create a DocFile for a file that will be opened for reading. */
    public static DocFile createFileForInput(BaseConfiguration configuration,
            String file) {
        return DocFileFactory.getFactory(configuration)
            .createFileForInput(file);
    }
    ...
    public static DocFile createFileForOutput(BaseConfiguration configuration,
            DocPath path) {
        return DocFileFactory.getFactory(configuration)
     ...
     *  list files
     */
    public static Iterable<DocFile> list(BaseConfiguration configuration,
            Location location, DocPath path) {
    ...
    public abstract OutputStream openOutputStream()
            throws DocFileIOException, UnsupportedEncodingException;
     ...
     * @throws ResourceIOException if there is a problem while reading the resource
     */
    public void copyResource(DocPath resource, Resources resources)
            throws DocFileIOException, ResourceIOException {
        copyResource(resource, true, resources);
    }

    private void copyResource(DocPath resource, boolean replaceNewLine,
            Resources resources)
            throws DocFileIOException, ResourceIOException {
        try {
            InputStream in = BaseConfiguration.class
                .getResourceAsStream(resource.getPath());
            if (in == null)
                return;
                ...
                if (replaceNewLine) {
                    try (BufferedReader reader
                        ...
                        try (Writer writer = openWriter()) {
                            String line;
                            while ((line
                                = readResourceLine(resource, reader)) != null) {
                                write(this, writer, resources == null ? line
                                    : localize(line, resources));
    ...
    private static final Pattern replacePtn
        = Pattern.compile("##REPLACE:(?<key>[A-Za-z0-9._]+)##");
     ...
     */
    private static int read(DocFile inFile, InputStream input, byte[] buf)
            throws DocFileIOException {
        try {
            return input.read(buf);
        } catch (IOException e) {
            throw new DocFileIOException(inFile, DocFileIOException.Mode.READ,
                e);
        }
    }
     ...
     */
    private static void write(DocFile outFile, OutputStream out, byte[] buf,
            int len) throws DocFileIOException {
        try {
            out.write(buf, 0, len);
        } catch (IOException e) {
            throw new DocFileIOException(outFile, DocFileIOException.Mode.WRITE,
                e);
        }
    }
     ...
     * @throws DocFileIOException if an exception occurred while writing the stream
     */
    private static void write(DocFile outFile, Writer out, String text)
            throws DocFileIOException {
        try {
            out.write(text);
        } catch (IOException e) {
     ...
     */
    private static int readResource(DocPath docPath, InputStream in, byte[] buf)
     ...
     * @throws ResourceIOException if an exception occurred while reading the stream
     */
    private static String readResourceLine(DocPath docPath, BufferedReader in)
            throws ResourceIOException {
        try {
            return in.readLine();
        } catch (IOException e) {
