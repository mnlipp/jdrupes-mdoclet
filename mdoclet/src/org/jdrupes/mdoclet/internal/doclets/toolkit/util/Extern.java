...
public class Extern {
     ...
     */
    public DocLink getExternalLink(Element element, DocPath relativepath,
            String filename) {
    ...
    public DocLink getExternalLink(Element element, DocPath relativepath,
        ...
        DocPath p
            = fnd.relative ? relativepath.resolve(fnd.path).resolve(filename)
                : fnd.path.resolve(filename);
        return new DocLink(p,
            fnd.useOldFormId ? getOldFormHtmlName(memberName) : memberName);
    }
     ...
     */
    public boolean link(String url, Reporter reporter)
     ...
     */
    public boolean link(String url, String elemlisturl, Reporter reporter)
            throws DocFileIOException {
     ...
     */
    public void checkPlatformLinks(String linkPlatformProperties,
            Reporter reporter) {
        PackageElement javaLang
            = utils.elementUtils.getPackageElement("java.lang");
        if (utils.isIncluded(javaLang)) {
            return;
        }
        DocLink link = getExternalLink(javaLang, DocPath.empty,
        ...
        try {
            ...
            if (linkPlatformProperties != null) {
                docUrl = getCustomPlatformDocs(versionNumber,
            ...
            if (elementListUrl == null) {
                ...
                try (InputStream in = open(elementListUrl)) {
                    readElementList(in, docUrl, false, versionNumber,
                        isOldFormPlatformDocs(versionNumber));
     ...
     */
    private String getCustomPlatformDocs(int version,
            String linkPlatformProperties) throws Fault {
        String url;
        try {
            ...
            if (isUrl(linkPlatformProperties)) {
                ...
                inputStream = DocFile
                    .createFileForInput(configuration, linkPlatformProperties)
                    .openInputStream();
            }
            try (inputStream) {
            ...
            throw new Fault(resources.getText("doclet.MalformedURL",
                linkPlatformProperties), exc);
        } catch (IOException exc) {
            throw new Fault(
                resources.getText("doclet.URL_error", linkPlatformProperties),
                exc);
        } catch (DocFileIOException exc) {
            throw new Fault(
                resources.getText("doclet.File_error", linkPlatformProperties),
                exc);
        }
        return url;
    }
    ...
    private int getSourceVersionNumber() {
        ...
        assert SourceVersion
            .valueOf("RELEASE_" + versionNumber) == sourceVersion;
    ...
    private boolean isPrerelease(int sourceVersion) {
        ...
        return docletVersion.feature() == sourceVersion
            && docletVersion.pre().isPresent();
    }
     ...
     * @param reporter The <code>DocErrorReporter</code> used to report errors.
     * 
     * @param linkoffline True if -linkoffline is used and false if -link is
     * used.
     * 
     * @return true if successful, false otherwise
     * 
     * @throws DocFileIOException if there is a problem reading the element list
     * file
     */
    private boolean link(String url, String elemlisturl, Reporter reporter,
            boolean linkoffline)
            throws DocFileIOException {
        ...
        try {
            ...
            if (isUrl(elemlisturl)) {
                readElementListFromURL(url,
                    toURL(adjustEndFileSeparator(elemlisturl)));
            } else {
                readElementListFromFile(url,
                    DocFile.createFileForInput(configuration, elemlisturl));
            }
            return true;
        } catch (Fault f) {
    ...
    private Item findElementItem(Element element) {
        ...
        if (element instanceof ModuleElement me) {
            ...
            Map<String, Item> pkgMap
                = packageItems.get(utils.getModuleName(moduleElement));
            item = (pkgMap != null) ? pkgMap.get(utils.getPackageName(pkg))
                : null;
        }
        return item;
    }
     ...
     */
    private void readElementListFromURL(String urlpath, URL elemlisturlpath)
            throws Fault {
        try {
            URL link = elemlisturlpath.toURI()
                .resolve(DocPaths.ELEMENT_LIST.getPath()).toURL();
            try (InputStream in = open(link)) {
                readElementList(in, urlpath, false, 0, false);
            }
        } catch (URISyntaxException | MalformedURLException exc) {
            throw new Fault(resources.getText("doclet.MalformedURL",
                elemlisturlpath.toString()), exc);
        } catch (IOException exc) {
     ...
     */
    private void readPackageListFromURL(String urlpath, URL elemlisturlpath)
            throws Fault {
        try {
            URL link = elemlisturlpath.toURI()
                .resolve(DocPaths.PACKAGE_LIST.getPath()).toURL();
            try (InputStream in = open(link)) {
            ...
            throw new Fault(resources.getText("doclet.URL_error",
                elemlisturlpath.toString()), exc);
        }
    }
    ...
    private void readElementListFromFile(String path, DocFile elemListPath)
        ...
        if (file.exists()) {
            ...
            if (file1.exists()) {
                ...
                throw new Fault(
                    resources.getText("doclet.File_error", file.getPath()),
                    null);
            }
        }
    }

    private void readElementList(DocFile file, String path,
            boolean isOldFormDoc) throws Fault, DocFileIOException {
        try {
            if (file.canRead()) {
                boolean pathIsRelative
                    = !isUrl(path)
                        && !DocFile.createFileForInput(configuration, path)
                            .isAbsolute();
            ...
            throw new Fault(
                resources.getText("doclet.File_error", file.getPath()), exc);
        }
    }
     ...
     */
    private void readElementList(InputStream input, String path,
            boolean relative, int platformVersion,
            ...
            while ((elemname = in.readLine()) != null) {
                if (elemname.length() > 0) {
                    ...
                    if (elemname.startsWith(DocletConstants.MODULE_PREFIX)) {
                        moduleName = elemname
                            .replace(DocletConstants.MODULE_PREFIX, "");
                        Item item = new Item(moduleName, elempath, relative,
                            isOldFormDoc);
                        moduleItems.put(moduleName, item);
                    } else {
                        DocPath pkgPath
                            = DocPath.create(elemname.replace('.', '/'));
                        // Although being modular, JDKs 9 and 10 do not use
                        // module names in javadoc URL paths.
                        if (moduleName != null && platformVersion != 9
                            && platformVersion != 10) {
                            elempath = elempath.resolve(
                                DocPath.create(moduleName).resolve(pkgPath));
                        } else {
                            elempath = elempath.resolve(pkgPath);
                        }
                        String actualModuleName;
                        // For user provided libraries we check whether
                        // modularity matches the actual library.
                        // We trust modularity to be correct for platform
                        // library element lists.
                        if (platformVersion == 0) {
                            actualModuleName = checkLinkCompatibility(elemname,
                                moduleName, path, showDiagnostic);
                        } else {
                            actualModuleName = moduleName == null
                                ? DocletConstants.DEFAULT_ELEMENT_NAME
                                : moduleName;
                        }
                        Item item = new Item(elemname, elempath, relative,
                            isOldFormDoc);
                        packageItems
                            .computeIfAbsent(actualModuleName,
                                k -> new TreeMap<>())
                            .putIfAbsent(elemname, item); // first-one-wins
                                                          // semantics
     ...
     */
    private String checkLinkCompatibility(String packageName, String moduleName,
            String path, boolean showDiagnostic) {
        ...
        if (pe != null) {
            ...
            if (me == null || me.isUnnamed()) {
                if (moduleName != null && showDiagnostic) {
                    printModularityMismatchDiagnostic(
                        "doclet.linkMismatch_PackagedLinkedtoModule", path);
                }
                // library is not modular, ignore module name even if
                // documentation is modular
                return DocletConstants.DEFAULT_ELEMENT_NAME;
            } else if (moduleName == null) {
                // suppress the diagnostic message in the case of automatic
                // modules
                if (!utils.elementUtils.isAutomaticModule(me)
                    && showDiagnostic) {
                    printModularityMismatchDiagnostic(
                        "doclet.linkMismatch_ModuleLinkedtoPackage", path);
                }
                // library is modular, use module name for lookup even though
                // documentation is not
                return utils.getModuleName(me);
            }
        }
        return moduleName == null ? DocletConstants.DEFAULT_ELEMENT_NAME
            : moduleName;
    }
    ...
    private String getOldFormHtmlName(String name) {
        /*
         * The HTML 4 spec at http://www.w3.org/TR/html4/types.html#h-6.2
         * mentions
         * that the name/id should begin with a letter followed by other valid
         * characters.
         * The HTML 5 spec (draft) is more permissive on names/ids where the
         * only restriction
         * is that it should be at least one character long and should not
         * contain spaces.
         * The spec draft is @
         * http://www.w3.org/html/wg/drafts/html/master/dom.html#the-id-
         * attribute.
         *
         * For HTML 4, we need to check for non-characters at the beginning of
         * the name and
         * substitute it accordingly, "_" and "$" can appear at the beginning of
         * a member name.
         * The method substitutes "$" with "Z:Z:D" and will prefix "_" with
         * "Z:Z".
         */
        ...
        for (int i = 0; i < name.length(); i++) {
            ...
            case ']':
                sb.append(":A");
                break;
            // Any appearance of $ needs to be substituted with ":D" and not
            // with hyphen
            // since a field name "P$$ and a method P(), both valid member
            // names, can end
            // up as "P--". A member name beginning with $ needs to be
            // substituted with
            // "Z:Z:D".
            case '$':
                if (i == 0)
                ...
                sb.append(":D");
                break;
            // A member name beginning with _ needs to be prefixed with "Z:Z"
            // since valid anchor
