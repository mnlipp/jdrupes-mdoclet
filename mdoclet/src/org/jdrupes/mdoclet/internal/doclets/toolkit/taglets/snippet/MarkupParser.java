...
public final class MarkupParser {
    ...
    // Parsing machinery is adapted from
    ...
    protected List<Attribute> attrs() throws ParseException {
        ...
        while (bp < buflen && isIdentifierStart(ch)) {
            ...
            if (ch == '=') {
                ...
                skipWhitespace();
                if (ch == '\'' || ch == '"') {
                    vkind = (ch == '\'') ? ValueKind.SINGLE_QUOTED
                        : ValueKind.DOUBLE_QUOTED;
                    char quote = ch;
                    nextChar();
                    valueStartPos = bp;
                    ...
                    if (bp >= buflen) {
                        String message = resources.getText(
            ...
            var attribute = vkind == ValueKind.EMPTY
                ...
                : new Attribute.Valued(name, value.toString(), nameStartPos,
                    valueStartPos);
