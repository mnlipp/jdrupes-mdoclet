...
import org.jdrupes.mdoclet.internal.doclets.toolkit.SerializedFormWriter;
...
public class SerializedFormBuilder extends AbstractBuilder {
    ...
    public void build() throws DocletException {
        SortedSet<TypeElement> rootclasses
    ...
    protected void buildPackageSerializedForm(Content target)
        ...
        SortedSet<TypeElement> classes
            = utils.getAllClassesUnfiltered(currentPackage);
        if (classes.isEmpty()) {
            return;
        }
    ...
    protected void buildClassSerializedForm(Content target)
        ...
        SortedSet<TypeElement> typeElements
            = utils.getAllClassesUnfiltered(currentPackage);
        for (TypeElement typeElement : typeElements) {
            ...
            if (utils.isClass(currentTypeElement)
                && utils.isSerializable(currentTypeElement)) {
    ...
    protected void buildSerialUIDInfo(Content target) {
        ...
        for (VariableElement field : utils
            .getFieldsUnfiltered(currentTypeElement)) {
            if (field.getSimpleName().toString()
                .compareTo(SERIAL_VERSION_UID) == 0 &&
    ...
    protected void buildSerializableMethods(Content target)
            throws DocletException {
        Content serializableMethodsHeader
            = methodWriter.getSerializableMethodsHeader();
        for (var i = utils.serializationMethods(currentTypeElement).iterator();
                i.hasNext();) {
            currentMember = i.next();
            Content methodsContent
                = methodWriter.getMethodsContentHeader(!i.hasNext());
        ...
        if (!utils.serializationMethods(currentTypeElement).isEmpty()) {
            ...
            if (utils.isSerializable(currentTypeElement)
                && !utils.isExternalizable(currentTypeElement)) {
                if (utils.serializationMethods(currentTypeElement).isEmpty()) {
    ...
    protected void buildMethodSubHeader(Content methodsContent) {
        methodWriter.addMemberHeader((ExecutableElement) currentMember,
            methodsContent);
    }
    ...
    protected void buildDeprecatedMethodInfo(Content methodsContent) {
        methodWriter.addDeprecatedMemberInfo((ExecutableElement) currentMember,
            methodsContent);
    }
    ...
    protected void buildMethodDescription(Content methodsContent) {
        methodWriter.addMemberDescription((ExecutableElement) currentMember,
            methodsContent);
    }
    ...
    protected void buildMethodTags(Content methodsContent) {
        methodWriter.addMemberTags((ExecutableElement) currentMember,
            methodsContent);
        ...
        if (method.getSimpleName().toString().compareTo("writeExternal") == 0
            ...
            if (options.serialWarn()) {
                ...
                messages.warning(currentMember,
                    ...
                    encl.getQualifiedName().toString(),
     ...
     * @param classContent the content to which the documentation will be added
     */
    public void buildFieldSerializationOverview(TypeElement typeElement,
        ...
        if (utils.definesSerializableFields(typeElement)) {
            ...
            if (fieldWriter.shouldPrintOverview(ve)) {
                Content serializableFieldsHeader
                    = fieldWriter.getSerializableFieldsHeader();
                Content fieldsOverviewContent
                    = fieldWriter.getFieldsContentHeader(true);
    ...
    protected void buildSerializableFields(Content target)
            throws DocletException {
        Collection<VariableElement> members
            = utils.serializableFields(currentTypeElement);
        if (!members.isEmpty()) {
            Content serializableFieldsHeader
                = fieldWriter.getSerializableFieldsHeader();
            for (var i = members.iterator(); i.hasNext();) {
                ...
                if (!utils.definesSerializableFields(currentTypeElement)) {
                    Content fieldsContent
                        = fieldWriter.getFieldsContentHeader(!i.hasNext());
    ...
    protected void buildSerialFieldTagsInfo(Content target) {
        ...
        // element.)
        SortedSet<SerialFieldTree> tags
            = new TreeSet<>(utils.comparators.makeSerialFieldTreeComparator());
        // sort the elements
        tags.addAll(utils.getSerialFieldTrees(field));

        CommentHelper ch = utils.getCommentHelper(field);
        for (SerialFieldTree tag : tags) {
            if (tag.getName() == null || tag.getType() == null)  // ignore
                                                                 // malformed
                                                                 // @serialField
                                                                 // tags
                continue;
            Content fieldsContent
                = fieldWriter.getFieldsContentHeader(tag.equals(tags.last()));
            TypeMirror type = ch.getReferencedType(tag);
            fieldWriter.addMemberHeader(type,
                tag.getName().getName().toString(), fieldsContent);
    ...
    protected void buildFieldInfo(Content fieldsContent) {
        ...
        if ((utils.getSerialTrees(field).isEmpty()) /*
                                                     * && !field.isSynthetic()
                                                     */
            && options.serialWarn()) {
    ...
    private static boolean serialClassInclude(Utils utils, TypeElement te) {
        ...
        if (utils.isSerializable(te)) {
            if (utils.hasDocCommentTree(te)
                && !utils.getSerialTrees(te).isEmpty()) {
