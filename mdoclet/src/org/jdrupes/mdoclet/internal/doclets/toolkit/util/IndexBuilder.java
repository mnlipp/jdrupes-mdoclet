...
public class IndexBuilder {
    ...
    public IndexBuilder(BaseConfiguration configuration,
        ...
        mainComparator
            = classesOnly ? makeClassComparator() : makeIndexComparator();
    }
    ...
    public void addElements() {
        ...
        Set<PackageElement> packages
            = configuration.getSpecifiedPackageElements();
        if (packages.isEmpty()) {
            packages = classes
                .stream()
    ...
    public SortedSet<IndexItem> getItems(DocTree.Kind kind) {
        ...
        return itemsByCategory
            .getOrDefault(IndexItem.Category.TAGS, Collections.emptySortedSet())
            .stream()
            .filter(i -> i.isKind(kind))
            .collect(
                Collectors.toCollection(() -> new TreeSet<>(mainComparator)));
    }
    ...
    private void indexMembers(TypeElement te) {
        ...
        indexMembers(te,
            vmt.getVisibleMembers(ANNOTATION_TYPE_MEMBER_OPTIONAL));
        indexMembers(te,
            vmt.getVisibleMembers(ANNOTATION_TYPE_MEMBER_REQUIRED));
    ...
    private boolean shouldIndex(Element element) {
        ...
        if (utils.isPackage(element)) {
            ...
            // element is marked as deprecated or the containing package is
            // marked as
            // deprecated.
            return !(noDeprecated &&
    ...
    private Comparator<IndexItem> makeClassComparator() {
        return Comparator.comparing(IndexItem::getElement,
    ...
    private Comparator<IndexItem> makeIndexComparator() {
        ...
        // In order to produce consistent results, it is important that the base
        // comparator
        ...
        Comparator<IndexItem> baseComparator = (ii1, ii2) -> utils
            .compareStrings(getIndexItemKey(ii1), getIndexItemKey(ii2));
        Comparator<IndexItem> searchTagComparator = baseComparator
        ...
        return (ii1, ii2) -> {
            ...
            if (ii1.isElementItem() && ii2.isElementItem()) {
                int d = elementComparator.compare(ii1.getElement(),
                    ii2.getElement());
                if (d == 0) {
                    /*
            ...
            // If one is an element item, compare item keys; if equal, put
            // element item last
            if (ii1.isElementItem() || ii2.isElementItem()) {
    ...
    public Comparator<IndexItem> makeTypeSearchIndexComparator() {
        Comparator<IndexItem> simpleNameComparator = (ii1, ii2) -> utils
            .compareStrings(ii1.getSimpleName(), ii2.getSimpleName());
    ...
    public Comparator<IndexItem> makeGenericSearchIndexComparator() {
        Comparator<IndexItem> labelComparator = (ii1, ii2) -> utils
            .compareStrings(ii1.getLabel(), ii2.getLabel());
