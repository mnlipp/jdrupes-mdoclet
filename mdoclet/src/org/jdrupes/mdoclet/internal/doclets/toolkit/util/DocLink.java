...
public class DocLink {
    ...
    public DocLink relativizeAgainst(DocPath base) {
        ...
        // The following guards against the (ugly) use-case of using DocPath to
        // contain a URL
        ...
        // avoid generating an empty link by using the basename of the path if
        // necessary
    ...
    private boolean isAbsoluteURL(DocPath path) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            return (c == ':' && i + 2 < s.length() && s.charAt(i + 1) == '/'
                && s.charAt(i + 2) == '/');
        }
        return false;
    }
