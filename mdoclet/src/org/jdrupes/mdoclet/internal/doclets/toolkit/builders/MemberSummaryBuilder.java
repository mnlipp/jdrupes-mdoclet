...
import org.jdrupes.mdoclet.internal.doclets.toolkit.ClassWriter;
import org.jdrupes.mdoclet.internal.doclets.toolkit.Content;
import org.jdrupes.mdoclet.internal.doclets.toolkit.MemberSummaryWriter;
import org.jdrupes.mdoclet.internal.doclets.toolkit.WriterFactory;
...
public abstract class MemberSummaryBuilder extends AbstractMemberBuilder {
     ...
     */
    private final EnumMap<VisibleMemberTable.Kind,
            MemberSummaryWriter> memberSummaryWriters;
    ...
    public static MemberSummaryBuilder getInstance(
        ...
        MemberSummaryBuilder builder
            = new MemberSummaryBuilder(context, classWriter.getTypeElement()) {
                @Override
        ...
        for (VisibleMemberTable.Kind kind : VisibleMemberTable.Kind.values()) {
            MemberSummaryWriter msw
                = builder.getVisibleMemberTable().hasVisibleMembers(kind)
     ...
     * @see VisibleMemberTable
     */
    public MemberSummaryWriter
            getMemberSummaryWriter(VisibleMemberTable.Kind kind) {
     ...
     */
    protected void
            buildAnnotationTypeOptionalMemberSummary(Content summariesList) {
        MemberSummaryWriter writer
            = memberSummaryWriters.get(ANNOTATION_TYPE_MEMBER_OPTIONAL);
        addSummary(writer, ANNOTATION_TYPE_MEMBER_OPTIONAL, false,
            summariesList);
    }
     ...
     */
    protected void
            buildAnnotationTypeRequiredMemberSummary(Content summariesList) {
        MemberSummaryWriter writer
            = memberSummaryWriters.get(ANNOTATION_TYPE_MEMBER_REQUIRED);
        addSummary(writer, ANNOTATION_TYPE_MEMBER_REQUIRED, false,
            summariesList);
    }
    ...
    private void buildSummary(MemberSummaryWriter writer,
        ...
        SortedSet<? extends Element> members
            = asSortedSet(getVisibleMembers(kind));
        if (!members.isEmpty()) {
            for (Element member : members) {
                final Element property = pHelper.getPropertyElement(member);
                if (property != null
                    && member instanceof ExecutableElement ee) {
                    configuration.cmtUtils.updatePropertyMethodComment(ee,
                        property);
                }
                if (utils.isMethod(member)) {
                    var docFinder = utils.docFinder();
                    Optional<List<? extends DocTree>> r
                        = docFinder.search((ExecutableElement) member, (m -> {
                            var firstSentenceTrees
                                = utils.getFirstSentenceTrees(m);
                            Optional<List<? extends DocTree>> optional
                                = firstSentenceTrees.isEmpty()
                                    ? Optional.empty()
                                    : Optional.of(firstSentenceTrees);
                            return Result.fromOptional(optional);
                        })).toOptional();
                    // The fact that we use `member` for possibly unrelated tags
                    // is suspicious
                    writer.addMemberSummary(typeElement, member,
                        r.orElse(List.of()));
                } else {
                    writer.addMemberSummary(typeElement, member,
    ...
    private void buildInheritedSummary(MemberSummaryWriter writer,
        ...
        SortedSet<? extends Element> inheritedMembersFromMap
            = asSortedSet(visibleMemberTable.getAllVisibleMembers(kind));

        for (TypeElement inheritedClass : visibleMemberTable
            .getVisibleTypeElements()) {
            if (!(utils.isPublic(inheritedClass)
                || utils.isLinkable(inheritedClass))) {
                continue;
            }
            ...
            List<? extends Element> members = inheritedMembersFromMap.stream()
                .filter(e -> Objects.equals(utils.getEnclosingTypeElement(e),
                    inheritedClass))
                .toList();
            ...
            if (!members.isEmpty()) {
                ...
                Content inheritedHeader
                    = writer.getInheritedSummaryHeader(inheritedClass);
    ...
    private void addSummaryFootNote(TypeElement inheritedClass,
            Iterable<Element> inheritedMembers,
        ...
        for (var iterator = inheritedMembers.iterator(); iterator.hasNext();) {
            ...
            TypeElement t = utils.isUndocumentedEnclosure(inheritedClass)
                ? typeElement
                : inheritedClass;
            writer.addInheritedMemberSummary(t, member, isFirst,
                !iterator.hasNext(), links);
            isFirst = false;
        }
    }
    ...
    private void addSummary(MemberSummaryWriter writer,
        ...
        if (!summaryTreeList.isEmpty()) {
            Content member
                = writer.getMemberSummaryHeader(typeElement, summariesList);
            summaryTreeList.forEach(member::add);
            writer.addSummary(summariesList, member);
        }
    }

    private SortedSet<? extends Element>
            asSortedSet(Collection<? extends Element> members) {
    ...
    static class PropertyHelper {
        ...
        private void computeProperties() {
            ...
            List<ExecutableElement> props
                = ElementFilter.methodsIn(vmt.getVisibleMembers(PROPERTIES));
            for (ExecutableElement propertyMethod : props) {
                ExecutableElement getter
                    = vmt.getPropertyGetter(propertyMethod);
                ExecutableElement setter
                    = vmt.getPropertySetter(propertyMethod);
        ...
        private void addToPropertiesMap(ExecutableElement propertyMethod,
                ...
                ExecutableElement getter,
                ExecutableElement setter) {
            // determine the preferred element from which to derive the property
            // description
            ...
            if (e == field && builder.utils.hasDocCommentTree(propertyMethod)) {
                ...
                configuration.getReporter().print(Diagnostic.Kind.WARNING,
                    propertyMethod, configuration.getDocResources()
                        .getText("doclet.duplicate.comment.for.property"));
            }
        ...
        private void addToPropertiesMap(Element propertyMethod,
             ...
             * this case the comment is at the property method (not at the
             * field)
