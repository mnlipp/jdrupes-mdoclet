...
public class CommentUtils {
    ...
    public List<? extends DocTree>
            makePropertyDescriptionTree(List<? extends DocTree> content) {
    ...
    public ParamTree makeParamTree(Name name,
            List<? extends DocTree> description) {
        return treeFactory.newParamTree(false,
            treeFactory.newIdentifierTree(name), description);
    }
    ...
    public DocTree makeSeeTree(String sig, Element e) {
        return treeFactory
            .newSeeTree(List.of(treeFactory.newReferenceTree(sig)));
    }
    ...
    private List<DocTree> parse(String s) {
        ...
        while (m.find()) {
            ...
            if (m.start() > 0) {
                list.add(
                    treeFactory.newTextTree(s.substring(start, m.start())));
            }
            Name name = elementUtils.getName(m.group(2));
            list.add(m.group(1) == null
                ? treeFactory.newStartElementTree(name, List.of(),
                    m.group(3) != null)
        ...
        if (list == null) {
            ...
            if (start < s.length()) {
                list.add(
                    treeFactory.newTextTree(s.substring(start, s.length())));
            }
            return list;
        }
    }
    ...
    public void setEnumValuesTree(ExecutableElement ee) {
        ...
        fullBody.add(treeFactory
            .newTextTree(resources.getText("doclet.enum_values_doc.fullbody")));

        List<DocTree> descriptions = new ArrayList<>();
        descriptions.add(treeFactory
            .newTextTree(resources.getText("doclet.enum_values_doc.return")));
    ...
    public void setEnumValueOfTree(ExecutableElement ee) {
        List<DocTree> fullBody
            = parse(resources.getText("doclet.enum_valueof_doc.fullbody"));

        List<DocTree> tags = new ArrayList<>();

        List<DocTree> paramDescs = new ArrayList<>();
        paramDescs.add(treeFactory.newTextTree(
            resources.getText("doclet.enum_valueof_doc.param_name")));
        java.util.List<? extends VariableElement> parameters
            = ee.getParameters();
        VariableElement param = parameters.get(0);
        IdentifierTree id = treeFactory.newIdentifierTree(
            elementUtils.getName(param.getSimpleName().toString()));
        tags.add(treeFactory.newParamTree(false, id, paramDescs));

        List<DocTree> returnDescs = new ArrayList<>();
        returnDescs.add(treeFactory
            .newTextTree(resources.getText("doclet.enum_valueof_doc.return")));
        tags.add(treeFactory.newReturnTree(returnDescs));

        List<DocTree> throwsDescs = new ArrayList<>();
        throwsDescs.add(treeFactory.newTextTree(
            resources.getText("doclet.enum_valueof_doc.throws_ila")));

        ReferenceTree ref = treeFactory
            .newReferenceTree("java.lang.IllegalArgumentException");
        tags.add(treeFactory.newThrowsTree(ref, throwsDescs));

        throwsDescs = new ArrayList<>();
        throwsDescs.add(treeFactory.newTextTree(
            resources.getText("doclet.enum_valueof_doc.throws_npe")));
    ...
    public void setRecordConstructorTree(ExecutableElement ee) {
        ...
        List<DocTree> fullBody = makeDescriptionWithName(
            "doclet.record_constructor_doc.fullbody", te.getSimpleName());
        ...
        for (VariableElement param : ee.getParameters()) {
            ...
            tags.add(treeFactory.newParamTree(false, id,
                makeDescriptionWithComponent(
                    "doclet.record_constructor_doc.param_name", te, name)));
        }
    ...
    public void setRecordEqualsTree(ExecutableElement ee) {
        ...
        List<? extends RecordComponentElement> comps
            = ((TypeElement) ee.getEnclosingElement()).getRecordComponents();
        ...
        List<DocTree> paramDesc = makeDescriptionWithName(
            "doclet.record_equals_doc.param_name", paramName);
        DocTree paramTree = treeFactory.newParamTree(false, id, paramDesc);

        DocTree returnTree = treeFactory.newReturnTree(
            makeDescriptionWithName("doclet.record_equals_doc.return",
                paramName));

        TreePath treePath = utils.getTreePath(ee.getEnclosingElement());
        DocCommentTree docTree = treeFactory.newDocCommentTree(fullBody,
            List.of(paramTree, returnTree));
    ...
    public void setRecordHashCodeTree(ExecutableElement ee) {
        List<DocTree> fullBody = List
            .of(makeTextTreeForResource("doclet.record_hashCode_doc.fullbody"));

        DocTree returnTree = treeFactory.newReturnTree(
            List.of(
                makeTextTreeForResource("doclet.record_hashCode_doc.return")));

        DocCommentTree docTree
            = treeFactory.newDocCommentTree(fullBody, List.of(returnTree));
        dcInfoMap.put(ee, new DocCommentInfo(null, docTree));
    }
    ...
    public void setRecordAccessorTree(ExecutableElement ee) {
        ...
        List<DocTree> fullBody = makeDescriptionWithComponent(
            "doclet.record_accessor_doc.fullbody", te, ee.getSimpleName());

        DocTree returnTree = treeFactory.newReturnTree(
            makeDescriptionWithComponent("doclet.record_accessor_doc.return",
                te, ee.getSimpleName()));
    ...
    public void setRecordFieldTree(VariableElement ve) {
        ...
        List<DocTree> fullBody = makeDescriptionWithComponent(
            "doclet.record_field_doc.fullbody", te, ve.getSimpleName());

        DocCommentTree docTree
            = treeFactory.newDocCommentTree(fullBody, List.of());
    ...
    public void updatePropertyMethodComment(ExecutableElement member,
        ...
        final boolean isGetter
            = memberName.startsWith("get") || memberName.startsWith("is");
        ...
        if (isGetter || isSetter) {
            ...
            if (isGetter) {
                ...
                fullBody.addAll(
                    getComment("doclet.PropertyGetterWithName", propName));
            ...
            if (isSetter) {
                ...
                fullBody.addAll(
                    getComment("doclet.PropertySetterWithName", propName));
            ...
            // Set the @return
            DocTree propName
                = makeCodeTree(configuration.propertyUtils.getBaseName(member));
            List<? extends DocTree> returnTags
                = utils.getBlockTags(property, DocTree.Kind.RETURN);
        ...
        // copy certain tags
        List<? extends SinceTree> sinceTags
            = utils.getBlockTags(property, DocTree.Kind.SINCE, SinceTree.class);
        ...
        if (setter != null && setter != member) {
            ...
            if (!utils.isTypeVariable(param.asType())) {
                sb.append("(")
                    .append(utils.getTypeSignature(param.asType(), false, true))
                    .append(")");
            }
     ...
     */
    private List<DocTree> makeDescriptionWithComponent(String key,
            TypeElement elem, Name component) {
        ...
        if (hasParamForComponent(elem, component)) {
            ...
            result
                .add(treeFactory.newStartElementTree(A, List.of(href), false));
            result.addAll(code);
     ...
     */
    public List<? extends DocTree> getComment(String key, Object o0, Object o1,
            Object o2) {
    ...
    /*
     * Returns the TreePath/DocCommentTree info that has been generated for an
     * element.
     * 
     * @param e the element
     * 
     ...
     */
    // "synthetic" is not the best word here, and should not be confused with
    // synthetic elements
    ...
    public DocCommentInfo setDocCommentTree(Element element,
            List<? extends DocTree> fullBody,
            List<? extends DocTree> blockTags) {
        DocCommentTree docTree
            = treeFactory.newDocCommentTree(fullBody, blockTags);
        return setDocCommentInfo(element, new DocCommentInfo(null, docTree));
    }

    public DocCommentInfo setDocCommentInfo(Element element,
            DocCommentInfo dci) {
        DocCommentInfo prev = dcInfoMap.put(element, dci);
        // A method having null comment (no comment) that might need to be
        // replaced
