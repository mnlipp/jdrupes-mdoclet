...
public class BaseTaglet implements Taglet {
    ...
    BaseTaglet(String name, boolean inline, Set<Location> sites) {
        this(name, inline ? DocTree.Kind.UNKNOWN_INLINE_TAG
            : DocTree.Kind.UNKNOWN_BLOCK_TAG, inline, sites);
    }

    private BaseTaglet(String name, DocTree.Kind tagKind, boolean inline,
            Set<Location> sites) {
        this.name = name;
    ...
    public Content getInlineTagOutput(Element element, DocTree tag,
            TagletWriter writer) {
        throw new UnsupportedTagletOperationException(
