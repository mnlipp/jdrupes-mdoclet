...
public class WorkArounds {
    ...
    public boolean isDeprecated0(Element e) {
        ...
        for (AnnotationMirror anno : e.getAnnotationMirrors()) {
            if (typeUtils.isSameType(
                anno.getAnnotationType().asElement().asType(), deprecatedType))
                return true;
        }
        return false;
    }
    ...
    public TypeElement searchClass(TypeElement klass, String className) {
        ...
        te = utils.findClassInPackageElement(utils.containingPackage(klass),
            className);
        if (te != null) {
            return te;
        }
    ...
    // ------------------Start of Serializable
    // Implementation---------------------//
    private final Map<TypeElement, NewSerializedForm> serializedForms
        = new HashMap<>();

    private NewSerializedForm getSerializedForm(TypeElement typeElem) {
        return serializedForms.computeIfAbsent(typeElem,
            te -> new NewSerializedForm(utils,
                configuration.docEnv.getElementUtils(), te));
    }

    public SortedSet<VariableElement>
            getSerializableFields(TypeElement typeElem) {
        return getSerializedForm(typeElem).fields;
    }

    public SortedSet<ExecutableElement>
            getSerializationMethods(TypeElement typeElem) {
        return getSerializedForm(typeElem).methods;
    }

    public boolean definesSerializableFields(TypeElement typeElem) {
        if (!utils.isSerializable(typeElem)
            || utils.isExternalizable(typeElem)) {
            return false;
        } else {
    ...
    static class NewSerializedForm {
        ...
        NewSerializedForm(Utils utils, Elements elements, TypeElement te) {
            ...
            methods = new TreeSet<>(
                utils.comparators.makeGeneralPurposeComparator());
            fields = new TreeSet<>(
                utils.comparators.makeGeneralPurposeComparator());
            if (utils.isExternalizable(te)) {
                ...
                ExecutableElement md = findMethod(te, "readExternal",
                    Arrays.asList(readExternalParamArr));
                if (md != null) {
                    methods.add(md);
                }
                md = findMethod(te, "writeExternal",
                    Arrays.asList(writeExternalParamArr));
                if (md != null) {
                    methods.add(md);
                }
                ...
                if (dsf != null) {
                    /*
                     * Define serializable fields with array of
                     * ObjectStreamField.
        ...
        private VarSymbol getDefinedSerializableFields(ClassSymbol def) {
             ...
             */
            for (Symbol sym : def.members()
                .getSymbolsByName(names.fromString(SERIALIZABLE_FIELDS))) {
                if (sym.kind == VAR) {
        ...
        private void addMethodIfExist(ClassSymbol def, String methodName) {
            ...
            for (Symbol sym : def.members()
                .getSymbolsByName(names.fromString(methodName))) {
                if (sym.kind == MTH) {
                    MethodSymbol md = (MethodSymbol) sym;
                    if ((md.flags() & Flags.STATIC) == 0) {
                        /*
                         * WARNING: not robust if unqualifiedMethodName is
                         * overloaded
                         * method. Signature checking could make more robust.
                         * READOBJECT takes a single parameter,
                         * java.io.ObjectInputStream.
                         * WRITEOBJECT takes a single parameter,
        ...
        public ExecutableElement findMethod(TypeElement te, String methodName,
            ...
            List<? extends Element> allMembers
                = this.elements.getAllMembers(te);
            loop: for (Element e : allMembers) {
                if (e.getKind() != METHOD) {
                    continue;
                }
    ...
    public Map<? extends ExecutableElement, ? extends AnnotationValue>
        ...
        TypeElement featureType = elementUtils
            .getTypeElement("jdk.internal.javac.PreviewFeature.Feature");
        TypeElement jepType = elementUtils
            .getTypeElement("jdk.internal.javac.PreviewFeature.JEP");
        var featureVar = featureType.getEnclosedElements().stream()
            .filter(e -> feature.equals(e.getSimpleName().toString()))
            .findFirst();
        if (featureVar.isPresent()) {
            for (AnnotationMirror anno : featureVar.get()
