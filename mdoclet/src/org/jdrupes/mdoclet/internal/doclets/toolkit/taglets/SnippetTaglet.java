...
import org.jdrupes.mdoclet.internal.doclets.toolkit.taglets.snippet.Action;
import org.jdrupes.mdoclet.internal.doclets.toolkit.taglets.snippet.ParseException;
import org.jdrupes.mdoclet.internal.doclets.toolkit.taglets.snippet.Parser;
...
public class SnippetTaglet extends BaseTaglet {
    ...
    public Content getInlineTagOutput(Element holder, DocTree tag,
        ...
        try {
            ...
            String details = writer.configuration().getDocResources()
                .getText(e.key(), e.args());
    ...
    private Content generateContent(Element holder, DocTree tag,
        ...
        // organize snippet attributes in a map, performing basic checks along
        // the way
        ...
        if (region != null) {
            ...
            if (regionName.isBlank()) {
                throw new BadSnippetException(region,
        ...
        if (containsFile || containsClass) {
            ...
            String v = containsFile
                ...
                : stringValueOf((a = attributes.get(CLASS))).replace(".", "/")
                    + ".java";

            if (v.isBlank()) {
                throw new BadSnippetException(a,
                    "doclet.tag.attribute.value.illegal",
                    containsFile ? FILE : CLASS, v);
            }

            // we didn't create JavaFileManager, so we won't close it; even if
            // an error occurs
            ...
            try {
                ...
                var pkgName = pkg.getQualifiedName().toString(); // note: empty
                                                                 // string for
                                                                 // unnamed
                                                                 // package
                var relativeName = "snippet-files/" + v;
                fileObject = fileManager.getFileForInput(pkgLocation, pkgName,
                    relativeName);

                // if not found in local snippet-files directory, look on
                // snippet path
                if (fileObject == null
                    && fileManager.hasLocation(Location.SNIPPET_PATH)) {
                    fileObject = fileManager
                        .getFileForInput(Location.SNIPPET_PATH, "", v);
                }
            } catch (IOException | IllegalArgumentException e) { // TODO: test
                                                                 // this when
                                                                 // JDK-8276892
                                                                 // is
                                                                 // integrated
                // JavaFileManager.getFileForInput can throw
                // IllegalArgumentException in certain cases
            ...
            if (fileObject == null) {
                ...
                throw new BadSnippetException(a,
                    "doclet.snippet_file_not_found", v);
            }

            try {
                externalContent = fileObject.getCharContent(true).toString();
            } catch (IOException e) {  // TODO: test this when JDK-8276892 is
                                       // integrated
                throw new BadSnippetException(a, "doclet.exception.read.file",
        ...
        try {
            Diags d = (text, pos) -> {
                ...
                writer.configuration().getReporter().print(
                    Diagnostic.Kind.WARNING,
                    path, pos, pos, pos, text);
            };
            if (inlineContent != null) {
                inlineSnippet = parse(writer.configuration().getDocResources(),
                    d, language, inlineContent);
            }
        } catch (ParseException e) {
            var path = writer.configuration().utils.getCommentHelper(holder)
                .getDocTreePath(snippetTag.getBody());
            // TODO: there should be a method in Messages; that method should
            // mirror Reporter's; use that method instead accessing Reporter.
        ...
        try {
            ...
            Diags d = (text, pos) -> writer.configuration().getMessages()
                .warning(finalFileObject, pos, pos, pos, text);
            if (externalContent != null) {
                externalSnippet
                    = parse(writer.configuration().getDocResources(), d,
                        language, externalContent);
            }
        } catch (ParseException e) {
            assert fileObject != null;
            writer.configuration().getMessages().error(fileObject,
                e.getPosition(),
                e.getPosition(), e.getPosition(), "doclet.snippet.markup",
                e.getMessage());
            return badSnippet(writer, Optional.of(e.getMessage()));
        }
        ...
        if (regionName != null) {
            ...
            if (r1 == null && r2 == null) {
                throw new BadSnippetException(tag,
                    "doclet.snippet.region.not_found", regionName);
            }
        }
        ...
        if (inlineSnippet != null && externalSnippet != null) {
            ...
            if (!Objects.equals(inlineStr, externalStr)) {
                throw new BadSnippetException(tag,
                    ...
                    diff(inlineStr, externalStr));
            }
        }

        assert inlineSnippet != null || externalSnippet != null;
        StyledText text
            = inlineSnippet != null ? inlineSnippet : externalSnippet;
    ...
    private StyledText parse(Resources resources, Diags diags,
            Optional<Language> language, String content) throws ParseException {
        Parser.Result result
            = new Parser(resources).parse(diags, language, content);
    ...
    private static String stringValueOf(AttributeTree at)
        ...
        if (at.getValueKind() == AttributeTree.ValueKind.EMPTY) {
            throw new BadSnippetException(at,
    ...
    private String languageFromFileName(String fileName) {
        // TODO: find a way to extend/customize the list of recognized file name
        // extensions
    ...
    private void error(TagletWriter writer, Element holder, DocTree tag,
    ...
    private Content badSnippet(TagletWriter writer, Optional<String> details) {
        ...
        return writer.invalidTagOutput(
            resources.getText("doclet.tag.invalid", "snippet"), details);
    }
    ...
    private static StyledText toDisplayForm(StyledText source) {
        ...
        boolean endsWithLineFeed = !sourceString.isEmpty()
            && sourceString.charAt(source.length() - 1) == '\n';
        while (originalLines.hasNext() && unindentedLines.hasNext()) { // [^1]
            ...
            result.append(
                source.subText(pos + idx, pos + idx + unindentedLine.length()));
