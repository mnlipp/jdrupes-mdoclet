...
public class StyledText {
     ...
     */
    private void replace(int start, int end, Set<? extends Style> styles,
    ...
    /*
     * Updates the text given the scope of the change to reflect text
     * continuity.
     */
    private void update(int start, int end, int newLength, SubText text) {
        ...
        if (end <= text.start) { // the change is on the left-hand side of the
                                 // text
            int diff = newLength - (end - start);
            text.start += diff;
            text.end += diff;
        } else if (text.end <= start) { // the change is on the right-hand side
                                        // of the text
            // no-op; explicit "if" for clarity
        } else { // the change intersects with the text
            if (text.start <= start && end <= text.end) { // the change is
                                                          // within the text
                text.end += newLength - (end - start);
            } else {
                int intersectionLen
                    = min(end, text.end) - max(start, text.start);
    ...
    private void addStyle(int start, int end,
            Set<? extends Style> additionalStyles) {
    ...
    public StyledText append(Set<? extends Style> styles,
    ...
    public StyledText append(StyledText fragment) {
        fragment.consumeBy((style, sequence) -> subText(length(), length())
            .replace(style, sequence));
        return this;
    }
    ...
    private static final class Styles {
        ...
        private void add(int fromIndex, int toIndex,
                Set<? extends Style> additional) {
            Set<Style> copyOfAdditional = Set.copyOf(additional);
            list.subList(fromIndex, toIndex)
                .replaceAll(current -> sum(current, copyOfAdditional));
        }
        ...
        private void consumeBy(StyledText.Consumer consumer, CharSequence seq,
                int start, int end) {
            if (start == end) {
    ...
    final class SubText extends StyledText {
        ...
        @Override
        public void replace(Set<? extends Style> styles,
                CharSequence plaintext) {
        ...
        @Override
        public StyledText subText(int start, int end) {
            return StyledText.this.subText(this.start + start,
                this.start + end);
        }
