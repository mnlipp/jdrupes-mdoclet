...
public class DocFinder {
    ...
    private final Function<ExecutableElement,
            ExecutableElement> overriddenMethodLookup;
    private final BiFunction<ExecutableElement, ExecutableElement,
            Iterable<ExecutableElement>> implementedMethodsLookup;

    DocFinder(
            Function<ExecutableElement,
                    ExecutableElement> overriddenMethodLookup,
            BiFunction<ExecutableElement, ExecutableElement,
                    Iterable<ExecutableElement>> implementedMethodsLookup) {
    ...
    public <T, X extends Throwable> Result<T> trySearch(
     ...
     *
     * Depending on how it is instructed, the search begins from either the
     * given
    ...
    private <T, X extends Throwable> Result<T> search0(ExecutableElement method,
            ...
            Criterion<T, X> criterion)
            throws NoOverriddenMethodsFound, X {
        // if the "overrides" check is requested and does not pass, throw the
        // exception
        // first so that it trumps the result that the search would otherwise
        // had
        ...
        Result<T> r = includeMethodInSearch ? criterion.apply(method)
            : Result.CONTINUE();
        ...
        while (methods.hasNext()) {
            ...
            r = search0(m, true, false /* don't check for overrides */,
                criterion);
    ...
    private Iterator<ExecutableElement>
            methodsOverriddenBy(ExecutableElement method) {
    ...
    private record Concluded<T>(T value)
            implements DocFinder.Result.Conclude<T> {
