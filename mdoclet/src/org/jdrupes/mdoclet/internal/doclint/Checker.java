...
public class Checker extends DocTreePathScanner<Void, Void> {
    ...
    private final Deque<TagStackItem> tagStack; // TODO: maybe want to record
    ...
    public Void visitStartElement(StartElementTree tree, Void ignore) {
        ...
        if (t == null) {
            ...
            // tag specific checks
            switch (t) {
            // check for out of sequence headings, such as <h1>...</h1>
            // <h3>...</h3>
    ...
    private void checkStructure(StartElementTree tree, HtmlTag t) {
        ...
        case OTHER -> {
            switch (t) {
            case SCRIPT -> {
                // <script> may or may not be allowed, depending on
                // --allow-script-in-comments,
                // but we allow it here, and rely on a separate scanner to
                // detect all uses
                // of JavaScript, including <script> tags, and use in
                // attributes, etc.
            }

            default -> env.messages.error(HTML, tree, "dc.tag.not.allowed",
                treeName);
            }
            return;
        }
    ...
    private void checkHeading(StartElementTree tree, HtmlTag tag) {
        ...
        if (getHeadingRank(tag) > getHeadingRank(currHeadingTag) + 1) {
            if (currHeadingTag == null) {
                env.messages.error(ACCESSIBILITY, tree,
                    "dc.tag.heading.sequence.1",
                    tag, implicitHeadingRank);
            } else {
                env.messages.error(ACCESSIBILITY, tree,
                    "dc.tag.heading.sequence.2",
                    tag, currHeadingTag);
            }
    ...
    public Void visitEndElement(EndElementTree tree, Void ignore) {
        ...
        if (t == null) {
            ...
            env.messages.error(HTML, tree, "dc.tag.end.not.permitted",
                treeName);
        } else {
            boolean done = false;
            while (!tagStack.isEmpty()) {
                ...
                if (t == top.tag) {
                    ...
                    case SECTION, ARTICLE -> {
                        if (!top.flags.contains(Flag.HAS_HEADING)) {
                            env.messages.error(HTML, tree,
                                "dc.tag.requires.heading", treeName);
                        }
                    }
                    }
                    ...
                    break;
                } else if (top.tag == null
                    || top.tag.endKind != HtmlTag.EndKind.REQUIRED) {
                    ...
                    if (found
                        && top.tree.getKind() == DocTree.Kind.START_ELEMENT) {
                        env.messages.error(HTML, top.tree,
                            "dc.tag.start.unmatched",
                            ((StartElementTree) top.tree).getName());
                        tagStack.pop();
                    } else {
                        env.messages.error(HTML, tree, "dc.tag.end.unexpected",
                            treeName);
                        done = true;
                        break;
                    }
    ...
    public Void visitAttribute(AttributeTree tree, Void ignore) {
        ...
        if (currTag != null && currTag.elemKind != ElemKind.HTML4) {
            ...
            // for now, doclint allows all attribute names beginning with "on"
            // as event handler names,
            ...
            if (!name.toString().startsWith("on")) {
                ...
                case OBSOLETE -> env.messages.warning(HTML, tree,
                    "dc.attr.obsolete", name);

                case HTML4 -> env.messages.error(HTML, tree,
                    "dc.attr.not.supported.html5", name);

                case INVALID -> env.messages.error(HTML, tree,
                    "dc.attr.unknown", name);
                }
            }
            ...
            if (attr != null) {
                ...
                case ID -> {
                    ...
                    if (value == null) {
                        ...
                        if (!validId.matcher(value).matches()) {
                            env.messages.error(HTML, tree, "dc.invalid.anchor",
                                value);
                        }
                        if (!checkAnchor(value)) {
                            env.messages.error(HTML, tree,
                                "dc.anchor.already.defined", value);
                        }
                    }
                }
                ...
                case BORDER -> {
                    if (currTag == HtmlTag.TABLE) {
                        ...
                        try {
                            if (v == null
                                || (!v.isEmpty() && Integer.parseInt(v) != 1)) {
                                env.messages.error(HTML, tree,
                        ...
                        try {
                            if (v == null
                                || (!v.isEmpty() && Integer.parseInt(v) != 0)) {
                                env.messages.error(HTML, tree,
                ...
                case ROLE -> {
                    if (currTag == HtmlTag.TABLE) {
                        ...
                        if (Objects.equals(v, "presentation")) {
                            tagStack.peek().flags
                                .add(Flag.TABLE_IS_PRESENTATION);
                        }
                    }
                }
                }
    ...
    private boolean checkAnchor(String name) {
        ...
        return e == null
            || foundAnchors.computeIfAbsent(e, k -> new HashSet<>()).add(name);
    }
    ...
    private static final Pattern docRoot
    ...
    private void checkURI(AttributeTree tree, String uri) {
        // allow URIs beginning with javascript:, which would otherwise be
        // rejected by the URI API.
    ...
    public Void visitIndex(IndexTree tree, Void ignore) {
        markEnclosingTag(Flag.HAS_INLINE_TAG);
        if (inIndex) {
            env.messages.warning(HTML, tree, "dc.tag.nested.tag",
                "@" + tree.getTagName());
        }
        for (TagStackItem tsi : tagStack) {
    ...
    public Void visitParam(ParamTree tree, Void ignore) {
        ...
        Element paramElement = nameTree != null
            ? env.trees.getElement(new DocTreePath(getCurrentPath(), nameTree))
            : null;
        ...
        if (paramElement == null) {
            ...
            case CLASS, INTERFACE -> {
                if (typaram) {
                    env.messages.error(REFERENCE, nameTree,
                        "dc.param.name.not.found");
                } else {
                    env.messages.error(REFERENCE, tree, "dc.invalid.param");
                }
            }

            case METHOD, CONSTRUCTOR, RECORD -> env.messages.error(REFERENCE,
                nameTree, "dc.param.name.not.found");

            default -> env.messages.error(REFERENCE, tree, "dc.invalid.param");
            }
        } else {
            boolean unique = foundParams.add(paramElement);

            if (!unique) {
                env.messages.warning(REFERENCE, tree, "dc.exists.param",
                    nameTree);
            }
        }
    ...
    public Void visitProvides(ProvidesTree tree, Void ignore) {
        ...
        Element se = env.trees
            .getElement(new DocTreePath(getCurrentPath(), serviceType));
        if (se == null) {
        ...
        if (!env.strictReferenceChecks) {
            ...
            if (sep > 0) {
                ...
                if (SourceVersion.isName(moduleName)) {
                    ...
                    if (m == null) {
                        env.messages.warning(REFERENCE, tree,
                            "dc.ref.in.missing.module", moduleName);
                        return;
                    }
                }
            }
    ...
    public Void visitReturn(ReturnTree tree, Void ignore) {
        ...
        if (tree.isInline()) {
            ...
            if (dct.getFirstSentence().isEmpty()
                || tree != dct.getFirstSentence().get(0)) {
    ...
    public Void visitSummary(SummaryTree tree, Void aVoid) {
        ...
        if ((idx == 1 && hasNonWhitespaceText) || idx > 1) {
            env.messages.warning(SYNTAX, tree, "dc.invalid.summary",
                tree.getTagName());
        }
    ...
    public Void visitThrows(ThrowsTree tree, Void ignore) {
        ...
        Element ex
            = env.trees.getElement(new DocTreePath(getCurrentPath(), exName));
        if (ex == null) {
            env.messages.error(REFERENCE, tree, "dc.ref.not.found");
            ...
            case CONSTRUCTOR, METHOD -> {
                if (isCheckedException(ex.asType())) {
                    ExecutableElement ee = (ExecutableElement) env.currElement;
                    checkThrowsDeclared(exName, ex.asType(),
                        ee.getThrownTypes());
                }
            }

            default -> env.messages.error(REFERENCE, tree, "dc.invalid.throws");
            }
        } else {
    ...
    private boolean isThrowable(TypeMirror tm) {
        ...
        case DECLARED, TYPEVAR -> env.types.isAssignable(tm,
            env.java_lang_Throwable);
        default -> false;
        };
    }

    private void checkThrowsDeclared(ReferenceTree tree, TypeMirror t,
    ...
    public Void visitValue(ValueTree tree, Void ignore) {
        ...
        if (ref == null || ref.getSignature().isEmpty()) {
            ...
            Element e
                = env.trees.getElement(new DocTreePath(getCurrentPath(), ref));
            if (!isConstant(e))
    ...
    private boolean isConstant(Element e) {
        if (e != null && e.getKind() == ElementKind.FIELD) {
            ...
            return (value != null); // can't distinguish "not a constant" from
                                    // "constant is null"
        } else {
            return false;
        }
    ...
    public Void visitErroneous(ErroneousTree tree, Void ignore) {
        env.messages.error(SYNTAX, tree, null,
            tree.getDiagnostic().getMessage(null));
        return null;
    }
    // </editor-fold>
    ...
    private boolean isSynthetic() {
        return env.elements
            .getOrigin(env.currElement) == Elements.Origin.SYNTHETIC;
    }
    ...
    private boolean isRecordComponentOrField() {
        return env.currElement.getKind() == ElementKind.RECORD_COMPONENT
            || env.currElement.getEnclosingElement() != null
                && env.currElement.getEnclosingElement()
    ...
    void toString(TreePath p, StringBuilder sb) {
        ...
        sb.append(p.getLeaf().getKind()).append(":").append(env.getPos(p))
            .append(":S").append(env.getStartPos(p));
    }
