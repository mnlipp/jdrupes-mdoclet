...
import org.jdrupes.mdoclet.internal.doclint.Env.AccessKind;
...
public class Messages {
    ...
    protected void report(Group group, Diagnostic.Kind dkind, DocTree tree,
            String code, Object... args) {
        if (options.isEnabled(group, env.currAccess)) {
            if (dkind == Diagnostic.Kind.WARNING
                && env.suppressWarnings(group)) {
                return;
            }
            String msg
                = (code == null) ? (String) args[0] : localize(code, args);
    ...
    protected void report(Group group, Diagnostic.Kind dkind, Tree tree,
            String code, Object... args) {
        if (options.isEnabled(group, env.currAccess)) {
            if (dkind == Diagnostic.Kind.WARNING
                && env.suppressWarnings(group)) {
            ...
            env.trees.printMessage(dkind, msg, tree,
                env.currPath.getCompilationUnit());
    ...
    static class Options {
        ...
        private void setOption(String arg) throws IllegalArgumentException {
            ...
            int sep = arg.indexOf("/");
            if (sep > 0) {
                Env.AccessKind ak = Env.AccessKind
                    .valueOf(StringUtils.toUpperCase(arg.substring(sep + 1)));
        ...
        private void setOption(String opt, Env.AccessKind ak) {
            map.put(opt, (ak != null) ? ak
                : opt.startsWith("-") ? Env.AccessKind.PUBLIC
                    : Env.AccessKind.PRIVATE);
        }
    ...
    static class Stats {
        ...
        void report(PrintWriter out) {
            ...
            for (Diagnostic.Kind k : Diagnostic.Kind.values()) {
                dkindTable.put(StringUtils.toLowerCase(k.toString()),
                    dkindCounts[k.ordinal()]);
            }
        ...
        private static class Table {
            ...
            private static final Comparator<Integer> DECREASING
                = Comparator.reverseOrder();
            private final TreeMap<Integer, Set<String>> map
                = new TreeMap<>(DECREASING);
