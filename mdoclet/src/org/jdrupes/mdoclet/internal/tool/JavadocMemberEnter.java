...
public class JavadocMemberEnter extends MemberEnter {
    ...
    public static void preRegister(Context context) {
        context.put(memberEnterKey,
            (Context.Factory<MemberEnter>) JavadocMemberEnter::new);
    }
    ...
    public void visitMethodDef(JCMethodDecl tree) {
        ...
        if (meth == null || meth.kind != MTH)
            return;
        TreePath treePath
            = toolEnv.getTreePath(env.toplevel, env.enclClass, tree);
        ...
        // release resources
        // handle constructors for record types specially, because of downstream
        // checks
        if ((env.enclClass.mods.flags & Flags.RECORD) != 0
    ...
    public void visitVarDef(JCVariableDecl tree) {
        ...
        if (tree.sym != null && tree.sym.kind == VAR
            ...
            toolEnv.setElementToTreePath(tree.sym,
                toolEnv.getTreePath(env.toplevel, env.enclClass, tree));
        }
    }
    ...
    private static boolean containsNonConstantExpression(JCExpression tree) {
        return new MaybeConstantExpressionScanner()
            .containsNonConstantExpression(tree);
    }
