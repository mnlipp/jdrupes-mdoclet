...
public class JavadocEnter extends Enter {
    ...
    public void visitTopLevel(JCCompilationUnit tree) {
        ...
        if (tree.sourcefile.isNameCompatible("package-info",
            JavaFileObject.Kind.SOURCE)) {
            JCPackageDecl pd = tree.getPackage();
            TreePath tp = pd == null ? toolEnv.getTreePath(tree)
                : toolEnv.getTreePath(tree, pd);
    ...
    public void visitClassDef(JCClassDecl tree) {
        ...
        if (tree.sym.kind == TYP || tree.sym.kind == ERR) {
            ...
            toolEnv.setElementToTreePath(c,
                toolEnv.getTreePath(env.toplevel, tree));
