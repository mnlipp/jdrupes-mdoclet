...
public class JavadocLog extends Log implements Reporter {
     ...
     */
    private final LinkedHashMap<JavaFileObject,
            SoftReference<DiagnosticSource>> diagSourceCache;
    ...
    private static PrintWriter createPrintWriter(PrintStream ps,
            boolean autoflush) {
    ...
    public JavadocLog(Context context, String programName) {
        // use the current values of System.out, System.err, in case they have
        // been redirected
    ...
    public JavadocLog(Context context, String programName,
        ...
        messages.add(locale -> ResourceBundle.getBundle(
            "org.jdrupes.mdoclet.internal.tool.resources.javadoc",
            locale));
        ...
        diagSourceCache = new LinkedHashMap<>() {
            ...
            @Override
            protected boolean removeEldestEntry(Map.Entry<JavaFileObject,
                    SoftReference<DiagnosticSource>> eldest) {
    ...
    @Override // Reporter
    public void print(Diagnostic.Kind kind, DocTreePath path, int start,
            int pos, int end, String message) {
        ...
        DCTree.DCDocComment docComment
            = (DCTree.DCDocComment) path.getDocComment();
        DCTree docTree = (DCTree) path.getLeaf();
        // note: it is important to evaluate the offsets in the context of the
        // position
        // within the comment text, and not in the context of the overall source
        // text
        ...
        DiagnosticPosition dp
            = createDiagnosticPosition(null, sStart, sPos, sEnd);
    ...
    public void print(Kind kind, FileObject file, int start, int pos, int end,
        ...
        DiagnosticType dt = getDiagnosticType(kind);
        Set<DiagnosticFlag> flags = getDiagnosticFlags(kind);
        // Although not required to do so, it is the case that any file object
        // returned from the
        // javac impl of JavaFileManager will return an object that implements
        // JavaFileObject.
        // See PathFileObject, which provides the primary impls of
        // (Java)FileObject.
        JavaFileObject fo = file instanceof JavaFileObject _fo ? _fo
            : new WrappingJavaFileObject(file);
    ...
    public void printError(DocTreePath path, String message) {
        DiagnosticSource ds = getDiagnosticSource(path);
        DiagnosticPosition dp = getDiagnosticPosition(path);
        report(DiagnosticType.ERROR, EnumSet.noneOf(DiagnosticFlag.class), ds,
            dp, message);
    }
    ...
    public void printWarning(DocTreePath path, String message) {
        DiagnosticSource ds = getDiagnosticSource(path);
        DiagnosticPosition dp = getDiagnosticPosition(path);
        report(DiagnosticType.WARNING, EnumSet.noneOf(DiagnosticFlag.class), ds,
            dp, message);
    }
     ...
     * @param args    the arguments for the message
     */
    public void printWarningUsingKey(Element element, String key,
            Object... args) {
    ...
    public void printNote(String message) {
        // Ideally, for consistency with errors and warnings, we would use the
        // following:
        // report(Kind.NOTE, null, null, message);
        // but the default formatting in Log for Kind.NOTE is to prefix the line
        // with "Note:"
        // which is undesirable and inconsistent with existing javadoc output.
        // For now, to avoid the prefix, we write directly to the underlying
        // stream.
     ...
     * @param message the message
     */
    private void report(Diagnostic.Kind kind, DiagnosticSource ds,
            DiagnosticPosition dp, String message) {
        report(getDiagnosticType(kind), getDiagnosticFlags(kind), ds, dp,
            message);
    }
     ...
     * @param message the message
     */
    private void report(DiagnosticType dt, DiagnosticSource ds,
            DiagnosticPosition dp, String message) {
     ...
     * @param message the message
     */
    private void report(DiagnosticType dt, Set<DiagnosticFlag> flags,
            DiagnosticSource ds, DiagnosticPosition dp, String message) {
        report(
            javadocDiags.create(dt, null, flags, ds, dp, "message", message));
    }
     ...
     * @return the diagnostic position
     */
    private DiagnosticPosition createDiagnosticPosition(JCTree tree, int start,
            int pos, int end) {
    ...
    private DiagnosticSource getDiagnosticSource(DocTreePath path) {
        return getDiagnosticSource(
            path.getTreePath().getCompilationUnit().getSourceFile());
    }
