...
import org.jdrupes.mdoclet.internal.Versions;
import org.jdrupes.mdoclet.internal.tool.Main.Result;
import org.jdrupes.mdoclet.internal.tool.ToolOptions.ToolOption;
...
public class Start {
    ...
    private ToolOptions getToolOptions() {
        ToolOptions.ShowHelper helper = new ToolOptions.ShowHelper() {
            @Override
            public void usage() {
                showUsage("main.usage", ToolOption.Kind.STANDARD,
                    "main.usage.foot");
            }

            @Override
            public void Xusage() {
                showUsage("main.Xusage", ToolOption.Kind.EXTENDED,
                    "main.Xusage.foot");
            }

            @Override
            public void version() {
                showVersion("javadoc.version", orDefault(
                    () -> Versions.shortVersionStringOf(toolVersion())));
            }

            @Override
            public void fullVersion() {
                showVersion("javadoc.fullversion", orDefault(
                    () -> Versions.fullVersionStringOf(toolVersion())));
            }
    ...
    private void showUsage(String headerKey, ToolOption.Kind kind,
            String footerKey) {
    ...
    private void showDocletOptions(Option.Kind kind) {
        ...
        var comp = new Comparator<Doclet.Option>() {
            ...
            public int compare(Doclet.Option o1, Doclet.Option o2) {
                return collator.compare(o1.getNames().get(0),
                    o2.getNames().get(0));
            }
        };
    ...
    private static final String COMPACT_FORMAT
        = SMALL_INDENT + "%-" + DEFAULT_SYNOPSIS_WIDTH + "s %s";

    void showOption(List<String> names, String parameters, String description) {
        String synopses = names.stream()
            .map(s -> s + parameters)
            .collect(Collectors.joining(", "));
        // If option synopses and description fit on a single line of reasonable
        // length,
        // display using COMPACT_FORMAT
        if (synopses.length() < DEFAULT_SYNOPSIS_WIDTH
            && !description.contains("\n")
            && (SMALL_INDENT.length() + DEFAULT_SYNOPSIS_WIDTH + 1
                + description.length() <= DEFAULT_MAX_LINE_LENGTH)) {
            showLines(String.format(COMPACT_FORMAT, synopses, description));
            return;
        }

        // If option synopses fit on a single line of reasonable length, show
        // that;
        ...
        showLines(
            LARGE_INDENT + description.replace("\n", "\n" + LARGE_INDENT));
    }
    ...
    private Result begin(List<String> options,
            Iterable<? extends JavaFileObject> fileObjects) {
        ...
        // Perform an initial scan of the options to determine the doclet to be
        // used (if any),
        ...
        try {
            ...
            // The detail message from javac already includes a localized
            // "error: " prefix,
            // so print the message directly.
            // It would be even better to rethrow this as
            ...
            // simply rethrow these exceptions, to be caught and handled by
            // JavadocTaskImpl
            throw e;
     ...
     */
    private Result parseAndExecute(List<String> argList,
            Iterable<? extends JavaFileObject> fileObjects)
        ...
        try {
            options.processCompilerOption(
                com.sun.tools.javac.main.Option.XLINT_CUSTOM,
                "-Xlint:-options");
        ...
        if (!success && beforeCount == afterCount) { // if there were failures
        ...
        if (fileManager instanceof BaseFileManager bfm) {
            ...
            for (Map.Entry<com.sun.tools.javac.main.Option, String> e : options
                .fileManagerOptions().entrySet()) {
                String optName = e.getKey().getPrimaryName();
                String optValue = e.getValue();
                try {
                    if (!fileManager.handleOption(optName,
                        List.of(optValue).iterator())) {
                        log.error("main.unknown.option.for.filemanager",
                            optName);
                    }
                } catch (IllegalArgumentException ex) {
                    log.error("main.bad.arg.for.filemanager.option", optName,
                        ex.getMessage());
                }
            }
        }
            ...
            if (options.subpackages().isEmpty()) {
                if (javaNames.isEmpty() && isEmpty(fileObjects)) {
                    String text = log.getText(
                        "main.No_modules_packages_or_classes_specified");
        ...
        DocletEnvironment docEnv
            = comp.getEnvironment(options, javaNames, fileObjects);

        // release resources
        comp = null;

        if (options.breakIterator()
    ...
    boolean matches(List<String> names, String arg) {
        for (String name : names) {
            if (StringUtils.toLowerCase(name)
                .equals(StringUtils.toLowerCase(arg)))
                return true;
        }
        return false;
    }
     ...
     */
    int consumeDocletOption(int idx, List<String> args, boolean isToolOption)
        ...
        for (Doclet.Option opt : docletOptions) {
            if (matches(opt, argBase)) {
                if (argVal != null) {
                    ...
                    case 0:
                        text = log.getText("main.unnecessary_arg_provided",
                            argBase);
                    ...
                    case 1:
                        ...
                        break;
                    default:
                        text = log.getText("main.only_one_argument_with_equals",
                            argBase);
                        throw new OptionException(ERROR, this::showUsage, text);
                    }
                } else {
                    ...
                    if (!opt.process(arg, args.subList(idx + 1,
                        idx + 1 + opt.getArgumentCount()))) {
                        m = -1;
                    }
    ...
    private Doclet preprocess(List<String> argv)
        ...
        for (int i = 0; i < argv.size(); i++) {
            ...
            if (arg.equals(ToolOptions.DUMP_ON_ERROR)) {
                // although this option is not needed in order to initialize the
                // doclet,
                // it is helpful if it is set before trying to initialize the
                // doclet
                ...
                if (userDocletName != null) {
                    if (apiMode) {
                        throw new IllegalArgumentException(
                            "More than one doclet specified (" +
            ...
            if (userDocletName != null) {
                ...
                if (cl == null) {
                    if (!fileManager.hasLocation(DOCLET_PATH)) {
                        ...
                        if (userDocletPath != null) {
                            for (String pathname : userDocletPath
                        ...
                        try {
                            ((StandardJavaFileManager) fileManager)
                                .setLocation(DOCLET_PATH, paths);
                        } catch (IOException ioe) {
                            if (apiMode) {
                                throw new IllegalArgumentException(
                                    "Could not set location for " +
                                        userDocletPath,
                                    ioe);
                            }
                            String text = log.getText(
                                "main.doclet_could_not_set_location",
                                userDocletPath);
                    ...
                    if (cl == null) {
                        // despite doclet specified on cmdline no classloader
                        // found!
                        if (apiMode) {
                            throw new IllegalArgumentException(
                                "Could not obtain classloader to load "

                                    + userDocletPath);
                        }
                        String text
                            = log.getText("main.doclet_no_classloader_found",
                                userDocletName);
                        throw new ToolException(CMDERR, text);
                    }
                }
                ...
                docletClass
                    = loadDocletClass(docletName, getClass().getClassLoader());
            } else {
        ...
        if (Doclet.class.isAssignableFrom(docletClass)) {
            log.setLocale(Locale.getDefault());  // use default locale for
                                                 // console messages
            try {
                ...
                String text = log.getText("main.could_not_instantiate_class",
                    docletClass.getName());
                throw new ToolException(ERROR, text);
            }
        } else {
            String text
                = log.getText("main.not_a_doclet", docletClass.getName());
            throw new ToolException(ERROR, text);
        }
        return doclet;
    }

    private Class<?> loadDocletClass(String docletName, ClassLoader classLoader)
        ...
        try {
            ...
            if (apiMode) {
                throw new IllegalArgumentException(
                    "Cannot find doclet class " + docletName);
            }
            String text
                = log.getText("main.doclet_class_not_found", docletName);
    ...
    private void checkOneArg(List<String> args, int index)
        ...
        if ((index + 1) >= args.size()
            || args.get(index + 1).startsWith("-d")) {
            String text
                = log.getText("main.requires_argument", args.get(index));
    ...
    private Locale getLocale(String localeName) throws ToolException {
        try {
            // Tolerate, at least for a while, the older syntax accepted by
            // javadoc,
            // using _ as the separator
            localeName = localeName.replace("_", "-");
            Locale l = new Locale.Builder().setLanguageTag(localeName).build();
            // Ensure that a non-empty language is available for the <HTML
            // lang=...> element
