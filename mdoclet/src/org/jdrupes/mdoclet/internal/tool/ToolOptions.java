...
import static org.jdrupes.mdoclet.internal.tool.Main.Result.OK;
import static org.jdrupes.mdoclet.internal.tool.ToolOptions.ToolOption.Kind.*;
...
public class ToolOptions {
    // The following are the names of options handled in the first pass of
    // option decoding,
    // in Start.preprocess.
    ...
    private List<ToolOption> supportedOptions = List.of(
        ...
        new ToolOption("--module-source-path", STANDARD, true) {
            @Override
            public void process(String arg) throws InvalidValueException {
                processCompilerOption(Option.MODULE_SOURCE_PATH, primaryName,
                    arg);
            }
        },
        ...
        new ToolOption("--upgrade-module-path", STANDARD, true) {
            @Override
            public void process(String arg) throws InvalidValueException {
                processCompilerOption(Option.UPGRADE_MODULE_PATH, primaryName,
                    arg);
            }
        },
        ...
        new ToolOption("--source -source", STANDARD, true) {
            ...
            public void process(String arg) throws InvalidValueException {
                ...
                processCompilerOption(Option.TARGET, Option.TARGET.primaryName,
                    arg);
            }
        },
        ...
        // This option exists so that it is documented in the command-line help.
        // It is actually implemented by expanding argv early on during
        // execution,
        // and can only be used when using the command-line and related
        // interfaces
    ...
    static class ToolOption {
        ...
        void process(String arg)
                throws OptionException, Option.InvalidValueException {
        }
        ...
        private String getKey(String optionName, String suffix) {
            return "main.opt."
                + optionName
                    ...
                    .replaceAll("[^A-Za-z0-9]+$", "")   // remove trailing
                                                        // non-alphanumeric
                    .replaceAll("[^A-Za-z0-9]", ".")    // replace internal
                                                        // non-alphanumeric
                + suffix;
        }
    }
    ...
    private IllegalOptionValue illegalOptionValue(String arg) {
        return new IllegalOptionValue(showHelper::usage,
            log.getText("main.illegal_option_value", arg));
    }
     ...
     */
    void processCompilerOption(Option option, String opt)
            throws Option.InvalidValueException {
     ...
     * @throws Option.InvalidValueException if the command-line option is invalid
     */
    private void processCompilerOption(Option option, String opt, String arg)
            throws Option.InvalidValueException {
    ...
    private OptionHelper getOptionHelper() {
        return new OptionHelper.GrumpyHelper(log) {
            ...
            @Override
            public boolean handleFileManagerOption(
                    com.sun.tools.javac.main.Option option, String value) {
