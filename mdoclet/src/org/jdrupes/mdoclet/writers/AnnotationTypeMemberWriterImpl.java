...
/*
 * Adapted copy required because of visibility restrictions of the original 
 * class in jdk.javadoc.internal.doclets.formats.html
 */
...
public class AnnotationTypeMemberWriterImpl extends AbstractMemberWriter
    ...
    private HtmlIdsWrapper htmlIdsWrapper;
    ...
    public AnnotationTypeMemberWriterImpl(SubWriterHolderWriter writer,
        ...
        this.kind = kind;
        htmlIdsWrapper = new HtmlIdsWrapper(configuration, htmlIds);
    }
    ...
    public void addSummary(Content summariesList, Content content) {
        writer.addSummary(HtmlStyle.memberSummary,
            switch (kind) {
            case REQUIRED -> HtmlIdsWrapper.ANNOTATION_TYPE_REQUIRED_ELEMENT_SUMMARY;
            case OPTIONAL -> HtmlIdsWrapper.ANNOTATION_TYPE_OPTIONAL_ELEMENT_SUMMARY;
            case ANY -> throw new UnsupportedOperationException(
                "unsupported member kind");
            },
    ...
    public Content getAnnotationHeaderContent(Element member) {
        ...
        return HtmlTree.SECTION(HtmlStyle.detail, content)
            .setId(htmlIdsWrapper.forMember(typeElement,
    ...
    @Override
    public Content getAnnotationDetails(Content annotationDetailsHeader,
            Content annotationDetails) {
        Content c
            = new ContentBuilder(annotationDetailsHeader, annotationDetails);
    ...
    protected void addSummaryLink(HtmlLinkInfo.Kind context,
            ...
            Content content) {
        Content memberLink = writer.getDocLink(context,
            utils.getEnclosingTypeElement(member), member,
    ...
    protected Content getSummaryLink(Element member) {
        String name = utils.getFullyQualifiedName(member) + "."
            + member.getSimpleName();
        return writer.getDocLink(HtmlLinkInfo.Kind.MEMBER_DEPRECATED_PREVIEW,
            member, name);
    }
