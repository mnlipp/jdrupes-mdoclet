...
abstract public class ExtraModuleInfoTransform implements TransformAction<ExtraModuleInfoTransform.Parameter> {
    ...
    public void transform(TransformOutputs outputs) {
        ...
        if (isModule(originalJar)) {
            ...
            String[] parts = originalJarName.split("-");
            String autoName = String.join(".", Arrays.copyOf(parts, parts.length - 1));
            addAutomaticModuleName(originalJar,  
                getModuleJar(outputs, originalJar), autoName);
        }
    }
